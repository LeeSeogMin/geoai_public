...
=====================================================================
같은 분석 절차(RF → 공간 CV → SHAP)를 공공 취약성이 아니라 상권에 적용한다.
...
이라 부른다. 실제 점포 수가 기대 공급량보다 적은 동을 추가 조사가 필요한 후보로 분류한다.
이 분류만으로 입점 여부를 결정하지 않는다.

네 가지를 확인한다.
  1) 공간 시차를 넣으면 예측이 좋아지는가 (넣고 뺀 비교)
  2) 무작위 CV와 자치구 단위 CV의 차이 — "강남에서 배운 모델이 노원에 통하는가"
  3) 라벨 셔플 대조군 — 결과와 무관한 라벨을 쓰면 점수가 낮아지는가
  4) 1)과 2)의 차이가 반복 결과의 범위보다 큰가 — 시드를 바꿔 비교한다
...
def cv_r2_seeded(X, y, cv, groups, seed: int) -> tuple[float, np.ndarray]:
    ...
    '모델 시드만 바꿨을 때 결과가 얼마나 달라지는가'를 계산한다.
    OOF R²는 본문 표와 같은 정의(전체 예측을 모아 한 번에 계산)이고,
    폴드별 R²는 폴드마다 점수가 얼마나 다른지 확인하기 위해 따로 계산한다.
    """
...
def main() -> None:
    ...
    # 자치구별로 검증했을 때의 성능. 점수가 낮은 자치구를 확인한다.
    per_gu = []
    ...
    # ── 3. 라벨 셔플 대조군 ─────────────────────────────────────────────
    print("\n" + "-" * 66)
    print("[3] 대조군 — 결과변수를 섞으면 점수가 낮아지는가")
    print("-" * 66)
    ...
    print(f"  → 셔플 결과와 실제 점수 {r2_full_group:.3f}를 비교해 모델이 자료의 관계를 학습했는지 확인한다")

    # ── 4. SHAP 기여도 ──────────────────────────────────────────────────
    print("\n" + "-" * 66)
    print("[4] SHAP — 어떤 피처가 기대 공급량 예측에 크게 기여하는가")
    print("-" * 66)
    import shap
    ...
    #       코드를 다시 실행할 때 달라지는 요소를 더 많이 반영한다.
    #       본 실행의 GroupKFold는 shuffle 없이(=배정 고정) 돌렸으므로, (B)의
    #       결과는 본 실행 수치의 재현이 아니라 반복 결과의 비교 기준으로 읽는다.
    #
    # 이 블록은 앞 절의 계산값을 변경하지 않는다. 위 [1]~[5]를 마친 뒤 실행하며,
    # 난수를 쓰는 객체를 새로 만들어 쓰므로 앞 절의 수치는 그대로 보존된다.
    print("\n" + "-" * 66)
    print("[6] 시드를 바꾸면 위 두 차이가 얼마나 달라지는가")
    print("-" * 66)
    ...
    for tag, desc in (("A", "모델 시드만 교체 (폴드 배정 고정, 42=본 실행 재현)"),
        ...
        if tag == "B":
            print(f"\n  → 재려는 차이(개선 {r2_full_group - r2_demand:+.3f}, "
                  ...
                  f"\n    차이를 반복 결과의 범위 및 표준편차와 비교한 뒤 방향을 해석한다.")
    ...
    print("       새 매장이 기존 매장의 고객 수를 얼마나 줄이는지는 이 모델이 답하지 못한다.")
