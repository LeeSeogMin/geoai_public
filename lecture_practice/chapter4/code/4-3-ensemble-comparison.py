...
# ============================================================
# 9. 시드 반복 — 4절의 과대추정 차이가 반복 결과의 차이보다 큰가
# ============================================================
# 4절이 보고한 과대추정(+0.010 ~ +0.013)은 같은 절의 폴드 간 표준편차
# (Block CV 기준 0.015~0.021)보다 작다. 차이가 이 표준편차보다 작으면 방향을
# 확정하기 어렵다. 반복 실행에서 과대추정 차이가 달라지는 범위를 계산한다.
#
# 무엇을 바꿔 반복할지 정한다. 과대추정은 무작위 CV와 Block CV의 성능 차이다.
# 이 차이에 영향을 주는 조건은 **196개 블록을 다섯
# 폴드에 어떻게 배정하는가**다. 각 폴드에서 제외하는 블록에 따라 검증
# 지역이 학습 지역에서 얼마나 멀어지는지가 달라지기 때문이다. 모델 시드만
# 바꾸는 반복은 블록 배정 조건을 바꾸지 않는다. 그래서 그 반복에서 결과가 일정해도
# "과대추정이 실재한다"는 근거가 되지 못한다.
#
# 세 가지로 나눠 돌린다.
...
#       시드를, Block CV는 블록→폴드 배정 시드를 바꾼다. 확인할 조건을
#       실제로 바꾸는 반복이며, 판정의 근거는 이쪽이다.
#   (C) 픽셀 표집 시드 교체 — 클래스별 1,500개를 다시 뽑는다. 자료가
#       달라져도 같은 결론이 나오는지 본다(폴드 배정·모델 시드는 고정).
#
# 이 블록은 앞 절의 계산값을 변경하지 않는다. 1~8절을 마친 뒤 실행하며,
...
# (B) 모델 시드 + 폴드 배정 시드 — 모델과 폴드 배정 조건을 바꾼다
rows_b = []
for s in REPEAT_SEEDS:
...
print("    폴드 간 표준편차보다 작다. 차이의 방향을 확정하기 전에 이 표준편차와 비교한다.")
