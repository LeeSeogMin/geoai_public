...
def main():
    ...
    log("[표 읽는 법] '±1SE' 열은 기각률의 몬테카를로 표준오차 자체다(=sqrt(p(1-p)/R)).")
    log("  뒤의 '대조군 2' 블록은 같은 표준오차의 2배를 95% 신뢰구간 반폭으로 찍는다.")
    log("  두 값은 같은 양이 아니라 1배와 2배이므로 폭을 인용할 때 어느 쪽인지 밝힌다.")
    log("  MDE 열은 로그 스케일 계수다. 백분율 효과는 exp(계수)-1로 환산한다.")
    log()
    ...
    for wname, eff, sp in worlds:
        ...
        log(f"{'설계':<24}{'평균추정':>9}{'편향':>9}{'경험SD':>9}{'평균SE':>9}"
            f"{'기각률':>8}{'±1SE':>7}{'MDE':>9}{'단위':>6}{'노출률':>8}")
    ...
    log("아래 ± 는 몬테카를로 표준오차의 2배, 즉 95% 신뢰구간 반폭이다.")
    log("세계별 요약표의 '±1SE' 열은 표준오차 자체이므로 이 값의 절반이다.")
    inflated = []
    for _, r in C_tab.iterrows():
        ...
        if lo > ALPHA:
            ...
            verdict = "명목 수준 포함"
        log(f"  {r['설계']:<24} {r['기각률']:.3f} ± {2 * r['MC 오차']:.3f}(95% CI 반폭)  "
    ...
    log("결과변수가 로그 매출이므로 MDE(%) 열은 exp(MDE 로그)-1로 환산한 값이다.")
