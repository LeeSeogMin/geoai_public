...
def main():
    ...
    print(f"  → 재현율 {r0:.3f} → {r1:.3f} (변화 {r1-r0:+.3f})")
    print("  ※ 거짓양성을 지우면 정밀도가 오른다. 필터가 참양성 경계까지 깎으면 재현율이 떨어질 수 있다.")
    print("    위 재현율 두 값이 같다면 지워진 픽셀이 모두 거짓양성이었다는 뜻이다(이 시드 자료가 그렇다).")
    print("    재현율이 1에 못 미치는 원인은 필터가 아니라 예측 마스크 자체의 경계 침식(거짓음성)이다.")
