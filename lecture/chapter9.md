...
□ 이 장의 합성 자료에는 참 정책효과가 **−0.80%p**로 심겨 있음. 방금 계산한 −1.306%p는 참값보다 1.6배 넘게 큼  
  ...
  ○ 결론을 먼저 밝히면, 6절의 이중차분으로 다시 계산한 값은 **−0.822%p**로 참값에 훨씬 가까워짐  
...
□ 6절부터 11절까지는 설명과 실습을 나누지 않음 — 각 절 첫머리에 실행할 명령이 나오고 그 아래에 코드와 실제 실행 결과가 이어짐. 순서대로 따라가면 이 장의 모든 수치를 직접 만들게 됨  

□ 먼저 패키지를 설치함. 인터넷도 GPU도 필요 없고 노트북에서 끝까지 돌아감  
...
| 순서 | 실행할 파일 | 하는 일 | 절 |
| ---: | --- | --- | ---: |
| 1 | `9-0-simdata-prep.py` | 합성 자료 세 벌 생성 | — |
| 2 | `9-1-protected-area-forest-loss.py` | 정책효과 추정·위험 예측·우선순위표 | 6·7·10 |
| 3 | `9-2-causal-heterogeneity.py` | 이질성 추정과 표적화 | 8 |
| 4 | `9-3-supply-chain-due-diligence.py` | 컷오프·예산 배분·공급망 사례 | 9·11 |

○ 자료 생성과 분석이 분리되어 있으므로 **1번을 먼저 실행해야 함** — 나머지 세 코드는 1번이 만든 파일을 읽고, 2~4번 사이에는 순서 의존이 없음  
○ 1번이 만드는 자료는 세 벌이며, 어느 절이 어느 자료를 쓰는지는 파일 이름으로 갈림  

```text
# 출처: lecture_practice/chapter9/results/9-0-simdata-prep.log
  산림손실 패널 6000행(300단위×20년, 보호 150) → forest_loss_panel.parquet
  참 CATE 격자 1600개(40×40, 보호 692) → cate_grid.parquet
  조달 구역 240개(참 위반확률 평균 0.128, 실현 위반 25건) → sourcing_districts.parquet
```

○ **6·7·10절이 쓰는 자료(300단위 20년 패널)와 8절이 쓰는 자료(40×40 격자)는 서로 다른 합성 자료임.** 패널에는 모든 격자에 같은 정책효과가 심겨 있어 이질성을 물을 수 없고, 8절은 이질성을 채점하려고 효과 크기가 격자마다 다르게 심긴 별도 자료를 씀. **두 자료의 수치를 같은 표본의 분해로 읽으면 안 됨**  
○ 본문에 인용한 출력은 모두 `lecture_practice/chapter9/results/`의 실행 로그에서 그대로 가져온 것이며, 어느 파일에서 왔는지를 출력 블록 첫 줄에 적어 두었음 — 화면에 나온 값과 그 파일을 한 줄씩 대조하면 됨  

## 6. 정책효과를 추정한다

### 6.1 이중차분으로 선택 편향을 걷어낸다

□ 먼저 실행함. 2절의 표와 이 절의 추정치, 7절의 위험 예측, 10절의 우선순위표가 모두 이 두 명령의 출력임  
...
```python
# 주석에 붙인 출력의 출처: lecture_practice/chapter9/results/9-1-protected-area-forest-loss.log
import statsmodels.formula.api as smf

# [1] 사후 시점만 놓고 안-밖을 뺀 값 — 2절이 보여 준 잘못된 비교
post = df[df["post"] == 1]
naive = (post[post.treated == 1]["loss"].mean()
         - post[post.treated == 0]["loss"].mean())
# 출력: naive 추정(사후 안-밖 차이): -1.306 %p  ← selection bias로 효과 과대추정

# [2] 이중차분 — 상호작용 항의 계수가 정책효과이고, 표준오차는 단위별 군집 강건
res = smf.ols("loss ~ treated + post + treated:post", data=df).fit(
    cov_type="cluster", cov_kwds={"groups": df["unit"]})
# 출력: === [심화] DID 추정 (전체 표본, n=6000) ===
#       DID 효과(treated:post): -0.822 %p  (SE=0.020, p=0.0000)

# [3] 공통 지지 구간으로 표본을 제한하고 같은 회귀를 다시 돌림
matched = df[(df.pressure >= low) & (df.pressure <= high)]
# 출력: 공통 지지 구간 압력 [0.36, 0.84] → 매칭 후 단위 232개
#       === [심화] DID 추정 (매칭(공통지지) 표본, n=4640) ===
#       DID 효과(treated:post): -0.804 %p  (SE=0.022, p=0.0000)
```
...
□ 화면에 나온 값에서 읽을 것  
  ...
  ○ p값이 작다는 것과 참값에 가깝다는 것은 다른 말임 — 두 DID 모두 p = 0.0000이지만 참값과의 거리는 −0.022%p와 −0.004%p로 다섯 배 넘게 차이가 남. 참값을 알고 대조할 수 있는 것은 합성 자료이기 때문이며, 실제 자료에서는 이 대조가 불가능함  
...
□ 개발 압력이 0.1인 오지와 0.9인 도시 인접지는 시간에 따른 변화 양상도 다를 수 있음  
  ○ 두 집단의 공변량 분포가 겹치는 구간으로 표본을 제한하면 이 위험이 줄어듦. 이 실습 자료에서 그 구간은 압력 [0.36, 0.84]이고, 제한하면 300단위 중 232단위(n = 4,640)가 남음 — 위 실행의 `공통 지지 구간 압력 [0.36, 0.84] → 매칭 후 단위 232개` 줄에서 그대로 온 값임  
...
## 7. 보호가 없을 때의 위험을 예측한다

□ 이 절의 수치는 6.1에서 실행한 `9-1-protected-area-forest-loss.py`의 뒷부분 출력임. 6절을 건너뛰었다면 여기서 실행함  

```bash
python lecture_practice/chapter9/code/9-1-protected-area-forest-loss.py
```

### 7.1 학습 표본을 통제구역으로 제한한다

□ 6절은 이미 시행한 정책의 평균 효과를 답함. 다음 배분을 정하려면 다른 값이 필요함 — 아직 보호되지 않은 격자 가운데 **보호가 없다면 어디가 크게 훼손될 것인가**  
...
```text
# 출처: lecture_practice/chapter9/results/9-1-protected-area-forest-loss.log
=== [2층 AI] 공간 명시적 ML 벌채위험 예측 (2겹 공간 블록 CV, 통제구역 학습) ===
공간 피처 포함 : R²=0.307, RMSE=0.459
공간 피처 제외 : R²=-0.080, RMSE=0.570
→ 공간 시차(미관측 공간효과) 기여: R² +0.386 (공간 ML 정당)
피처 중요도(설명가능성): spatial_lag_pre_loss=0.569, habitat_value=0.219, pressure=0.212
```
...
통제구역 보정표본 n=150 · 목표 포함확률 0.90 conformal 계열(교육용 OOF 근사)
OOF 경험적 포함률 = 0.907 (목표 0.90)
구간 반폭(%p): 중앙값 0.511, 범위 [0.125, 1.327]
신뢰등급 분포: 높음 100 · 중간 100 · 낮음 100
```
...
### 8.1 평균 하나로는 배분을 정할 수 없다

□ 6절의 이중차분은 보호구역 정책이 손실률을 평균 0.80%p 낮췄다고 답함  
...
□ 세 번째 코드를 실행함. 8절의 표 세 개와 그림 9.2가 모두 이 한 번의 실행에서 나옴  
...
```text
# 출처: lecture_practice/chapter9/results/9-2-causal-heterogeneity.log
표본 N = 1600 (40×40 격자),  보호구역(처치) = 692개
참 CATE 범위 = [-1.60, -0.20] %p, 참 ATE = -0.891 %p
공변량: ['pressure', 'enforce', 'habitat', 'elevation', 'slope']  (elevation·slope는 CATE와 무관한 잡음 대조군)
```

□ 이 절은 **다른 합성 자료**를 씀(5절 참고). 손실 수준은 개발 압력이 정하고, 보호가 막아 주는 손실의 크기는 **집행역량**이 따로 정하도록 설계됨  
  ○ 참 CATE는 τ(x) = −(0.20 + 1.40 × 집행역량)으로 심었으며, 격자 1,600개 중 692개가 보호됨. 참 CATE의 범위는 [−1.60, −0.20]%p, 참 ATE는 −0.891%p임  

### 8.2 R-러너가 교란을 걷어내는 방식
...
```python
# 주석에 붙인 출력의 출처: lecture_practice/chapter9/results/9-2-causal-heterogeneity.log
# [1] 누이선스 교차적합 — 자기 자신을 학습한 모형으로 자기를 예측하지 않음
...
e_hat[te] = np.clip(rf_e.predict_proba(X[te])[:, 1], 0.02, 0.98)   # positivity 절단

# [2] 잔차 — 공변량으로 설명되는 부분을 뺀 나머지
y_res = df["y"].values - m_hat
d_res = df["treated"].values - e_hat

# [3] 유사결과 Ỹ/D̃를 목표로, D̃²를 가중치로 하는 가중 회귀
pseudo = np.where(np.abs(d_res) > 1e-8, y_res / d_res, 0.0)
lo, hi = np.nanpercentile(pseudo, [1, 99])
pseudo = np.clip(pseudo, lo, hi)          # 극단값이 소수 잎을 지배하지 않게 윈저화
rf = RandomForestRegressor(n_estimators=400, min_samples_leaf=8,
                           random_state=42, n_jobs=1)
rf.fit(X, pseudo, sample_weight=d_res ** 2)
# 출력: [1] CATE 복원: 참 τ(x) vs 추정 τ̂(x) (out-of-fold)
#       Pearson 상관 r = 0.744,  Spearman ρ = 0.731
```
...
| 이중강건(AIPW) ATE | −0.937 | −0.047 |
| R-러너 τ̂ 평균 | −0.942 | −0.052 |
...
### 9.3 비용비를 바꾸면 문턱이 움직인다

□ 네 번째 코드를 실행함. 이 절의 두 표와 그림 9.3, 11절의 규칙 비교표가 모두 이 한 번의 실행에서 나옴  
...
```python
# 주석에 붙인 출력의 출처: lecture_practice/chapter9/results/9-3-supply-chain-due-diligence.log
# [1] 비용 최적 확률 컷오프 p* = C_FP/(C_FP+C_FN) = 1 − 임계비
def prob_cutoff(cost_ratio):
    return 1.0 / (1.0 + cost_ratio)

# [2] 기대 총비용 — 실사 1건은 1단위, 놓친 위반 1건은 R단위
...
# [3] 비용비만 바꿔 가며 컷오프에 걸리는 구역 수를 셈
for R in [1, 3, 5, 10, 20, 50, 100]:
    sel = p_hat > prob_cutoff(R)
# 출력: R=C_FN/C_FP  임계비  확률컷오프p*  실사건수  기대총비용  놓친위반(기대건수)
#                1   0.500       0.5000         7        28.8             26.29
#               10   0.909       0.0909       123       149.5              5.21
#              100   0.990       0.0099       237       208.5              0.02
```

○ 실사 1건 = 1단위, 240구역 기준. 아래 표는 그 출력의 `[3단계 비용 곡선]` 구간 일곱 행을 그대로 옮긴 것임  

| R = C_FN/C_FP | 확률 컷오프 p\* | 실사 건수 | 기대 총비용 | 놓친 위반(기대 건수) |
| ---: | ---: | ---: | ---: | ---: |
| 1 | 0.5000 | 7 | 28.8 | 26.29 |
| 3 | 0.2500 | 32 | 74.3 | 18.27 |
...
### 10.1 우선순위표는 위험 → 신뢰 → 집중도 순으로 읽는다

○ 앞 절들을 거치면 격자마다 관측 손실률, 위험 예측값, 구간 반폭, 신뢰등급, 순위가 붙음  
○ 아래 표는 6.1에서 실행한 `9-1-protected-area-forest-loss.py`의 `[불확실성] ml_risk 기준 행정구역 우선순위` 구간임  
...
○ **전체는 개선됐는데 취약지역이 정체하면 배분을 다시 검토할 근거가 됨.** 이 분해를 별도 보고서가 아니라 10.1의 우선순위표에 취약계층 인구 비율 열로 추가하면, 상위 구역이 취약지역과 겹치는지 한눈에 확인됨  

### 10.3 무엇을 확정하고 무엇을 조건부로 읽을 것인가
...
## 11. 사례 전체: 공급망 산림 리스크 실사

□ 이 절의 수치는 9.3에서 실행한 `9-3-supply-chain-due-diligence.py`의 뒷부분 출력임. 9절을 건너뛰었다면 여기서 실행함  
...
□ 공공 사례의 목적이 "같은 예산으로 막는 손실을 최대화"였다면, 여기서는 **기대 총비용을 최소화**함  
  ○ 효과 추정과 이질성 분석(6·8절)은 하지 않음. **물음이 인과가 아니라 배분이기 때문임**  

○ 국내 식품·목재 기업이 표준위험 생산국 4개국의 하위 행정구역 240곳에서 원료를 조달하는 상황을 둠  

```text
# 출처: lecture_practice/chapter9/results/9-3-supply-chain-due-diligence.log [1단계 관측]
  분위    p50   p75   p90   p95   p99
  손실률% 0.51   0.89   1.58   2.28   4.19
  평균 0.780% · 최대 8.761%
  손실 면적 상위 10% 구역(24개)이 전체 손실 면적의 39.6%를 차지
```
...
○ 위반확률은 추정하지 않고 로지스틱 위험함수로 **선언**함. 규칙으로 정의된 값을 지도학습으로 복원하면 9.1의 순환논증이 위험함수에도 그대로 적용되기 때문임. 이 설정에서 참 위반확률은 평균 0.128, 범위 [0.006, 0.827]이고, 실현 위반은 240구역 중 25건임  
...
| --- | ---: | ---: | ---: |
| 무실사 | 0 | 307.8 | 0.0 |
| 제도 기본선 3%(표준위험 점검률 전용) | 7 | 272.5 | +35.2 |
...
□ 세 가지를 읽음  
  ...
  1. **제도 점검률을 그대로 옮기면 비용 최적보다 크게 더 씀.** 이 사례의 조달국이 표준위험이므로 옮겨 붙을 값은 3%이고, 그대로 쓰면 7건·272.5로 비용 최적(149.5)보다 123.0단위를 더 씀. 고위험 기준 9%로 올려도 233.3이라 83.8단위, 즉 실사 1건 비용의 84배를 더 씀
...
```text
# 출처: lecture_practice/chapter9/results/9-3-supply-chain-due-diligence.log [7단계 대조군]
  (a) 신호 0 대조군 — 위반을 300회 다시 뽑아 실현 비용으로 채점
      세계                     위험순위        무작위         차이(순위−무작위)
      신호 있음                 208.2      297.3       -89.0 (SD 32.0)
      신호 0                  297.2      297.2        +0.0 (SD 27.2)
```
...
○ **사후 단순 비교는 선택 편향을 그대로 안음.** 실습에서 −1.306%p로 참값 −0.80%p의 1.6배가 나왔고, 이중차분으로 비교 방식만 바꾸자 −0.822%p가 됨 (2절, 6.1)  
...
○ **위험 모형은 통제구역으로 학습함.** 처치구역 사후 손실로 학습하면 예측 대상에 정책효과가 섞이는데 성능 지표는 정상으로 나옴 (7.1)  
○ **평균효과로는 배분을 정할 수 없음.** 손실이 큰 곳을 고르는 전략이 무작위보다 못했음(83.28 대 86.17). 손실 수준과 보호효과가 서로 다른 변수이기 때문임 (8.1, 8.4)  
○ **산포가 있다는 사실만으로는 이질성의 근거가 되지 않음.** GATES·BLP로 검정하고, BLP 기울기가 1보다 작으면 τ̂를 순위 산정에만 씀 (8.3)  
○ **문턱은 자료가 아니라 두 오류의 비용이 정함.** 자료도 예측도 그대로인데 실사 대상이 7건에서 237건까지 움직이며, 움직인 것은 비용비뿐임 (9.2, 9.3)  
○ **예산이 구속되면 문턱 문제가 순위 문제로 바뀜.** 제도가 제시하는 점검률은 감독 당국의 자원 배분 결과이지 기업의 목표율이 아니며, 표준위험 점검률 3%를 그대로 쓰면 비용 최적보다 123.0단위, 고위험 9%를 써도 83.8단위를 더 씀 (9.4, 11절)  
