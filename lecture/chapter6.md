...
□ 필터는 두 규칙임 — 식생지수 차분이 0.25를 넘는 픽셀만 남기고, 연결된 덩어리가 1헥타르(100픽셀) 미만이면 지움  
  ...
  ○ **정밀도만 0.572에서 1.000으로 오르고 재현율은 0.865에서 그대로임.** 이 비대칭이 우연이 아니라는 것을 6절에서 계산으로 확인함  
...
□ **변화 탐지는 별도 계열이 아니라 두 시점을 입력으로 받는 세그멘테이션**임. 계산의 뼈대는 같고 달라지는 지점이 둘 있음  
  ...
  2. 검증 지표의 우선순위가 반대로 놓일 수 있음. 재난 피해 평가에서는 놓친 피해가 비용이 크고, 단속 행정에서는 잘못 잡은 변화가 비용이 큼(6.2)
...
○ 2절의 사례는 산출 단위가 구역이었으므로 개체 단위 계산이 필요 없었음. 10절의 부지 탐색은 산출 단위가 개체이므로 7·8절의 계산이 전부 필요함  
...
○ 이 4/s 관계는 8절에서 다시 나옴. 같은 침식 깊이라도 작은 필지가 상대적으로 더 많이 깎이는 이유가 여기에 있음  
...
○ 두 값을 정한 뒤에는 6.3처럼 필터 전후의 정밀도·재현율로 효과를 측정해 확정함  

## 5. 마스크는 어디서 오는가

○ 실습에서는 마스크를 미리 만들어 둔 자료에서 불러오지만, 그 마스크가 어떤 계산에서 나오는지 알아야 오차의 방향(8절)을 이해할 수 있음  
...
○ **이 구조가 이 장의 오차와 이어짐.** 해상도를 줄였다 되돌리는 과정에서 경계의 위치 정보가 흐려지고, 그 결과 예측 마스크의 가장자리가 실제보다 안쪽으로 깎임. 이것이 8절에서 다룰 **경계 침식**이며, 이 장에서 가장 큰 영향을 미치는 오차임  
...
| 순서 | 실행할 파일 | 하는 일 | 절 |
| ---: | --- | --- | ---: |
| 1 | `6-0-simdata-prep.py` | 두 시점 NDVI 래스터와 변화 마스크 생성 | 6 |
| 2 | `6-1-change-detection-policy.py` | 변화 마스크 채점·거짓 변화 필터링·구역 집계 | 6, 9.1 |
| 3 | `6-0b-site-simdata-prep.py` | 도시 블록 래스터 생성(도로·건물·부지 정답·오차 3종) | 7 |
| 4 | `6-2-site-sourcing.py` | 객체화·기하 피처·요건 필터·편향 보정·대조군·절단선 | 7~10 |

○ 2번은 1번이 만든 `change_masks.npz`를, 4번은 3번이 만든 `urban_block.npz`를 입력으로 받으므로 **준비 스크립트를 먼저 실행**해야 함. 1·2번과 3·4번은 서로 독립이라 한쪽 자료를 다시 만들어도 다른 쪽 수치는 바뀌지 않음  
○ 네 코드 모두 인터넷 없이 돌아가며 GPU도 쓰지 않음  
...
○ 본문에 인용한 출력은 모두 `lecture_practice/chapter6/results/`의 실행 로그에서 그대로 가져온 것이며, 어느 파일에서 왔는지를 출력 블록 첫 줄에 적어 두었음 — 화면에 나온 값과 그 파일을 한 줄씩 대조하면 됨  

## 6. 마스크를 채점하기

□ 검증은 두 층위로 나눔 — 픽셀 단위 채점(마스크가 정답과 얼마나 겹치는가)과 객체 단위 채점(개체를 몇 개 찾았고 각 개체의 면적을 얼마나 정확히 쟀는가)  
  ○ 어느 층위를 쓸지는 3.2에서 정한 산출 단위가 정함. 구역 면적 집계가 최종 산출물이면 픽셀 단위로 충분하고, 개체 목록이 판단 단위이면 픽셀 지표만으로는 부족함. 이 절은 픽셀 단위를 다루고, 객체 단위는 8절에서 다룸  

□ 첫 번째와 두 번째 코드를 실행함. 앞의 것이 두 시점 NDVI 래스터와 변화 마스크를 만들고, 뒤의 것이 그 마스크를 채점·필터·집계함. 6절과 9.1의 숫자는 전부 이 두 명령의 출력임  
...
```text
# 출처: lecture_practice/chapter6/results/6-0-simdata-prep.log
  래스터 128×128(픽셀 10m, 0.01ha) | 진짜 변화 2001px, 예측 변화 3022px
  → change_masks.npz 저장 완료
```

○ 2절에서 먼저 본 무대가 이 출력임 — 진짜 변화 2,001픽셀에 예측 변화 3,022픽셀이고, 픽셀 하나가 0.01헥타르임  
...
def iou_prf(pred, true):
    ...
    prec = inter / pred.sum() if pred.sum() else 0.0        # 판정한 것 중 진짜
    rec = inter / true.sum() if true.sum() else 0.0         # 진짜 중 잡아낸 것
...
#         → 정밀도가 낮다 = 거짓양성(구름·계절 잡음)이 섞여 있다.
```

□ 화면에 나온 값에서 읽을 것  
  ○ 재현율 `0.865`는 높은데 정밀도가 `0.572`로 낮음 — 진짜 변화는 대부분 잡았으나 그 판정에 가짜가 많이 섞였다는 뜻임  
  ○ IoU `0.525`는 정밀도와 재현율을 함께 반영한 값이라 둘 중 낮은 쪽에 끌려 내려감  
...
□ 4.3의 마스킹 규칙과 최소 면적 필터는 **예측 안의 가짜를 깎는 장치**임. 그래서 정밀도만 올리고 재현율은 올리지 못함  
  ○ 같은 실행이 이어서 3·4단계 필터를 걸고 다시 채점함  
...
□ 화면에 나온 값에서 읽을 것  
  ○ 정밀도가 `0.572`에서 `1.000`으로 오르는 동안 재현율은 `0.865` 그대로임. 2절의 표에 적은 비대칭이 이 출력임  
  ○ 이유는 계산 구조에 있음. 모델이 애초에 놓친 픽셀은 예측 안에 존재하지 않으므로, **예측을 깎는 어떤 연산으로도 되살릴 수 없음**  
  ○ 재현율을 올리려면 판정 임계를 낮추거나 학습 설정을 바꿔야 함. 후처리 단계에서는 방법이 없음  

□ 재현율이 그대로인 이유  
  ○ 필터가 지운 픽셀이 전부 거짓양성이었기 때문임. 필터 후 정밀도가 `1.000`이므로 남은 예측은 모두 진짜 변화이고, 재현율이 `0.865`에서 움직이지 않았으므로 진짜 변화와 겹치는 픽셀 수도 그대로임  
  ○ 두 조건이 참양성을 비켜 간 결과임. 개발 패치는 NDVI가 0.65대에서 0.15대로 떨어져 ΔNDVI가 임계 `0.25`를 크게 넘고, 반지름 9~14픽셀 원형이라 넓이도 최소면적 `100px`을 여유 있게 넘김 (`6-0-simdata-prep.py`)  
  ○ 반면 거짓양성은 계절성 잡음 위에 뿌린 산발 픽셀이라 ΔNDVI가 임계에 못 미쳐 첫 조건에서 걸러짐  
  ○ 재현율이 `1.000`에 못 미치는 원인은 필터가 아니라 예측 마스크를 만들 때 준 경계 침식임. 이 손실은 필터를 걸기 전부터 있었음  
  ○ **다른 자료에서는 필터가 참양성 경계까지 깎아 재현율이 떨어질 수 있음.** 코드가 필터 전후 재현율을 나란히 찍는 이유가 그것을 눈으로 확인하라는 것임  
...
□ 세 번째와 네 번째 코드를 실행함. 앞의 것이 정답을 아는 도시 블록 래스터를 만들고, 뒤의 것이 객체화·기하 피처·요건 필터·편향 보정·대조군·절단선을 한 번에 계산함. **7절부터 10절까지의 표와 출력은 모두 이 한 번의 실행에서 나옴**  
...
```text
# 출처: lecture_practice/chapter6/results/6-0b-site-simdata-prep.log
  래스터 512×512 (픽셀 1m = 1㎡, 총 26.2ha)
  도로 50,005㎡ | 기존 건물 40동 (26,877㎡, 배치 실패 0) | 오탐 덩어리 5개(배치 실패 0)
  유휴 부지 정답 32필지 (47,294㎡, 배치 실패 0) | 그중 맹지 5필지
  필지 면적 최소 432㎡ / 중앙 1,155㎡ / 최대 3,685㎡
  오차 심기 — 두 겹 침식 8필지 | 과분할 3필지(id [1, 2, 9]) | 큰 오탐 5개 + 산발 얼룩
  예측 마스크 55,313㎡ (정답 대비 1.170배)
```

○ 정답과 오차를 함께 심어 두었으므로 채점이 가능함. 심어 둔 오차는 세 가지 — 경계 침식, 오탐, 과분할이고, 8.3의 대조군은 이 셋을 하나씩만 빼서 돌린 결과임  
○ 예측 마스크의 총 면적이 정답의 `1.170배`라는 점을 기억해 둠. 8.2에서 개별 필지의 면적이 오히려 작게 나오는 것과 나란히 놓고 읽음  
...
□ 화면에 나온 값에서 읽을 것  
  ○ 연결요소가 **949개** 나왔는데 그중 **909개가 50㎡ 미만 얼룩**이었고 분석 대상은 **40개**만 남음  
...
○ 다섯 값을 개체마다 계산하는 부분이 `object_table`임. 둘레는 픽셀 사이 경계선을 세는 크랙 둘레를 쓰고, 최소폭은 거리변환의 최댓값을 두 배 한 내접원 지름으로 대신함  
...
□ 화면에 나온 값에서 읽을 것  
  ○ 허용오차를 0으로 두면 분석 대상 40개 중 **37개**가 접면 부족으로 탈락하고, 남은 **3개 중 진짜 필지는 0개**임. 위 네 단계가 그대로 숫자로 나타난 결과임  
  ○ 그래서 도로에서 일정 거리 안을 접면으로 인정하는 허용오차를 두고, 그 크기를 마스크의 침식 깊이보다 크게 잡음. 이것은 임계를 느슨하게 하는 타협이 아니라 **오차 구조에 맞춰 판정 규칙을 정의하는 절차**임  
  ○ 이 실습에서 쓴 허용오차는 `FRONT_TOL = 3.0`, 즉 3m임  
...
○ 코드에서 이 두 임계가 상수로 적혀 있음 — `MATCH_OBJ = 0.20`은 객체 면적의 20% 이상이 어느 필지와 겹쳐야 그 필지의 객체로 보는 기준이고, `MATCH_PARCEL = 0.50`은 필지 면적의 절반 이상이 덮여야 검출로 보는 기준임  
...
□ 화면에 나온 값에서 읽을 것  
  ○ 검출률 `0.938`은 32필지 중 30필지를 찾았다는 뜻이고, 여기에 정답과 무관한 오탐 객체가 `5개` 더 얹혀 있음  
...
□ 화면에 나온 값에서 읽을 것  
  ...
  ○ 무작위 오차라면 표본을 늘리면 사라지지만, 경계 침식처럼 항상 면적을 깎는 오차는 평균해도 남음  
  ○ 큰 절반 `-0.112`, 작은 절반 `-0.136`으로 편향의 크기가 개체 크기에 달려 있음. 4.1에서 본 대로 경계 한 겹이 지우는 픽셀 수는 대략 둘레만큼이므로 정사각형 대상의 상대 손실은 4/한 변의 길이이고, **같은 침식 깊이라도 작은 객체가 상대적으로 더 많이 깎임**  
  ○ 과분할된 세 필지는 참 면적 2,394~2,511㎡인데 가장 큰 조각만 보면 1,026~1,216㎡로, 참 면적 대비 0.41~0.51임. 조각 하나만 보고하면 필지 크기를 절반 안팎으로 잘못 세게 됨  

> **주의.** 총량이 맞는 것과 개체별로 맞는 것은 다름. 7절 첫머리의 출력에서 예측 마스크의 총 면적이 정답의 1.170배였는데, 여기서는 개별 필지의 면적이 중앙값 12.5% 작게 나옴. 오탐이 총량을 부풀리고 침식이 개별 개체를 깎으면 두 오차가 상쇄되어 총량 비교로는 아무것도 드러나지 않음.
...
○ 대조군 없이 "침식 때문에 후보를 놓쳤다"고 쓰면 그것은 **해석이지 측정이 아님**. C1에서 오탈락이 0이 되어야 그 주장이 성립함. 10절이 이 세 대조군의 실측을 담고 있음  
...
□ 6절에서 실행한 `6-1-change-detection-policy.py`가 이어서 이 계산을 함. 128×128 래스터를 4×4=16개 구역으로 나누고 필터를 거친 마스크를 포갬  
...
□ 화면에 나온 값에서 읽을 것  
  ○ 16개 구역 중 15개에 변화가 잡히고 합계가 `17.3헥타르`임. 표에 찍히지 않은 구역 3은 변화 픽셀이 0이라 `changed_ha > 0` 조건에서 빠졌고, 저장된 CSV에는 16행이 그대로 들어 있음  
  ○ 상위 세 구역이 `2.93`·`2.47`·`2.30`헥타르로 촘촘함. 구역 8위와 9위는 둘 다 `0.92`헥타르로 같은 값이라 순위가 정렬 순서에 따라 갈림 — **순위표를 읽을 때는 값의 간격을 함께 봄**  
  ○ **단위 경계를 어떻게 잡느냐가 순위를 바꿈.** 여기서는 격자로 나눴지만 실제 행정경계로 나누면 상위 구역이 달라질 수 있음  
...
□ 이 차이 때문에 절단형 결정에는 두 계산이 더 필요함 — 체계적 편향을 되돌리는 보정(9.3), 임계 자체를 비용으로 계산하는 절차(9.4)  
  ○ 어느 형태든 탈락하거나 순위에서 밀린 대상의 목록을 별도 파일로 남김. 이 목록이 없으면 오탈락을 사후에 셀 수 없음. 10절의 실습은 `ch6_site_candidates.csv`와 `ch6_missed_sites.csv` 두 파일로 이를 남김  
...
○ 둘레 모형의 ĉ가 `0.916`으로 이론값 1에 가깝게 나옴. 심어 둔 오차가 실제로 한 겹 침식이었다는 것이 이 값에서 확인됨  

> **주의.** 보정 계수는 표본을 어느 모집단에서 뽑았는지에 달려 있음. 10절의 실습에서 같은 방식으로 계산해도 실사 검토 풀 표본은 1.128, 검출된 필지 전수는 1.217, 검출된 모든 객체는 1.453이 나옴. 표본을 늘려도 고쳐지지 않으므로 표본의 추출 모집단을 보고서에 적음.
...
○ 도시 안의 유휴 부지를 찾아 현장 실사 대상을 좁히는 상황을 시뮬레이션함. 산출 단위가 개체(필지)이고 결정이 절단형이므로 7·8·9절의 계산이 모두 필요함  

○ 이 절의 표와 출력은 전부 7절 첫머리에서 실행한 `6-2-site-sourcing.py` 한 번의 결과이며, 로그 `lecture_practice/chapter6/results/6-2-site-sourcing.log`의 `[1]`부터 `[11]`까지 구간에 그대로 찍혀 있음  
...
○ 개발 요건은 예시로 설정한 사업 요건이며 법정 기준이 아님. 코드에는 `A_MIN`·`L_MIN`·`W_MIN` 세 상수로 적혀 있음  
○ 필지 면적은 최소 432㎡, 중앙값 1,155㎡, 최대 3,685㎡임(7절 첫머리의 준비 출력)  
○ 심어 둔 오차: 32필지 중 8필지는 두 겹 침식, 3필지(id 1·2·9)는 폭 2m 띠로 갈라 과분할, 산발 얼룩과 큰 오탐 덩어리 5개. 예측 마스크 총 면적은 55,313㎡로 정답의 **1.170배**  

**객체화와 채점**

○ 연결요소 949개 중 909개가 50㎡ 미만 얼룩 → 분석 대상 40개(7.2)  
○ 필지 검출률 30/32(0.938), 오탐 객체 5개, 과분할 3필지·병합 0객체(8.1)  
○ 면적 상대오차 중앙값 **−0.125**, 사분위수까지 전부 음수(8.2)  
...
□ 화면에 나온 값에서 읽을 것  
  ...
  ○ 면적 조건이 40건을 20건으로, 접도 조건이 다시 9건으로 자름. **가장 좁은 칸은 접도임** — 한 요건을 느슨하게 했을 때 늘어나는 후보 수를 따지려면 이 순서를 먼저 봄(9.4)  
  ○ 최종 후보 8건 중 우량 부지는 5필지뿐. **우량 부지 10필지 중 5필지(필지 19·20·21·22·23)가 목록에 오르지 못했고, 이들은 모두 침식으로 추정 면적이 임계 아래로 내려간 임계 근처 필지군임**  

**면적 편향 보정**

○ 실사 검토 풀 16건 중 무작위 12건을 실측하고 2건은 부지가 아닌 것으로 판명되어 10건으로 계수를 추정함. 스칼라 보정 k = 1.128, 둘레 모형 ĉ = 0.916(9.3)  
...
□ 화면에 나온 값에서 읽을 것  
  ○ 보정이 오탈락을 5건에서 2건으로 줄임. 되돌린 3건은 한 겹 침식으로 임계 바로 아래에 있던 필지이고, 남은 필지 19·21은 두 겹 침식이 걸린 필지임. **전역 계수 하나로는 필지마다 다른 침식 깊이를 되돌리지 못함**  
...
○ **후처리 필터는 정밀도만 올림.** 예측 안의 가짜를 깎는 장치라서 모델이 놓친 픽셀은 되살릴 수 없음. 실습에서 정밀도 0.572→1.000, 재현율 0.865 고정 (6.3)  
...
○ **침식 오차는 방향이 있어 평균해도 남음.** 실습의 면적 상대오차 중앙값 −0.125이고 부호가 전부 음수였으며, 작은 필지가 더 많이 깎임 (8.2)  
...
3. **실사 예산이 6건에서 12건으로 늘어난다면, 10절의 어느 표를 다시 보고 방문 순서를 정하겠는가? 그 표를 고른 이유를 적으라**
