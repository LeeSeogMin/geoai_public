...
**그림 6.2** U-Net의 구조. 왼쪽 아래로 내려가는 인코더는 `pool`을 지날 때마다 해상도가 절반이 되고, 오른쪽 위로 올라가는 디코더는 `up`으로 해상도를 되돌림. 아래로 갈수록 상자가 납작하고 두꺼워지는 것은 공간 해상도가 줄고 채널 수가 느는 상태임. 붉은 점선 두 개가 스킵 연결이며, 같은 해상도였던 인코더 특징을 건너편 디코더에 그대로 이어 붙임
