...
○ 같은 절차의 앞부분(집계·피처·접근성)은 민간 사업자의 입지 결정에도 그대로 쓰임. 공급이 수요를 못 따라가는 지역은 행정에게는 확충 대상이고 사업자에게는 진입 후보이며, 12절에서 서울 동물병원 자료로 이 겹침을 직접 계산함  
...
○ 이 다섯 줄 가운데 가장 눈에 띄는 것은 개입효과 행임. **방법을 바꾸는 것만으로 "효과가 미미한 사업"이 "효과가 확인된 사업"으로 판정이 뒤집힘.** 10절에서 이 계산을 처음부터 다시 따라감  
...
| --- | --- | --- | --- |
| 예측 | 다음 시점에 어느 단위가 위험한가 | 방문 인력 배치, 대비 물량 산정 | 7·8절 |
| 탐지 | 지금 어디에 발생이 비정상적으로 몰렸는가 | 역학조사 착수, 현장 확인 | 9절 |
| 인과 | 이미 집행한 개입이 결과를 바꾸었는가 | 사업 존속·확대·중단 판단 | 10절 |
...
□ 이 어긋남은 되먹임 구조를 만듦. 관측 기록이 많은 지역에 다음 회차 방문 인력이 더 배정되고, 그 배정이 다시 기록을 늘림  
  ...
  ○ 그래서 이 장의 처방은 예측이 낮게 나온 저정보 지역을 순위에서 빼는 것이 아니라 **별도의 현장확인 대상으로 돌리는 것**임(8.2의 조치표가 이 규칙을 표로 적음)  
...
□ 6절부터 12절까지는 설명과 실습을 나누지 않음 — 각 절 첫머리에 실행할 명령이 나오고 그 아래에 코드와 실제 실행 결과가 이어짐. 순서대로 따라가면 이 장의 모든 수치를 직접 만들게 됨  

□ 먼저 패키지를 설치함. GPU는 어느 실습에도 필요 없고 노트북에서 끝까지 돌아감  

```bash
pip install -r lecture_practice/requirements-student.txt
```

○ 12장이 쓰는 패키지만 따로 보려면 `lecture_practice/chapter12/code/requirements.txt`에 적혀 있으나, 설치는 위 공통 파일 하나로 끝남  
○ 공간 블록 교차검증과 999회 순열검정은 수 분이 걸리므로 여유를 두고 실행함  

□ 이 장에서 실행할 코드는 일곱 개이며 순서가 정해져 있음  

| 순서 | 실행할 파일 | 하는 일 | 절 |
| ---: | --- | --- | ---: |
| 1 | `12-0-simdata-prep.py` | 사각지대·감염병·DML·SaTScan 합성자료 생성 | 준비 |
| 2 | `12-0b-vetcare-data-prep.py` | 서울 동물병원·생활인구 실데이터 준비 | 준비 |
| 3 | `12-5-unmet-demand-siting.py` | 2SFCA 접근성·미충족 수요·순차 입지 선택 | 6.2, 12 |
| 4 | `12-1-welfare-blindspot-priority.py` | 사각지대 위험 예측·예측구간·신뢰등급 | 6.3, 7.1, 8, 11.1 |
| 5 | `12-2-infectious-spatiotemporal-risk.py` | 감염병 시공간 예측과 권역 집계 | 7.2, 11.1 |
| 6 | `12-4-satscan-cluster.py` | 공간 스캔 통계와 순열검정 | 9 |
| 7 | `12-3-dml-highdim-confounding.py` | 개입효과 DML 추정 | 10 |

○ 3번은 2번이 만든 파일을, 4~7번은 1번이 만든 파일을 입력으로 받으므로 **1·2번을 먼저 실행**해야 함  
○ 2번만 원자료를 따로 내려받아야 하고(`lecture_practice/chapter12/data/raw/README.md` 참고), 나머지는 인터넷 없이 돌아감  
○ 3~7번끼리는 서로 의존하지 않음. 위 순서는 본문이 결과를 인용하는 순서임  
○ 본문에 인용한 출력은 모두 `lecture_practice/chapter12/results/`의 실행 로그에서 그대로 가져온 것이며, 어느 파일에서 왔는지를 출력 블록 첫 줄에 적어 두었음  

□ 준비 단계 두 개를 지금 실행함. 여기서 만든 파일을 6절부터 12절까지 계속 씀  
...
```text
# 출처: lecture_practice/chapter12/results/12-0-simdata-prep.log
  복지 사각지대 읍면동 300개(20×15 격자) → welfare_units.parquet
  감염병 시공간 발생 16×16격자×40주(총 14345건) → infectious_incidence.parquet, infectious_pop.parquet
  DML 합성 표본 N=6000, 위기지표 p=20(+D,Y,m,ey_true) → dml_sample.parquet
  SaTScan 감염병 격자 300개(20×15), 참 군집 21개 → satscan_cases.parquet
```

```text
# 출처: lecture_practice/chapter12/results/12-0b-vetcare-data-prep.log
  서울 전체 점포 554,092개, 자치구 25개
  좌표 결측률(전 업종) 0.000%
  동물병원(소분류 '동물병원') 905개 | 자치구 25개 | 행정동 344개
  후보지: 점포 5개 이상인 칸 5,462개
  행정동 424개 | 기간 20260612~20260731 | 0~5시 평균
  서울 심야 생활인구 합계 9,823,054명
결합 점검: 대표점 행정동 중 생활인구와 매칭 97.2% (415/427)
```

○ 두 준비 스크립트가 찍는 값이 뒤 절의 분석 단위를 정함 — 읍면동 300개, 격자 256개, DML 표본 6,000개, 서울 행정동 415개, 후보지 5,462곳  

□ 네 스크립트(12-1~12-4)는 **합성자료**를 씀. 신호를 미리 설계해 넣은 자료라서 "방법이 설계한 신호를 회복하는가"를 확인하는 용도임  
  ○ **실제 복지 사각지대나 감염병의 크기를 알려 주는 자료가 아님.** 이 구분을 놓치면 합성자료의 수치를 실제 정책 결론으로 오해하게 됨  

○ 12-5(동물병원 입지)만 **국내 공개 실데이터**를 씀. 이 장에서 실제 지명·실제 수치가 등장하는 절은 6절과 12절뿐임  
...
□ 세 번째 코드를 실행함. 서울 동물병원 자료로 2SFCA 접근성부터 순차 입지 선택까지 한 번에 계산하며, 이 절과 12절의 모든 수치가 이 한 번의 실행에서 나옴  
...
```python
# 주석에 붙인 출력의 출처: lecture_practice/chapter12/results/12-5-unmet-demand-siting.log [1]
def two_sfca(D, d_dc, capacity, radius):
    inside = d_dc <= radius                        # 권역 안인가
    served = inside.T @ D                          # 병원별 권역 안 수요 합
    R = np.where(served > 0, capacity / np.maximum(served, 1e-9), 0.0)  # 1단계: 병원별 몫
    return inside @ R                              # 2단계: 동네별 접근성
# 출력: 추정 반려동물 수요 합계 1,000,186마리
#       병원 1곳당 현재 평균 담당 1,105마리 → 지점 용량 1,989마리/월 (×1.8)
#       접근성 A(마리당 월 진료 용량): 최소 0.685 / 중앙 1.717 / 최대 3.186
#       기준 접근성 A* = 수요가중 평균 = 1.8000
#       접근성 비 A/A*: 최소 0.381 / 중앙 0.954 / 최대 1.770
```

□ 화면에 나온 값에서 읽을 것  
  ○ 행정동별 접근성이 최소 0.685에서 최대 3.186까지 4.7배로 갈림 — 같은 서울 안에서도 마리당 확보 가능한 진료 용량이 이만큼 다름  
...
□ Aᵢ가 A\* 이상인 단위는 괄호 안이 음수가 되어 0으로 잘리고, Aᵢ가 A\*의 절반인 단위는 그 단위 수요의 절반이 미충족으로 계산됨  
  ○ A\*를 수요가중 평균 접근성으로 두면 A\* = (시설 수 × 지점 용량) ÷ 총수요가 유도되고, 지점 용량을 평균 담당 수요의 κ배로 두면 **A\* = κ**로 정리됨. 12절의 서울 자료에서 용량 배수 1.8을 넣었을 때 A\*가 실제로 1.8000으로 나온 것이 이 관계의 확인임  
...
□ 네 번째 코드를 실행함. 접근성 하나로 사각지대 위험이 얼마나 설명되는지 재는 것이 첫 출력이고, 7.1·8절·11.1절의 표도 모두 이 한 번의 실행에서 나옴  
...
```text
# 출처: lecture_practice/chapter12/results/12-1-welfare-blindspot-priority.log [1층 GIS]
분석 단위: 300개 읍면동(20×15 격자), 개인 식별 불가 집계

[1층 GIS] 의료 접근성 취약지 (이동시간 > 30분)
  접근성 취약 읍면동: 15개 (5.0%)
  취약지 평균 사각지대 위험: 13.56% vs 그 외: 10.88%
```

□ 화면에 나온 값에서 읽을 것  
  ○ 합성자료 300개 읍면동에서 이동시간 30분을 초과하는 접근성 취약 단위는 15개(5.0%)이고, 이 취약지의 평균 사각지대 위험은 13.56%로 그 외 지역의 10.88%보다 높음  
  ○ **차이는 2.68%p로 경향은 있지만 크지 않으며**, 나머지를 잡으려면 여러 신호를 결합하는 예측 모델이 필요함 — 7절이 그 모델을 세움  

## 7. 위험을 예측하고 공간으로 검증한다
...
□ 6.3에서 실행한 코드의 `[2층 AI]` 구간이 두 피처 구성을 같은 분할로 비교함  

```text
# 출처: lecture_practice/chapter12/results/12-1-welfare-blindspot-priority.log [2층 AI]
[2층 AI] 사각지대 위험 예측: 공간 피처 유무 + 검증 방식 비교
  공간 블록 CV | 비공간 피처     R² =  0.681
  공간 블록 CV | +공간 시차 피처 R² =  0.737  (개선 +0.056)
  랜덤    CV | +공간 시차 피처 R² =  0.737  (공간 누수 과대추정 +0.000)

  피처 중요도 상위 5(설명가능성):
    access_lag       0.527
    elderly          0.145
    single           0.100
    medical_access   0.069
    crisis_lag       0.062
```
...
□ 화면에 나온 값에서 읽을 것  
  ...
  ○ 피처 중요도 1위가 이웃 단위 접근성의 공간 시차(`access_lag` 0.527)로 자기 단위 접근성(`medical_access` 0.069)의 여덟 배 가까움 — 예측을 끌고 가는 값이 자기 단위가 아니라 이웃 단위에 있다는 뜻임  
...
□ 다섯 번째 코드를 실행함. 16×16 격자 40주 자료에서 다음 주 발생을 예측함  
...
```text
# 출처: lecture_practice/chapter12/results/12-2-infectious-spatiotemporal-risk.log
분석 단위: 256개 격자 × 40주, 보건소 권역 8개 (집계, 개인 식별 불가)
총 발생: 14345건, 피크 주 발생: 1279건

[2층 AI] 다음 주 발생 예측: 시공간 피처의 정보 가치
  시간 시차만(자기 과거)   R² =  0.694 | MAE = 1.483
  +공간 시차(이웃 과거)    R² =  0.776 | MAE = 1.294  (개선 R² +0.082, MAE +0.189)
...
□ 화면에 나온 값에서 읽을 것 — 피처 중요도에서 이웃 발생의 직전 주 값 0.440과 당주 값 0.420이 **자기 발생 0.074를 크게 앞섬**  
...
□ 6.3에서 실행한 코드의 `[불확실성]` 구간이 그 폭을 찍음  

```text
# 출처: lecture_practice/chapter12/results/12-1-welfare-blindspot-priority.log [불확실성]
[불확실성] 정규화 conformal 예측구간(90%, OOF 근사)과 신뢰등급
  90% 예측구간 경험적 포함률(OOF) = 0.903  (목표 0.90)
  구간 반폭(%p): 중앙값 1.53, 범위 [0.59, 4.19]
  신뢰등급 '낮음' 읍면동 = 100개
```

□ 화면에 나온 값에서 읽을 것  
  ○ 90% 예측구간의 경험적 포함률이 0.903으로 목표 0.90에 부합함 — 구간이 제 몫을 한다는 확인임  
  ○ 구간 반폭은 중앙값 1.53%p, 범위 [0.59, 4.19]%p로 단위마다 일곱 배까지 갈림. 반폭이 가장 넓은 쪽 100개 단위가 신뢰등급 '낮음'으로 분류됨  
...
### 8.2 저신뢰는 "위험 없음"이 아니라 "자료 부족"

○ 구간 반폭을 3분위로 나눠 좁은 쪽부터 높음·중간·낮음의 세 등급으로 표기하면 예측값과 조합해 규칙을 만들 수 있음  
○ 아래 표는 같은 실행 로그의 '불확실성을 반영한 방문상담 우선순위 상위 10'에서 상위 8행을 옮긴 것임  
...
□ 여섯 번째 코드를 실행함. 이 절의 수치는 전부 이 명령의 출력임  
...
```text
# 출처: lecture_practice/chapter12/results/12-4-satscan-cluster.log
분석 단위: 300개 격자(20×15), 총발생 C=10371건, 총인구 P=443692명
참 군집(심은 신호): 중심 (6.0, 4.0), 반경 2.6, 단위 21개, 상대위험 RR=3.0

[군집 탐지] 관측자료의 가장 가능성 높은 군집(MLC)
  중심 단위: (row 6, col 4) | 구성단위 21개
  관측 c=2245, 기대 e=892.6, 상대위험 관측 RR=2.52
  로그우도비 LLR=819.57

[유의성] 몬테카를로 순열 검정
  순열 최대 LLR: 중앙값 5.63, 95%분위 8.39
  관측 LLR 819.57 ≥ 순열 최대 LLR 인 경우 0/999
  p값 = 0.0010  (유의(군집 존재))

[검증] 참 군집 회복(construct validation)
  탐지 군집이 참 군집을 덮은 비율(recall) = 1.00 (21/21)
  탐지 군집 중 참 군집 비율(precision) = 1.00 (21/21)
  MLC 중심이 참 군집 내부인가: 예
```

□ 화면에 나온 값에서 읽을 것  
  ○ 위치도 크기도 지정하지 않았는데 탐지된 윈도우의 구성단위가 심어 둔 21개와 같고 중심도 참 군집 안에 들어감  
  ○ 관측 LLR 819.57은 순열 최대 LLR의 95%분위 8.39의 백 배 가까이 되고, 999회 가운데 관측값 이상이 한 번도 나오지 않아 p값이 이 설계의 최솟값 0.0010에 붙음  
  ○ 9.1부터 9.3까지는 이 출력이 어떤 절차에서 나왔는지를 단계별로 되짚음  

### 9.1 탐지는 예측이 아니다

□ 이 절은 7절과 같은 자료를 쓰지만 다른 질문에 답함  
...
□ LLR 값 하나만으로는 유의성을 판정할 수 없음. **모든 윈도우를 탐색해 최댓값을 골랐으므로 군집이 없는 자료에서도 어느 정도 큰 값이 나오기 때문임**  
  ○ 귀무가설은 "위험이 공간적으로 균일하다"임. 이 가설 아래에서 총발생 C를 각 단위의 인구 비율에 따라 무작위로 재분배한 자료를 999개 만들고, 각 자료에서 9.2의 다섯 단계를 그대로 수행해 최대 LLR을 구함  
...
□ 일곱 번째 코드를 실행함. 참 처치효과를 아는 합성자료에 세 추정량을 같은 표본으로 적용함  
...
```text
# 출처: lecture_practice/chapter12/results/12-3-dml-highdim-confounding.log
참 처치효과 θ(설계값) = -2.00  (개입이 위기점수를 낮춤)
표본 N = 6000,  위기지표 차원 p = 20
처치비율 = 0.625  (성향점수 범위 0.047~0.953, positivity 대략 성립)

[요약] 순진 OLS vs DML 편향 비교 (참 θ = -2.00)
추정량                        |       θ̂ |       편향 |      SE
...
□ 화면에 나온 값에서 읽을 것  
  ○ 같은 자료에 세 추정량을 댔는데 θ̂가 −0.579에서 −1.874까지 갈림. **자료가 아니라 방법이 답을 바꾼 것**이며, 10.1부터 10.4까지는 그 차이가 어디서 생기는지를 따라감  
  ○ 처치비율 0.625, 성향점수 범위 0.047~0.953은 두 군이 같은 구간에서 함께 관측된다는 확인임(10.4의 중첩 진단)  

### 10.1 통째로 비교하면 안 되는 이유
...
```python
# 주석에 붙인 출력의 출처: lecture_practice/chapter12/results/12-3-dml-highdim-confounding.log [2]
resid_y = Y - ghat                                       # 결과 잔차 Ỹ
resid_d = D - mhat                                       # 처치 잔차 D̃
denom = float(resid_d @ resid_d)
theta = float(resid_d @ resid_y) / denom                 # 잔차끼리 회귀
psi = resid_d * (resid_y - theta * resid_d)              # 강건 표준오차 계산용
# 출력: nuisance 추정정확도(참 함수 대비 RMSE/표준편차): 결과 ĝ=0.31, 처치 m̂=0.29
#       DML θ̂ = -1.874  (폴드 SE 0.036, 잔차부트스트랩 SE 0.035)
#       참값 대비 편향 +0.126  95% CI [-1.945, -1.803]
```
...
**그림 12.1** 왼쪽은 처치 잔차를 가로축, 결과 잔차를 세로축에 놓고 그린 산점도임. 빨간 직선의 기울기 −1.87이 DML 추정치이고 검은 점선이 참값 −2.00임 — **10.2의 2단계(두 잔차의 관계에서 처치효과를 뽑는다)가 실제로 하는 계산이 이 직선 하나임.** 오른쪽은 세 추정량의 편향 비교로, 순진 OLS만 참값에서 크게 떨어져 있음

### 10.4 DML도 그대로 믿지 않는다
...
## 11. 결과를 해석하고 처분과 분리한다

### 11.1 우선순위표와 권역 집계

□ 우선순위표를 읽는 순서는 **예측 위험 → 신뢰등급 → 근거 열**임. 예측 위험으로 후보를 좁히고, 신뢰등급으로 그 예측을 배분 근거로 쓸지 확인 대상으로 돌릴지 가르고, 근거 열로 왜 높게 나왔는지 확인함  
  ○ 8.2의 31번 단위는 12-1 실행 로그의 `[3층 정책]` 첫 행에 인구 7,509명, 65세 이상 비율 0.284, 1인 가구 비율 0.511, 위기신호 강도 21.508, 의료 접근성 28.367로 적혀 있음. 고령·1인 가구 비중이 높고 의료시설이 먼 조건이 겹친 지역임  
...
○ 아래 표는 12-2 실행 로그의 `[1층 GIS]`(누적 발생)와 `[3층 정책]`(다음 주 예측)을 권역별로 나란히 놓은 것이며, 상위 네 권역만 옮김  
...
## 12. 사례 전체: 미충족 수요와 동물병원 입지 선택

□ 앞 절들의 결정 주체는 행정이었음. 이 절은 같은 접근성 계산을 **민간 사업자**가 쓰는 경우를 다룸  
  ○ 달라지는 것은 목적함수와 뒷단임 — 미충족 해소가 아니라 기여이익 최대화가 목적이 되고, 자기잠식·용량 제약·원가 컷라인이 들어옴. 예측·탐지·인과(7~10절)는 수행하지 않음. **접근성은 계산으로 확정되고 남은 물음이 예측이 아니라 배분이기 때문임**  

□ 이 절의 표는 6.2에서 실행한 코드가 함께 만든 것임. 다시 실행해도 같은 결과가 나옴  
...
```text
# 출처: lecture_practice/chapter12/results/12-5-unmet-demand-siting.log [0]
[0] 자료와 가정
  기존 동물병원 905개 | 수요 단위(행정동) 415개 | 후보지 5,462개
  심야 생활인구 합계 9,617,177명 (상주 인구의 대리)
  [검증값] 마리당 월 병원비 37,000원 (농림축산식품부 2025년 반려동물 양육현황조사)
  [산출값] 인구 1인당 반려동물 ρ=0.104 (전국 0.152 × 서울/전국 양육가구비 0.682)
  [가정] β=2.0 | 2SFCA 반경 3,000m | 용량 배수 1.8 | 기여이익률 60% | 월 고정비 16백만원
```

○ 출력 첫 줄부터 자료와 가정을 갈라 적음 — 12.2가 이 구분을 표로 다시 정리함  
...
○ 기준 접근성 A\*는 6.3의 정의대로 수요가중 평균으로 두었고 실행값은 1.8000으로 용량 배수와 일치함. 접근성 비율 Aᵢ/A\*는 최소 0.381, 중앙값 0.954, 최대 1.770  
...
□ 같은 실행의 `[3]` 구간이 열 지점을 순서대로 찍음  

```text
# 출처: lecture_practice/chapter12/results/12-5-unmet-demand-siting.log [3]
  순서 | 후보ID | 자치구 | 단독배분 | 배분 | 실현 | 용량소진 | 한계순증 | 한계기여이익(백만) | 컷라인 | 동점
     1 |   1515 | 은평구    |    3,228 |  3,208 |  1,989 |   100.0% |    1,989 |               44.2 | 통과     |    72
     2 |    687 | 금천구    |    2,738 |  2,700 |  1,989 |   100.0% |    1,989 |               44.2 | 통과     |    69
     3 |   1699 | 관악구    |    2,676 |  2,611 |  1,989 |   100.0% |    1,989 |               44.2 | 통과     |    60
     4 |   1465 | 동작구    |    2,566 |  2,505 |  1,989 |   100.0% |    1,989 |               44.2 | 통과     |    48
     5 |   5447 | 강동구    |    2,492 |  2,489 |  1,989 |   100.0% |    1,989 |               44.2 | 통과     |    41
     6 |   4960 | 강남구    |    2,452 |  2,448 |  1,989 |   100.0% |    1,989 |               44.2 | 통과     |    38
     7 |   1539 | 영등포구   |    2,334 |  2,293 |  1,989 |   100.0% |    1,989 |               44.2 | 통과     |    35
     8 |   1586 | 관악구    |    2,325 |  2,241 |  1,989 |   100.0% |    1,989 |               44.2 | 통과     |    31
     9 |    251 | 구로구    |    2,182 |  2,162 |  1,989 |   100.0% |    1,989 |               44.2 | 통과     |    24
    10 |   1567 | 은평구    |    2,176 |  2,146 |  1,989 |   100.0% |    1,989 |               44.2 | 통과     |    19
  총 배분 24,801마리 / 실현 19,893마리
  자기잠식 손실률 1.5% (단독 합 25,168 → 동시 24,801)
```

□ 화면에 나온 값에서 읽을 것 — 열 지점이 모두 용량 상한 1,989마리에 걸림. 단독 배분(각 자리를 혼자 있다고 가정한 값) 합은 25,168마리였는데 동시 배분은 24,801마리로 줄어듦. **이 차이가 자기잠식이며 손실률은 1.5%**  
...
□ 같은 실행의 `[5]` 구간이 격차를 세 항으로 갈라 놓음  

```text
# 출처: lecture_practice/chapter12/results/12-5-unmet-demand-siting.log [5]
  → 탐욕 − N1: 실현 +6,494마리 (+48.5%), 자기잠식 -16.8%p, 영업이익 +144.2백만원/월
  격차 분해 (탐욕 − 미충족수요 순위, 마리):
    ① 자리 자체의 포획력(단독 합 차이)     +8,770
    ② 자기잠식 손실 차이                  +2,632
    ③ 용량 손실 차이                      -4,908
    합계(실현 차이)                       +6,494
```

□ 화면에 나온 값에서 읽을 것 — 탐욕 선택은 미충족 수요 순위 대비 실현 포획이 6,494마리(+48.5%) 많고 월 영업이익도 1억 4,410만원 많음. **선택된 격자는 두 순위표 어느 쪽과도 하나도 겹치지 않음**(Jaccard 0.00)  
...
○ 다만 자치구 수준에서는 크게 겹침. 탐욕 선택이 고른 곳은 강남·강동·관악·구로·금천·동작·영등포·은평구이고, 12.3의 상위 행정동이 속한 자치구와 상당 부분 같음. **진단은 어느 권역을 볼지 좁히고, 결정은 그 권역 안에서 배분·잠식·용량·원가가 정함**  

### 12.7 가정이 흔들려도 결론이 버티는가

□ 같은 실행의 `[8]` 구간이 가정값을 하나씩 흔들어 본 결과임  

```text
# 출처: lecture_practice/chapter12/results/12-5-unmet-demand-siting.log [8]
  시나리오 | 미충족합계 | G실현포획 | 개설가능 | 기준 대비 Jaccard | G−N1 실현차
  기준                   108,080    19,893        10              1.00      +6,494
  β 1.5                108,080    16,399        10              0.43      +3,257
  β 2.5                108,080    19,893        10              0.67      +7,026
  사육률 ρ −30%            75,656    13,925        10              1.00      +4,546
  사육률 ρ +30%           140,504    25,861        10              1.00      +8,442
  용량 배수 1.4            108,080    15,472        10              1.00      +2,300
  용량 배수 1.0            108,080    11,052        10              1.00          +0
```
...
□ 다만 결론의 부호는 바뀌지 않음. 탐욕 선택이 순위표를 앞서는 정도는 모든 시나리오에서 0 이상이며 부호가 뒤집힌 적이 없음. **순위표를 그대로 결정으로 쓰면 손해라는 결론은 가정에 견고하고, 손해의 크기만 가정에 달려 있음**  
  ○ 확인 못 한 사육률 ρ는 선택 자체를 바꾸지 않음(ρ를 ±30% 바꿔도 Jaccard 1.00). 6.3에서 A\*를 수요가중 평균으로 정의한 덕분에 접근성 비율이 ρ에 불변이기 때문임  
...
○ **신호 하나로는 못 씀.** 기저율 2%에서 민감도 60% 신호의 양성예측도는 0.109이고, 두 번째 신호를 겹치면 0.434로 오름. 다만 두 신호가 같은 지불 능력에서 나오면 이 이득이 줄어듦 (5.2)  
○ **접근성은 계산으로 확정되고 사각지대·감염병은 예측이 필요함.** 이동시간을 학습 모델로 근사하면 확정된 값을 부정확한 예측으로 바꾸는 셈임 (6.1)  
○ **2SFCA가 재는 값은 "수요 한 단위가 확보할 수 있는 용량"**이고, 미충족 수요는 총량 부족이 아니라 분포의 불균형만 잼 (6.2, 6.3)  
○ **저신뢰는 "위험 없음"이 아니라 "자료 부족"임.** 저신뢰 단위를 순위에서 빼면 소외된 지역이 자료 부족을 이유로 다시 배제되므로, 현장확인 우선 대상으로 돌림 (8.2)  
○ **탐지는 예측이 아님.** 스캔 통계는 볼 위치와 크기를 사전에 정하지 않고 현재의 이상 집중을 찾으며, 그 결과는 확진이 아니라 조사 착수의 근거임 (9.1, 9.3)  
○ **순열검정은 최댓값의 분포를 씀.** 탐색으로 인한 부풀림이 귀무분포에 이미 들어가므로 다중비교 보정이 따로 필요 없음 (9.3)  
○ **방법 선택이 사업 존폐 판정을 바꿈.** 같은 자료에서 순진 비교는 −0.579, DML은 −1.874를 산출했고 참값은 −2.0이었음 (10.3)  
○ **예측과 처분 사이에 사람의 검토를 둠.** 오탐이 불이익 처분으로 직결되면 예측 체계가 없었으면 생기지 않았을 피해가 새로 생기고, 그 피해는 이의제기 자원이 가장 적은 집단에 감 (11.2)  
○ **진단 지표의 상위가 곧 좋은 입지는 아님.** 탐욕 순차 선택이 미충족 수요 순위표보다 실현 포획 48.5% 많았고, 격차의 주된 출처는 자기잠식이 아니라 자리 자체의 포획력이었음 (12.6)  
...
2. 그 숫자가 왜 그렇게 나왔는가 — 12.6의 격차 분해(자리 포획력·자기잠식·용량)를 근거로 설명하라
