...
- 이렇게 받은 조각이 실제로 무엇인지 눈으로 확인함

![부분 읽기로 받아 온 강남 소구역](../lecture_practice/chapter3/data/gangnam_rgb_stretched.png)

**그림 3.3** 268.5MB 파일에서 0.048%만 잘라 받은 강남 소구역(216행×270열). 가시광선 세 밴드를 RGB로 합성하고 명암을 늘려 표시한 것이며, 격자 모양 도로망과 블록 단위 건물이 구분됨. **전체 장면을 내려받지 않고도 이 판독이 가능하다는 것이 부분 읽기의 요점임**
...
- 행정동마다 8개 업종군을 합쳐 포착률을 계산함. 분모가 30개 미만인 13개 동은 관측 하나에 비율이 크게 튀므로 제외하고 412개 동을 봄
  - 분포: 최소 0.000 / 중앙값 0.190 / 최대 1.069
  - 가장 높은 동: 가회동(1.07), 교남동(0.86), 삼청동(0.70) — 도심과 관광지
  - 가장 낮은 동: 상계8동(0.00), 문정2동(0.03), 시흥2동(0.03) — 외곽 주거지
- 시청까지의 거리로 회귀하면 거리 효과만 뚜렷하게 확인됨(계수 −0.3089, p = 2.27×10⁻¹³) — **빠짐은 무작위가 아니라 도심에서 멀수록 커짐**

![행정동별 OSM 포착률과 시청 거리](../lecture_practice/chapter3/results/3-4-coverage-map.png)

**그림 3.4** 왼쪽은 행정동 중심점에 찍은 OSM 포착률이며 밝을수록 높고 원 크기는 상가정보 점포 수 — 시청과 종로·중구 일대가 밝고 외곽으로 갈수록 어두워짐. 오른쪽은 서울시청까지 거리에 따른 포착률과 회귀 적합값으로, 거리가 늘수록 적합값이 0.30에서 0.12 부근까지 내려감. 같은 거리에서도 점이 위아래로 크게 흩어지는데, 거리만으로 설명되지 않는 동별 차이가 그만큼 크다는 뜻임
...
- 여기에 "애초에 서점이 0개인 동이 얼마나 되는가"(사전확률, 상가정보 기준 P(N=0) = 0.118)를 붙여 베이즈 규칙으로 뒤집으면, **0개로 보이는 동이 정말 0개일 확률**(사후확률)이 나옴

```
P(진짜 0개 | 0개로 보임) = P(N=0) ÷ Σₙ P(N=n)·(1−p)ⁿ
```

- 서점의 사후확률은 **0.150** — **지도에 서점이 0개로 보이는 동 일곱 곳 가운데 여섯 곳은, 서점이 있는데 등록되지 않았을 뿐임**
...
- 일곱 업종군 모두에서 판정 불가가 나오지만, 막히는 이유는 둘로 갈림
- **첫째 갈래 — 포착률이 모자라는 경우.** 서점·안경·화장품·약국이 여기에 해당함. 서점은 필요 p\* 0.85에 실측 0.074로 열 배 이상 모자람
- **둘째 갈래 — 질문이 성립하지 않는 경우.** 카페·편의점·미용·뷰티는 서울 425개 동 전부에 점포가 실재해 "0개인 동"이 하나도 없음. 사전확률 P(N=0)이 0이므로 사후확률도 0이 되고, 포착률을 아무리 올려도 컷라인에 닿지 않아 필요 p\* 열이 —로 남음. 어디에나 깔린 업종에서는 "0개"라는 관측이 애초에 나오지 않으므로, 판정 대상을 0개 여부가 아니라 개수 수준으로 바꿔야 함
...
- 마지막으로 412개 동 각각에 사용 등급을 붙임. 판정 축은 둘 — ① 사후확률(그 동의 0개 관측을 믿어도 되는가) ② 그 동의 포착률이 7절 회귀식의 예측에서 얼마나 벗어났는가(|z|)
- 두 축을 대기 전에 걸러야 할 것이 하나 있음 — 포착률이 1을 넘는 동은 확률로 해석할 수 없으므로 사후확률을 계산하지 않고 따로 뺌. 1 초과를 0.999로 잘라 넣으면 대응표가 어긋나 생긴 값이 오류 없이 계산을 통과해 가장 높은 등급을 받음
...
- **그대로 사용**은 412개 중 교남동 1곳뿐임(포착률 0.857) — 지도만으로 공백을 판정해도 되는 동은 서울에서도 예외에 가까움
- **대응표 점검**은 포착률이 1을 넘어 확률로 해석할 수 없는 동임 — 가회동은 분모 101개 중 카페가 58개인데 OSM 카페는 75개임. 관광지에서 상가정보의 다른 소분류로 등록된 점포가 OSM에서는 `amenity=cafe` 하나로 모이면 분자만 부풂. 등급을 붙이지 않고 해당 업종군의 대응 규칙을 다시 잡은 뒤 재집계함
...
- 이 결과가 2절 질문의 최종 답임 — **서울 412개 동 가운데 지도 자료만으로 "비어 있다"를 판정할 수 있는 동은 1곳뿐임.** 248개는 보정을 거쳐야 하고 162개는 다른 자료가 필요하며, 1개는 대응표부터 다시 잡아야 함
...
- 실행 후 본문과 대조할 기준값
  ...
  - `3-4`: 8개 업종군 합계 포착률 0.227, 미용·뷰티 1.249(좁은 대응), 서점 사후확률 0.150
...
- **빠짐은 무작위가 아님.** 도심에서 멀수록 포착률이 체계적으로 낮아지며(계수 −0.3089), 대응표를 바꾸면 이 결론 자체가 뒤집힐 수 있으므로 대응표를 결과와 함께 공개함. 절대 수준보다 동 사이의 순서(순위상관 0.994)가 안정적임 (7절)
- **"0개로 보임"과 "정말 0개"는 확률로 연결됨.** 서점 포착률 0.074에서는 0개로 보이는 동 일곱 곳 중 여섯 곳에 실제로 서점이 있음(사후확률 0.150). 판단 기준선인 컷라인은 자료가 아니라 오판 비용의 비가 정함 — 출점 결정 0.80, 공공 배치 0.20 (8절)
- **서울 412개 동 가운데 지도만으로 공백을 판정할 수 있는 동은 1곳뿐임.** 248개는 보정(관측 ÷ 예측 포착률) 후 사용, 162개는 다른 자료나 현장 확인이 필요함 (8.3절)
