...
□ 침수 실습 코드의 실제 실행 결과부터 봄 — 이 출력을 직접 만드는 명령은 4절 끝에서 자료를 준비한 뒤 5.3절에 나옴  

```text
# 출처: lecture_practice/chapter11/results/11-1-flood-risk-priority.log [1층 GIS]
  저지대 격자: 75개 | 평균 침수심 25.1cm vs 그 외 10.3cm
```
...
○ 같은 실행의 `[2층 AI]` 구간은 같은 자료를 세 가지 조건으로 검증한 결과를 찍음  
...
○ 같은 구간의 `피처 중요도` 출력이 어떤 변수가 예측에 크게 기여했는지 보여 줌  
...
○ 이 거리를 분석 시작 시점에 한 줄로 적어 두면, 나중에 모델을 고르는 기준과 감사 항목이 달라짐. 10·11절이 그 차이를 실측으로 보임  
...
```python
# 출처: lecture_practice/chapter11/code/11-1-flood-risk-priority.py
def spatial_lag(df, col):
    return _neighbor_mean(df["row"].values, df["col"].values, df[col].values)
```

○ 이 함수는 5.3절에서 실행할 `11-1-flood-risk-priority.py`의 일부임 — 격자의 `row`·`col`로 상하좌우 네 칸을 찾아 그 값의 평균을 냄  
...
○ 11장이 쓰는 패키지만 따로 보려면 `lecture_practice/chapter11/code/requirements.txt`에 적혀 있으나, 설치는 위 공통 파일 하나로 끝남  

□ 다음으로 자료를 준비함. 세 실습이 모두 이 한 번의 실행이 만든 parquet 파일을 입력으로 받음  
...
```text
# 출처: lecture_practice/chapter11/results/11-0-simdata-prep.log
  침수 격자 300개(20×15) → flood_terrain.parquet
  치안 필드 256개(16×16) → policing_field.parquet
  보험 건물 500개 → insurance_buildings.parquet
...
| 순서 | 실행할 파일 | 하는 일 | 절 |
| ---: | --- | --- | ---: |
| 1 | `11-0-simdata-prep.py` | 침수·치안·보험 세 자료 준비 | 4 |
| 2 | `11-1-flood-risk-priority.py` | 침수위험 예측·불확실성·우선순위 | 2, 5.3~8 |
| 3 | `11-2-predictive-policing-bias.py` | 예측적 치안의 되먹임 시뮬레이션 | 10 |
| 4 | `11-3-insurance-risk-grading.py` | 보험 위험등급과 보험료 차등 | 11 |

○ 2·3·4번은 모두 1번이 만든 parquet 파일을 읽으므로 **1번을 먼저 실행**해야 하고, 2·3·4번 사이에는 순서 의존이 없음  
○ 인터넷도 GPU도 필요 없음. 세 실습 모두 시드를 고정한 자료를 쓰므로 같은 명령을 다시 실행해도 같은 값이 나옴  
○ **세 자료는 모두 시뮬레이션 자료이며 실제 지역이나 실제 시장의 값이 아님.** 본문에서 다루는 숫자는 이 시뮬레이션 설계값이 만든 결과일 뿐, 실제 침수 확률이나 실제 보험 시장 통계로 읽으면 안 됨  
○ 본문에 인용한 출력은 모두 `lecture_practice/chapter11/results/`의 실행 로그에서 그대로 가져온 것이며, 어느 파일의 어느 구간에서 왔는지를 출력 블록 첫 줄에 적어 두었음  
...
□ 첫 실습 코드를 실행함. 5절부터 8절까지 인용하는 출력이 모두 이 한 번의 실행에서 나옴  

```bash
python lecture_practice/chapter11/code/11-1-flood-risk-priority.py
```

○ 출력은 네 덩어리로 나뉨 — `[1층 GIS]` 표고만 쓴 기준선(2절·8.3), `[2층 AI]` 검증 설계 비교와 피처 중요도(2절·6.1), `[3층 정책]` 격자·행정구역 우선순위표(7.3·8.1), `[불확실성]`과 `[적용가능영역 AoA]` 예측구간과 외삽 판정(7.1~7.2)  

○ 아래 설정이 그 코드에 실제로 들어 있는 값임  
...
□ 5.3에서 실행한 코드의 `[2층 AI]` 구간이 이 분할과 랜덤 분할의 차이를 그대로 찍음  

```text
# 출처: lecture_practice/chapter11/results/11-1-flood-risk-priority.log [2층 AI]
  공간 블록 CV | 비공간 피처     R² =  0.781
  공간 블록 CV | +공간 시차 피처 R² =  0.816  (개선 +0.034)
  랜덤    CV | +공간 시차 피처 R² =  0.874  (공간 누수 과대추정 +0.058)
```

□ 화면에 나온 값에서 읽을 것  
  ○ 피처도 모델 설정도 같고 분할 방식만 다른데 R²가 0.816에서 0.874로 오름 — 이 +0.058이 인접 누수의 크기임  
  ○ 로그가 그 줄에 붙인 "공간 누수 과대추정"이 이 뜻임. 모델이 좋아진 것이 아니라 검증이 쉬워졌음  
...
□ 5.3에서 실행한 코드의 `[불확실성]` 구간이 이 절차의 결과를 찍음  

```text
# 출처: lecture_practice/chapter11/results/11-1-flood-risk-priority.log [불확실성]
  90% 예측구간 경험적 포함률(OOF) = 0.903  (목표 0.90)
  구간 반폭(cm): 중앙값 3.1, 범위 [1.3, 18.7]
```
...
□ 같은 실행의 `[적용가능영역 AoA]` 구간 출력임  

```text
# 출처: lecture_practice/chapter11/results/11-1-flood-risk-priority.log [적용가능영역 AoA]
  DI 임계값(Q3+1.5·IQR) = 0.24 | 적용가능영역 밖 격자 = 9개
```

○ 격자 300개 가운데 9개가 임계값 0.24를 넘어 적용가능영역 밖으로 표시됨  
○ **AoA 판정은 예측값을 지우는 데 쓰지 않고 별도 열로 남김** — 적용범위 밖 격자에도 예측값은 나오지만, 그 값을 근거로 자원을 확정 배분하지 않고 추가 관측 대상으로 분류함  

### 7.3 불확실성을 반영한 우선순위표

□ 같은 실행의 마지막 구간(`불확실성·AoA를 반영한 행정구역 우선순위`)이 앞의 세 값을 행정구역 단위로 합쳐 찍음 — 출처 `lecture_practice/chapter11/results/11-1-flood-risk-priority.log`  
...
□ 화면에 나온 값에서 읽을 것 — **예측값이 가장 높은 admin 2가 순위 1위인데 신뢰등급은 '낮음'임**  
  ○ 예측 침수심이 높다고 곧바로 우선 집행하지 않고, 신뢰가 낮은 곳은 확정 배분 전에 추가 관측이나 현장확인으로 보완함  
  ○ 이 판단 기준을 9절의 조치 매트릭스에서 표로 정리함  
...
○ 아래는 같은 실행의 `[3층 정책]` 구간이 찍은 상위 10개 가운데 상위 5개임 — 출처 `lecture_practice/chapter11/results/11-1-flood-risk-priority.log`  
...
**둘째, 순위의 안정성을 봄.** 8.1에서 확인한 상위 5개 격자의 예측값(34.52~31.30cm)처럼 촘촘하면 1위와 5위의 차이는 모델의 오차 범위 안에 있을 수 있음. 이때는 순위를 그대로 배분 순서로 쓰지 않고 상위 그룹으로 묶어 처리함
...
○ **7.3에서 본 admin 2가 여기 해당함** — 예측 위험은 1순위이지만 신뢰등급이 '낮음'이므로 둘째 줄에 해당하는 조치(순위 유지 + 현장확인 후 확정)로 다뤄야 함  
...
□ **마지막 행이 이 장의 나머지를 정함** — 예측 결과로 자원을 배분하고 그 배분이 다음 시점의 관측자료를 바꾸면, 모형이 자기 결정의 흔적을 학습함  
  ...
  ○ **치안과 보험에서는 이 되먹임이 분석의 중심 문제가 됨** — 10·11절에서 두 사례를 봄  

## 10. 예측이 다음 관측을 바꿀 때 ① 예측적 치안
...
□ 두 번째 실습 코드를 실행함. 이 순환을 16×16 격자 256개에서 8라운드 돌림  
...
```text
# 출처: lecture_practice/chapter11/results/11-2-predictive-policing-bias.log
분석 단위: 256개 격자(16×16), 라운드 8회, 개인 식별 불가
진짜 범죄율 평균 8.5건 (관측 불가) | group A(과잉단속) 평균 진짜율 8.4 vs group B 8.5 → 진짜 위험은 거의 동일
```

○ `results/11-2-predictive-policing-bias.log`는 요약 줄을 고치기 전에 저장한 실행 기록임 — 지금 코드는 상관 요약을 라운드 1 기준으로 출력하므로 로그 맨 아래 요약 줄의 표현이 다름. 표의 숫자는 같음  

□ 시뮬레이션 설정 — **두 집단의 실제 범죄율은 평균 8.4건(A)과 8.5건(B)으로 거의 같게 두고, 초기 기록만 집단 A 지역에서 크게 만듦**  
...
○ 아래는 로그의 `[입증]` 구간 표를 옮긴 것임 — 출처 `lecture_practice/chapter11/results/11-2-predictive-policing-bias.log`  
...
□ 화면에 나온 값에서 읽을 것 — **네 열이 서로 다른 방향으로 움직임**  
  ○ 기록 기준 R²는 0.823에서 0.999로 올라감 — **성능표만 보면 모형이 계속 좋아진 것으로 읽힘**  
  ○ 그러나 순찰과 실제 위험의 상관은 **되먹임 구간인 라운드 1의 0.537에서 라운드 7의 0.429로 계속 내려감** — 라운드를 거듭할수록 순찰 배분이 실제 위험에서 멀어짐  
  ○ 라운드 0의 0.397이 더 낮은 까닭은 이때가 되먹임 이전 상태이기 때문임 — 초기 기록은 실제 위험이 아니라 집단 A에 1.8배, 집단 B에 0.8배를 곱한 과거 단속 편향으로 만들어져서 예측이 집단 경계만 따라감. 순찰이 기록을 바꾸기 시작한 라운드 1부터를 되먹임 구간으로 읽음  
...
○ 성능지표 하나로 모형을 판정하면 안 되는 이유가 이 표에 그대로 담겨 있음. 9.2의 감사표에서 배분 결과와 집단 형평을 별도 범주로 둔 근거이기도 함  
○ 되먹임을 끊는 완화 장치  
...
□ 실습 데이터 — 4절 끝에서 실행한 `11-0-simdata-prep.py`가 만든 것임. 300개 격자에 건물 500채, 가입률 55.4%, 저소득 비율 28.4%  
  ○ 저소득 가입률 42.3%, 고소득 가입률 60.6%. 소액 피해 미신고(가입자 중) 21건(저소득 가입자의 35.0%)  
  ○ 이 값은 교육용 시뮬레이션의 설계값이며 실제 보험시장의 추정치가 아님  

### 11.2 성능 차이의 원인을 대조군으로 분리한다

□ 세 번째 실습 코드를 실행함. 11.2부터 11.4까지의 모든 표와 숫자가 이 한 번의 실행에서 나옴  
...
○ 세 조건의 성능을 같은 채점 대상(전체 500채의 실제 피해확률)으로 비교하면 — 아래는 로그의 `표 11.6` 구간과 그 아래 대조군 줄을 옮긴 것임(출처 `lecture_practice/chapter11/results/11-3-insurance-risk-grading.log`)  
...
○ 같은 로그 첫머리에 찍히는 `관측 모델(가입자 277채, 대상=청구 기록) CV R²: 0.520`은 채점 대상이 다른 값임 — 가입자 277채의 청구 기록을 맞힌 점수이고, 위 표의 0.669는 전체 500채의 실제 피해확률을 맞힌 점수임. **두 값을 같은 지표로 나란히 읽지 않음**  
...
### 11.3 소득변수를 넣지 않아도 생기는 격차

□ 예측값을 4등급으로 나누고 등급별 보험료 배수를 정함. **등급표에는 위험 지표뿐 아니라 집단 구성 비율을 함께 실음**  

○ 아래는 로그의 `표 11.5` 구간임 — 출처 `lecture_practice/chapter11/results/11-3-insurance-risk-grading.log`  
...
□ 같은 실행의 `불확실성`과 `보험의 역설` 구간이 이 두 값을 찍음  

```text
# 출처: lecture_practice/chapter11/results/11-3-insurance-risk-grading.log
불확실성(정규화 conformal):
  q̂=1.601, 가입자 기준 포함률: 0.906 (목표 0.90)
  CI 반폭 중앙값: 0.116, 저신뢰 기준(상위 25%): 0.141
  구간폭-예측값 Spearman: 0.236 (1.000이면 '저신뢰=고위험'이 항등식)
  저신뢰 건물 수: 125개 (보험료 ×1.15 적용)
  저신뢰 건물의 최고위험 등급 비율: 43.2%

보험의 역설:
  진짜 고위험 건물 중 미가입: 70/125 (56.0%)
```

□ 화면에 나온 값에서 읽을 것  
  ○ 가입자 기준 포함률 0.906으로 목표 0.90을 지킴. 반폭 상위 25%인 125채에 보험료 ×1.15를 곱함  
  ○ 저신뢰 건물 가운데 최고위험 등급은 43.2%뿐임 — 나머지는 위험이 높아서가 아니라 예측이 흔들려서 할증을 받음. 순위상관 0.236이 뜻하는 바가 이 숫자로 드러남  
...
□ 마지막으로 확인할 되먹임 — **진짜 고위험 건물 125채 가운데 70채, 즉 56.0%를 미가입으로 설정함**  
  ...
  ○ 10절의 순찰–기록 순환과 같은 구조임 — 위험 기반 가격이 특정 저소득 지역에 집중되면 보험 접근성이 낮아지고, 접근성 저하가 다시 자료 공백을 키움  
...
○ **무작위 분할은 새 지역 일반화가 아니라 인접값 보간 능력을 잼.** 실습에서 랜덤 CV 0.874와 블록 CV 0.816의 차이 +0.058이 그 누수의 크기임 (2절, 6.1)  
○ **성능지표는 오차의 크기만 알려주고 오차가 어디에 몰렸는지는 알려주지 않음.** 잔차 Moran I를 함께 봄 (6.3)  
○ **예측구간은 학습 범위 안의 오차를 다루고, 적용가능영역은 그 범위 밖을 표시함.** AoA 판정은 예측값을 지우지 않고 별도 열로 남김 (7.1, 7.2)  
○ **예측 1순위가 저신뢰일 수 있음.** 실습의 admin 2가 그 경우이며, 조치는 즉시 집행이 아니라 현장확인 후 확정임 (7.3, 9.1)  
○ **성능이 오르는 동안 배분이 나빠질 수 있음.** 치안 시뮬레이션에서 R²는 라운드 0→7에 0.823→0.999로 오르는데 순찰–실제 위험 상관은 되먹임 구간인 라운드 1→7에 0.537→0.429로 떨어졌고, 실제 범죄율이 8.4 대 8.5로 거의 같은 두 집단에서 차별적 영향이 2.0배가 됨 (10절)  
○ **성능 저하의 원인은 대조군으로 분리함.** 보험에서 가입 선택의 몫은 0.011이고 표본 감소·라벨 왜곡의 몫이 0.231이며, 고위험 건물의 56%가 미가입이라 자료 공백이 스스로를 키움 (11.2, 11.4)  
...
○ **3번이 이 과제의 핵심**. 1·2번은 로그를 보면 답이 나오지만 3번은 9.1의 조치 매트릭스를 근거로 스스로 답해야 함  
