...
```mermaid
flowchart LR
    subgraph AI_계보["AI 계보"]
        AI["인공지능"] --> ML["머신러닝\nRF, XGBoost"]
        ML --> DL["딥러닝\nCNN, U-Net, LLM"]
    end

    subgraph 공간_계보["공간 분석 계보"]
        GA["지리공간 분석"] --> GIS["GIS\n좌표계, 버퍼, 오버레이"]
        GA --> RS["원격탐사\n위성영상 취득·전처리"]
        GA --> SS["공간 통계\nMoran's I, 크리깅"]
        GA --> SDS["공간 데이터 과학"]
    ...
    ML -->|"Part I Ch4"| GEOAI["GeoAI\n공간 구조 + AI"]
    DL -->|"Part II Ch5-7"| GEOAI
    SDS --> GEOAI
    GEOAI -->|"Part III·IV"| DEC["정책·비즈니스\n의사결정"]
```
...
```mermaid
flowchart TB
    subgraph Pillar1["지리공간 빅데이터"]
        D1["위성영상\nLandsat, Sentinel"]
        D2["벡터/센서 데이터"]
        D3["포인트 클라우드"]
    end

    subgraph Pillar2["머신러닝/딥러닝"]
        M1["CNN, Transformer"]
        M2["U-Net, SAM"]
        M3["LSTM, GNN"]
    end

    subgraph Pillar3["고성능 컴퓨팅"]
        C1["GPU 가속"]
        C2["클라우드 플랫폼\nGEE, AWS"]
    end
...
```
   ...
   (가장 넓음)          ┌──────┴──────┐               │
        │              │             │               │
       GIS         공간 통계      원격탐사            │
    GIScience      Moran's I     위성·SAR            │
                    크리깅                            │
                                                     │
  ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─┤
                                                     │
                              방법 축 ───────────  GeoAI
                   "어떤 기법으로 다루는가"      공간 구조 + AI
                              │                 ML / DL / FM
                              │
  ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ┤
                              │
...
```mermaid
flowchart TB
    GA["지리공간 분석 — 공간을 다루는 모든 분석"]
    GA --> SDS["공간 데이터 과학의 토대\n1층 · Part I · Ch1~4"]
    GA --> GEOAI["GeoAI — 공간 명시적 딥러닝\n2층 · Part II · Ch5~7"]
    GEOAI --> PUB["공공정책 의사결정\n3층 · Part III · Ch8~13"]
    GEOAI --> BIZ["비즈니스 의사결정\n3층 · Part IV · Ch14~15"]
    LI(["위치 인텔리전스 — 비즈니스 용도로 자른 단면"]) -.-> BIZ
    LI -.-> SDS
```
