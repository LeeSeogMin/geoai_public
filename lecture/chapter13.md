...
□ 절차의 순서는 **진단 → 유형화 → 예측 → 배분 → 평가 → 공표**임  
  ...
  ○ 이 장에는 다른 정책 장에 없는 논점이 하나 더 있음. **"소멸위험지역"이라는 이름을 붙여 발표하는 행위 자체가 그 지역을 더 쇠퇴시킬 수 있음**(10절)  
...
○ **세 한계 모두 지수를 버릴 이유가 아니라 다른 지표를 함께 둘 이유임** → 5절에서 청년 순이동, 생활서비스 접근성, 사업체 밀도와 결합함  
...
□ 이 자료에서 실제로 확인된 값 — 7절에서 실행할 `13-2-nightlight-economic-decline.py`의 출력이며, 출처를 적은 블록은 7절 첫머리에 있음  
...
□ 5절부터 11절까지는 설명과 실습을 나누지 않음 — 계산이 들어 있는 절의 첫머리에 실행할 명령이 나오고 그 아래에 코드와 실제 실행 결과가 이어짐. 순서대로 따라가면 이 장의 모든 수치를 직접 만들게 됨  
  ○ 명령이 붙는 절은 5·6.3·7·9·11이고, 8절(제도 조건)과 10절(공표 점검)은 실행 없이 절차만 다룸  
...
□ 이 장에서 실행할 코드는 여섯 개이며 순서가 정해져 있음  

| 순서 | 실행할 파일 | 하는 일 | 절 |
| ---: | --- | --- | ---: |
| 1 | `13-0-simdata-prep.py` | 13-1·13-2·13-3용 시뮬레이션 자료 준비 | 5 |
| 2 | `13-1-decline-vulnerability-typology.py` | 소멸위험 등급·복합 취약성 지수·지역 유형화 | 5, 6.5 |
| 3 | `13-2-nightlight-economic-decline.py` | 프록시 수축 보정·쇠퇴 예측·예측구간 | 4.3, 7 |
| 4 | `13-3-synthetic-control.py` | 단일 처치 단위의 합성통제법 평가 | 9 |
| 5 | `13-0b-trade-area-data-prep.py` | 상가정보 원자료를 읍면동 상권 지표로 집계 | 6.3 |
| 6 | `13-4-trade-area-exit-decision.py` | 상권 세분화와 편의점 철수 판정 | 6.3~6.4, 11 |

○ 2·3·4번은 1번이 만든 parquet 파일을 입력으로 받으므로 **1번을 먼저 실행**해야 하고, 6번은 5번이 만든 집계표를 받으므로 **5번을 먼저 실행**해야 함  
○ 1~4번은 교육용 시뮬레이션 자료를 쓰므로 인터넷이 필요 없고, **5·6번만 실데이터(상가정보 2026년 6월 스냅숏)를 씀** — 상가(상권)정보 시도별 CSV 16개를 공공데이터포털에서 한 번 직접 내려받아 `lecture_practice/chapter13/data/raw/`에 두어야 5번이 돌아감  
○ 본문에 인용한 출력은 모두 `lecture_practice/chapter13/results/`의 실행 로그에서 그대로 가져온 것이며, 어느 파일에서 왔는지를 출력 블록 첫 줄에 적어 두었음 — 화면에 나온 값과 그 파일을 한 줄씩 대조하면 됨  

## 5. 복합 취약성 지수를 만든다

□ 먼저 실행함. 이 절의 숫자와 6.5의 유형별 표는 전부 이 두 명령의 출력임  
...
○ 첫 명령이 시군구 지표 300개를 `decline_regions.parquet`으로 만들고, 둘째 명령이 그 자료로 소멸위험 등급·복합 취약성 지수·지역 유형화를 한 번에 계산함  
○ 첫 명령을 건너뛰면 둘째 명령이 `데이터가 없습니다`를 찍고 멈춤  

```text
# 출처: lecture_practice/chapter13/results/13-0-simdata-prep.log
  시군구 지표 300개(20×15) → decline_regions.parquet
  야간조명·쇠퇴 지표 300개(20×15) → econ_regions.parquet
  SCM 패널 31단위×30년 → scm_Y.parquet·scm_units.parquet·scm_alpha.parquet
```

○ 한 번의 준비로 5절(13-1)·7절(13-2)·9절(13-3)이 쓸 자료가 모두 만들어짐  

### 5.1 지표가 여럿이면 순서가 정해지지 않는다
...
○ 아래는 다섯 단계를 그대로 밟은 코드에 실제 실행 결과를 주석으로 붙인 것임  

```python
# 주석에 붙인 출력의 출처: lecture_practice/chapter13/results/13-1-decline-vulnerability-typology.log
import numpy as np
import pandas as pd
from sklearn.preprocessing import StandardScaler

# [1] 소멸위험 등급 — 4.2의 구간을 그대로 씀
bins = [-np.inf, 0.2, 0.5, 1.0, np.inf]
labels = ["소멸고위험", "위험진입", "주의", "보통이상"]
df["risk_grade"] = pd.cut(df["decline_risk_index"], bins=bins, labels=labels)
# 출력: 소멸고위험   :   2개 ( 0.7%)
#       위험진입    :  56개 (18.7%)
#       주의      : 169개 (56.3%)
#       보통이상    :  73개 (24.3%)

# [2] 방향 정렬 — 값이 커질 때 취약해지도록 부호를 맞춤
ind = pd.DataFrame({
    "소멸위험": -df["decline_risk_index"],   # 낮을수록 위험 → 부호 반전
    "청년유출": -df["youth_netmove"],         # 음수(유출)일수록 취약 → 반전
    "서비스접근": df["service_access"],       # 클수록 취약
    "사업체부족": -df["biz_density"],         # 낮을수록 취약 → 반전
})

# [3]~[5] 표준화 → 가중치 → 결합
z = StandardScaler().fit_transform(ind)
weights = np.array([0.35, 0.20, 0.25, 0.20])
df["vulnerability"] = z @ weights
df["vuln_rank"] = df["vulnerability"].rank(ascending=False).astype(int)
# 출력: 가중치 소멸위험0.35·청년유출0.20·서비스접근0.25·사업체0.20
```

□ 우선지원 지역은 취약성 점수 내림차순으로 뽑음. 상위 열 곳의 출력은 다음과 같음  

```text
# 출처: lecture_practice/chapter13/results/13-1-decline-vulnerability-typology.log [3층 정책]
 region_id  decline_risk_index risk_grade  youth_netmove  service_access  biz_density  type  vulnerability  vuln_rank
       298                0.28       위험진입          -1.78           46.69         8.77     1           1.75          1
       284                0.39       위험진입          -3.30           45.49        22.07     1           1.62          2
       281                0.22       위험진입          -2.09           43.44        19.43     1           1.61          3
       299                0.27       위험진입          -4.14           33.36        18.03     1           1.54          4
       236                0.33       위험진입          -2.72           36.67        11.47     1           1.49          5
       296                0.39       위험진입          -3.93           38.55        23.95     1           1.48          6
       239                0.34       위험진입          -2.58           43.25        25.93     1           1.48          7
       293                0.21       위험진입          -2.27           35.59        15.56     1           1.47          8
       263                0.39       위험진입          -3.86           36.97        22.95     1           1.44          9
       238                0.17      소멸고위험          -2.76           31.81        17.64     1           1.43         10
```

□ 화면에 나온 값에서 읽을 것  
  ○ 등급 분포에서 **주의 구간이 169곳**으로 절반을 넘음(56.3%) — 소멸위험지수 하나로는 그 169곳 안에서 순서를 정할 수 없고, 복합 지수가 필요한 이유가 이 분포에 먼저 나타남  
  ○ 상위 열 곳 중 아홉 곳의 등급이 `위험진입`이고 소멸고위험은 한 곳뿐임 — **취약성 순위가 소멸위험 등급의 순서와 같지 않음.** 서비스접근과 사업체밀도가 순위를 함께 정하기 때문임  
  ○ 1위 지역(298)의 소멸위험지수는 0.28로 3위(281)의 0.22보다 높은데도 앞섬 — 서비스접근이 46.69분으로 더 멀고 사업체밀도가 8.77로 더 낮아서임  
  ○ 부호 반전을 빠뜨리면 이 표가 그대로 뒤집힘. [2]의 네 줄이 4.4에서 말한 방향 정렬이고, 계산은 정상 종료하므로 출력만 봐서는 잘못을 알 수 없음  
...
4. **판정** — 상위 집합이 유지되면 순위를 배분 근거로 씀. 유지되지 않으면 순위 대신 유형화(6절) 결과로 배분을 설명함

□ **판정 기준은 미리 문장으로 적어 둠.** "상위 20위 집합의 Jaccard가 0.8을 밑돌면 지수 단독 순위를 배분 근거로 쓰지 않는다"는 식임. **결과를 본 뒤에 기준을 정하면 그 기준은 판정이 아니라 사후 해석이 됨**  
  ○ **예를 들어** 5.3의 예시에서 인구 구조에 더 큰 비중을 두어 가중치를 소멸위험 0.45, 청년유출 0.20, 서비스접근 0.15, 사업체 0.20으로 바꾸면 지역 나(2위)와 다(3위)가 자리를 바꿈. 1위와 4위는 네 지표 모두에서 같은 방향이므로 가중치와 무관하게 자리를 지킴  
...
□ 지수가 지역을 한 줄로 세우는 반면 **유형화는 부족한 것이 서로 다른 지역을 서로 다른 묶음으로 나눔**  
  ...
  ○ 5.3의 예시에서 지역 나와 다는 점수가 0.33과 0.11로 가까웠지만 결손의 위치가 달랐음. 나는 생활서비스가 멀고 다는 청년이 빠져나감. **순위표만 보면 두 지역에 같은 패키지를 배정하게 되지만**, 유형을 함께 보면 나에게는 생활서비스 거점이, 다에게는 일자리가 먼저임  
...
○ 앞 명령이 상가(상권)정보 원자료 16개를 읽어 읍면동 상권 지표와 편의점 점포 목록을 만들고, 뒤 명령이 그 자료로 상권 세분화(6.3~6.4)와 철수 판정(11절)을 한 번에 계산함  
○ 6.3·6.4의 값과 11절의 표는 모두 이 한 번의 실행에서 나오며, 출력은 `[1]`부터 `[7]`까지 일곱 덩어리로 찍힘  

```text
# 출처: lecture_practice/chapter13/results/13-0b-trade-area-data-prep.log
  전국 합계 2,772,484행
  고시 목록 89곳 중 자료에서 확인된 시군구 89곳
  분석 우주: 점포 370,491개 (인구감소지역 292,738 / 비지정 군 77,753)
  읍면동 1,151개  (인구감소지역 1,036 / 비지정 군 115)
  convenience 점포 6,818개 (인구감소지역 5,084 / 비지정 군 1,734)
```
...
```python
# 주석에 붙인 출력의 출처: lecture_practice/chapter13/results/13-4-trade-area-exit-decision.log [1]
from sklearn.cluster import KMeans
from sklearn.metrics import adjusted_rand_score, silhouette_score

# [1] k를 2부터 8까지 바꿔 가며 실루엣을 계산하고 최대인 k를 고름
scores = {}
for k in K_RANGE:
...
# 출력: 실루엣 계수: k=2:0.182  k=3:0.212  k=4:0.195  k=5:0.188
#                    k=6:0.175  k=7:0.167  k=8:0.171
#       → 선택 k = 3 (최대 실루엣 0.212)

# [2] 안정성 — 시드를 바꿔 다시 군집하고 라벨 쌍의 ARI를 평균냄
...
# 출력: 안정성(시드 10개 평균 쌍별 ARI) = 1.000

# [3] 퇴화 검사 — 점포 수 분위만으로 만든 라벨과 비교함
size_bins = pd.qcut(units["n_stores"].rank(method="first"), k,
                    labels=False, duplicates="drop")
# 출력: [퇴화 검사] 점포 수 분위 라벨과의 ARI = 0.320
```

□ 화면에 나온 값에서 읽을 것  
  ○ 읍면동 상권 자료에서 k = 2부터 8까지의 실루엣은 0.182, **0.212**, 0.195, 0.188, 0.175, 0.167, 0.171로 k = 3에서 최대  
  ○ k = 8의 0.171이 k = 7의 0.167보다 도로 높아짐 — 실루엣이 k에 따라 단조롭게 줄지 않으므로, 최댓값 하나만 보지 말고 전 구간을 함께 실어 근거를 남김  
  ○ 시드를 열 개 바꿔도 라벨 쌍의 ARI가 1.000임 — 초기값이 유형을 정한 것이 아님. 다만 이 값이 높다고 유형이 존재한다는 뜻은 아니고, 같은 자료에서 같은 결과가 재현된다는 뜻일 뿐임  
...
□ **라벨이 만들어졌다고 유형이 존재하는 것은 아님.** 세 가지를 계산해 함께 보고함. 아래 실측값은 6.3에서 실행한 `13-4`의 `[1]`·`[2]` 구간 출력임  
...
□ 이 절의 표는 5절에서 실행한 `13-1`의 `[2층 보조]` 구간 출력임. 같은 표준화 행렬 z에 KMeans(k=4)를 얹어 시군구 300개를 네 묶음으로 나눔  

```text
# 출처: lecture_practice/chapter13/results/13-1-decline-vulnerability-typology.log [2층 보조]
        n  소멸위험지수  청년순이동  서비스접근분  사업체밀도   취약성
type
1      41    0.44  -2.66   37.26  24.84  1.23
3      89    0.57  -1.36   31.26  39.62  0.59
2     112    0.83   0.02   23.25  54.40 -0.24
0      58    1.32   1.53   15.42  68.16 -1.29
```
...
□ **네 유형이 네 지표에서 모두 같은 순서로 배열됨.** 이 배열은 자료가 도시-농촌 경사를 따라 설계된 결과이며, **실제 자료에서 이런 배열이 나오면 6.4의 퇴화 검사가 필요한 전형적 조건임**  
...
□ 먼저 실행함. 이 절의 숫자와 **4.3에서 미리 인용한 세 값**이 전부 이 명령의 출력임  

```bash
python lecture_practice/chapter13/code/13-2-nightlight-economic-decline.py
```

○ 입력은 5절의 `13-0`이 만든 `econ_regions.parquet`이며, 출력은 프록시 검증 → 측정오차 편의 → 예측 → 예측구간 순으로 찍힘  

```text
# 출처: lecture_practice/chapter13/results/13-2-nightlight-economic-decline.log
[프록시 검증] 야간조명 ↔ (관측 불가) 진짜 경제활동
  상관 r = 0.872 (공유분산 r² = 0.760)

[규칙4] 거친 프록시를 예측변수로 쓸 때 측정오차 편의(attenuation)
  쇠퇴~진짜경제 표준화계수(오라클)   = -0.962
  쇠퇴~야간조명 표준화계수(관측)     = -0.838  (attenuation 12.9% 0쪽 수축)
  attenuation 보정 후(÷ r)            = -0.961  (진짜계수 복원)
```

□ 화면에 나온 값에서 읽을 것  
  ○ 오라클 계수 −0.962는 **시뮬레이션이라서 볼 수 있는 값임.** 실제 분석에서는 참 경제활동을 관측할 수 없으므로 −0.838만 손에 들어오고, 수축이 얼마나 일어났는지를 사후에 확인할 방법이 없음  
  ○ 보정은 상관 r로 나누는 한 번의 나눗셈임 — `−0.838 ÷ 0.872 = −0.961`. 코드는 이 r을 야간조명과 `true_econ`의 상관으로 구하는데, 참값이 없는 실제 분석에서는 r을 다른 근거로 확보해야 하고 그 출처를 밝히지 않은 보정은 근거가 없음  
...
```python
# 주석에 붙인 출력의 출처: lecture_practice/chapter13/results/13-2-nightlight-economic-decline.log
from sklearn.ensemble import GradientBoostingRegressor

# [1] 이웃 네 방향의 평균을 공간 시차 열로 만듦
...
# 출력: 비공간 피처(야간조명+행정지표)  R² =  0.830
#       +공간 시차 피처                 R² =  0.887  (개선 +0.057)

# [3] 채택한 구성으로 전체를 적합해 피처 중요도를 봄
...
□ 같은 실행의 `[불확실성]` 구간에 상위 열 곳이 그대로 찍힘  

```text
# 출처: lecture_practice/chapter13/results/13-2-nightlight-economic-decline.log [불확실성]
  90% 예측구간 경험적 포함률(OOF) = 0.903  (목표 0.90)
  구간 반폭(쇠퇴율 %p): 중앙값 1.21, 범위 [0.35, 3.61]
  신뢰등급 '낮음' 시군구 = 100개

 region_id  pred_decline  priority_rank  pi_half  pi_low  pi_high confidence
       246        10.428              1    3.139   7.289   13.567         낮음
       204         9.600              2    1.018   8.582   10.617         높음
       188         8.539              3    0.725   7.814    9.265         높음
       218         8.535              4    0.837   7.698    9.373         높음
       234         8.474              5    0.658   7.815    9.132         높음
       219         8.457              6    1.474   6.984    9.931         낮음
       231         8.218              7    1.406   6.812    9.624         낮음
       187         8.164              8    0.837   7.327    9.001         높음
       249         8.144              9    0.509   7.635    8.653         높음
       230         8.090             10    0.799   7.292    8.889         높음
```

○ 아래 표는 이 출력에서 논점이 되는 네 행만 뽑은 것임  
...
| 성과지표 요구 | 사업별 성과지표의 종류와 보고 주기 | 지표 조작 가능성 점검(10절), 복수 지표 설계 |
...
○ 세 번째 행은 5.5의 민감도 검사와 연결됨. **상위 20 집합의 Jaccard가 판정 기준을 밑돌면 그 표의 순위 열은 배분 근거로 쓰지 않고**, 유형 열과 예측 열만 남겨 유형별 패키지 배정으로 전환함  

## 9. 정책효과를 합성통제법으로 평가한다

□ 먼저 실행함. 이 절의 가중치·RMSPE·격차·p값은 전부 이 명령의 출력임  

```bash
python lecture_practice/chapter13/code/13-3-synthetic-control.py
```

○ 입력은 5절의 `13-0`이 만든 `scm_Y.parquet`·`scm_units.parquet`·`scm_alpha.parquet`이며, **참 효과를 미리 심어 둔 시뮬레이션이라 SCM이 그것을 되찾는지 채점할 수 있음**  
...
○ 실행 결과 — 31개 지자체(처치 1 + 기증 풀 30)의 30년 패널, 처치는 20년차에 시작, 참 효과(설계값)는 사후 평균 4.40  

```python
# 주석에 붙인 출력의 출처: lecture_practice/chapter13/results/13-3-synthetic-control.log
from scipy.optimize import minimize

# [1] 볼록조합 제약 아래 사전 궤적을 가장 잘 재현하는 가중치를 찾음
cons = ({"type": "eq", "fun": lambda w: np.sum(w) - 1.0,
         "jac": lambda w: np.ones_like(w)},)          # Σw = 1
bounds = [(0.0, 1.0)] * n                              # w ≥ 0
res = minimize(loss, w0, jac=grad, bounds=bounds, constraints=cons,
               method="SLSQP", options={"maxiter": 1000, "ftol": 1e-12})
# 출력: 통제 #3  0.473 | 통제 #18 0.157 | 통제 #9  0.138 | 통제 #28 0.113
#       통제 #14 0.079 | 통제 #5  0.024 | 통제 #8  0.016
#       (가중치 0.005 미만 23개 단위 생략, Σw = 1.000)

# [2] 사전·사후 RMSPE로 적합도와 격차 크기를 잼
pre_r = rmspe(gap, 0, t0)
post_r = rmspe(gap, t0, Y_all.shape[1])
# 출력: 사전기간(1~20년) RMSPE = 1.090 지수  (관측 잡음 SD 1.3 수준)
#       사후기간(21~30년) RMSPE = 4.658 지수

# [3] 기증 풀의 각 단위를 차례로 가짜 처치로 두고 같은 계산을 반복함
# 출력: 처치 지자체 RMSPE 비율(post/pre) = 4.27
#       기증 풀 placebo 비율: 중앙값 1.37, 최댓값 2.91
#       전체 31개 단위 중 처치 지자체 비율 순위 = 1위 → 유사 p값 = 0.032
```

□ 연차별 격차는 같은 실행의 `[3]` 구간에 그대로 찍힘  

```text
# 출처: lecture_practice/chapter13/results/13-3-synthetic-control.log [3]
    연차 |       실제 |       합성 |     격차(효과) |     참 효과
    21 |   101.14 |   101.63 |      -0.49 |     0.80
    22 |   100.78 |    99.28 |       1.50 |     1.60
    23 |    99.96 |    97.87 |       2.09 |     2.40
    24 |   100.67 |    97.47 |       3.20 |     3.20
    25 |   102.94 |    98.00 |       4.93 |     4.00
    26 |   102.79 |    99.00 |       3.79 |     4.80
    27 |   105.43 |    97.34 |       8.09 |     5.60
    28 |   103.81 |   100.24 |       3.57 |     6.40
    29 |   107.98 |   101.10 |       6.88 |     7.20
    30 |   108.13 |   102.15 |       5.97 |     8.00
  사후 평균 격차(SCM 추정) = 3.95 지수  (참 사후 평균 4.40)
```

□ 화면에 나온 값에서 읽을 것  
  ○ 첫 해(21년차) 격차가 **−0.49로 음수임** — 참 효과는 이미 0.80인데 추정은 방향조차 반대로 나옴. 연차 하나의 격차를 효과로 읽으면 안 되는 이유가 이 한 줄에 있음  
  ○ 사후 평균으로 묶으면 3.95로 참값 4.40에 가까워짐. **SCM이 복원하는 것은 연차별 궤적이 아니라 사후 구간의 평균임**  
  ○ 27년차의 격차 8.09는 참 효과 5.60보다 크고 28년차의 3.57은 6.40보다 작음 — 잡음이 연차별로 남아 있으며, 이 흔들림의 크기가 사전 RMSPE 1.090이 재는 값임  
...
## 11. 사례 전체: 상권 세분화와 점포 철수 결정

□ 이 절의 표와 출력은 6.3에서 실행한 것과 같은 명령의 나머지 구간임. 6.3을 건너뛰었다면 여기서 실행함  
...
○ 출력 일곱 덩어리 가운데 `[1]`·`[2]`는 6.3~6.4에서 읽었고, 이 절은 `[2]`의 공백률과 `[3]`~`[7]`을 읽음  
...
□ 오른쪽 세 열을 뒤집으면 공백률이 됨. 같은 실행의 `[2]`·`[3]` 구간이 그 값을 그대로 찍음  

```text
# 출처: lecture_practice/chapter13/results/13-4-trade-area-exit-decision.log [2]·[3]
[2] 실측 — 유형별 생활서비스 공백률 (편의점·약국·의원이 하나도 없는 읍면동)
  편의점 공백률 — 전체 24.5% | 유형0:44.5%  유형1:0.3%  유형2:20.2%
      [대조 N3] 유형 라벨 무작위화 1000회: 산포 0.1809, p = 0.001
  약국 공백률 — 전체 47.6% | 유형0:70.4%  유형1:4.0%  유형2:56.9%
      [대조 N3] 유형 라벨 무작위화 1000회: 산포 0.2865, p = 0.001
  의원 공백률 — 전체 64.2% | 유형0:87.4%  유형1:19.3%  유형2:74.3%
      [대조 N3] 유형 라벨 무작위화 1000회: 산포 0.2952, p = 0.001

[3] 대조 — 인구감소지역 고시 여부 (군 지역만 비교)
  비교 대상: 군 지역 읍면동 823개 (지정 708 / 비지정 115)
  편의점 공백률 — 지정 30.4% vs 비지정 9.6% | 차이 +20.8%
      [대조 N4] 지정 라벨 무작위화 1000회: p = 0.001
  약국 공백률 — 지정 54.8% vs 비지정 29.6% | 차이 +25.2%
      [대조 N4] 지정 라벨 무작위화 1000회: p = 0.001
  의원 공백률 — 지정 72.2% vs 비지정 42.6% | 차이 +29.6%
      [대조 N4] 지정 라벨 무작위화 1000회: p = 0.001
```

□ 편의점 공백은 전체 24.5%, 약국 47.6%, 의원 64.2%  
...
□ 컷라인은 손으로 고른 값이 아니라 단위 경제에서 닫힌 형태로 나옴  

```python
# 주석에 붙인 출력의 출처: lecture_practice/chapter13/results/13-4-trade-area-exit-decision.log [4]
# 고정비 F = 1. 월 기여이익 m(t) = s_ratio·(1+g)^t − 1
t = np.arange(1, T + 1)
disc = (1.0 + r) ** (-t.astype(float))
A = float(np.sum(((1.0 + g) ** t) * disc))
B = float(np.sum(disc))
E = psi * rho * T + R * rho            # 잔여 위약금 + 원상복구비

V = s_ratio * A - B                    # 유지 가치
s_cut_now = (B - E) / A                # 즉시 철수 컷라인
s_cut_expiry = (1.0 + g) ** (-T)       # 만료 시점에 적자가 되는 컷라인
# 출력: 즉시 철수 비용 E = 3.92 (고정비 3.92개월분)
#       컷라인(관측 단위): 즉시 철수 s < 42.79 | 만료 시점에 적자 s < 54.96
#       판정 3분류: 즉시 철수 2,436개 (35.7%) | 만료 시 철수 1,799개 (26.4%)
#                   | 유지 2,583개 (37.9%)
```

□ 화면에 나온 값에서 읽을 것  
  ○ 컷라인 두 개를 가르는 것은 `E = 3.92`임 — 즉시 닫는 데 고정비 3.92개월분이 들기 때문에 즉시 철수 컷라인 42.79가 손익분기 쪽 컷라인 54.96보다 **아래**에 놓임  
  ○ `s_cut_expiry`의 식에 E가 들어 있지 않음. 만료까지 버티면 위약금과 원상복구비를 내지 않으므로, 그 컷라인은 수요 감소율 g와 잔여 기간 T만으로 정해짐  
  ○ 비율 35.7%·26.4%·37.9%는 **중앙값 점포의 월 기여이익을 0에 맞춘 정규화가 정한 값임.** 정규화를 바꾸면 세 비율이 함께 움직이므로 비율 자체는 결론이 아님  
...
□ 가정값 여섯 개를 각각 ±30% 흔든 결과가 같은 실행의 `[7]` 구간에 있음  

```text
# 출처: lecture_practice/chapter13/results/13-4-trade-area-exit-decision.log [7]
        파라미터  배수       값   즉시철수  만료시철수     유지  컷라인(관측)  Jaccard
  rent_share 0.7  0.2450 0.3925 0.2287 0.3789   45.487    0.910
  rent_share 1.3  0.4550 0.3190 0.3021 0.3789   40.089    0.893
     penalty 0.7  0.2100 0.3808 0.2404 0.3789   44.523    0.938
     penalty 1.3  0.3900 0.3244 0.2967 0.3789   41.053    0.908
     restore 0.7  2.8000 0.3668 0.2543 0.3789   43.752    0.974
     restore 1.3  5.2000 0.3294 0.2917 0.3789   41.824    0.922
    discount 0.7  0.0035 0.3573 0.2639 0.3789   42.967    1.000
    discount 1.3  0.0065 0.3573 0.2639 0.3789   42.607    1.000
       decay 0.7 -0.0035 0.3328 0.2576 0.4097   42.018    0.931
       decay 1.3 -0.0065 0.3654 0.3016 0.3331   43.569    0.978
lease_months 0.7 17.0000 0.3221 0.2715 0.4064   40.953    0.901
lease_months 1.3 31.0000 0.3803 0.2866 0.3331   44.086    0.939

  즉시 철수 비율 범위 31.9%~39.2%, Jaccard 최소 0.893
```

○ 즉시 철수 비율은 31.9%에서 39.2%까지 움직이지만 판정 집합의 Jaccard는 0.893 아래로 내려가지 않음 — **가정이 바꾸는 것은 몇 개를 닫느냐이고, 어느 점포를 먼저 닫느냐는 거의 그대로임**  
○ 할인율을 ±30% 흔든 두 행의 Jaccard가 1.000임 — 잔여 24개월 구간에서는 월 할인율 0.005의 크기가 판정을 전혀 바꾸지 않음  

### 11.4 대리변수의 결함이 드러난 지점
...
□ 방향이 뒤집힌 대목과 그 진단은 같은 실행의 `[5]`·`[6]` 구간에 있음  

```text
# 출처: lecture_practice/chapter13/results/13-4-trade-area-exit-decision.log [5]·[6]
  (나) 모형 부분: 즉시 철수 후보 2,436개 중 인구감소지역 소재 1,651개 (67.8%)
       점포 대비 즉시 철수 비율: 인구감소지역 32.5% vs 비지정 군 45.3%
       → **방향이 뒤집혔다.** 인구감소지역 쪽이 오히려 낮다.
  (다) 왜 뒤집혔는가 — 프록시를 진단한다
       s 중앙값: 인구감소지역 51.49 vs 비지정 군 46.38
       읍면동 총 점포 수 중앙값: 668 vs 1266 | 편의점 비중 중앙값: 0.019 vs 0.022
       s와 편의점 비중의 상관 = -0.768

[6] 대조군(null control) — 심은 메커니즘만 제거해 다시 돌린다
  N1 동종 경쟁 나눗셈 제거(s = 총 점포 수): 즉시 철수 44.9% (기준 35.7%), Jaccard 0.358
  N1b 배후 대리 교체(s = 생활밀착 업종 수 / 편의점 수, 수리·개인 업종 0인 점포 17개 제외): 즉시 철수 39.6%, Jaccard 0.602
      지정 39.1% vs 비지정 군 41.3%  → 프록시를 바꿔도 방향이 그대로다(결함이 배후 대리 자체에 있다)
  N2 수요 감소 추세 제거(g = 0): 즉시 철수 32.0%, 만료 시 철수 17.8%, Jaccard 0.894
```
...
□ **이 사례가 남기는 것은 결론을 뒤집은 것이 더 정교한 모형이 아니라 대조군과 프록시 교체였다는 점임.** 유의성만 확인했다면 즉시 철수 비율의 지역 간 차이를 실측과 반대 방향으로 보고했을 것임  
  ○ 공표 원칙(10.2)은 공공에만 요구되지 않음. "이 지역 점포는 정리 대상"이라는 내부 판단이 밖으로 알려지면 가맹 희망자·임대인·주민의 기대가 함께 움직임. **11.3의 판정은 파라미터를 바꾸면 움직이므로 결정이지 예측이 아니며, 사실로 유통되면 가정이 시장의 정보가 됨**  
...
○ **복합 지수는 정보를 늘리는 것이 아니라 줄이는 도구임.** 축 사이의 상충이 사라지고 그 자리에 가중치라는 선택이 들어감 (5.1)  
○ **순위를 배분 근거로 쓰려면 민감도 검사를 함께 보고함.** 판정 기준은 결과를 보기 전에 문장으로 적어 두고, 흔들리는 것은 순위 전체가 아니라 축마다 방향이 다른 중간 구간임 (5.5)  
○ **라벨이 만들어졌다고 유형이 존재하는 것은 아님.** 특히 퇴화 검사에서 단일 변수 분위 라벨과의 ARI가 0.9를 넘으면 유형이 아니라 규모 구간이라는 이름으로 보고함 (6.4)  
○ **가장 쇠퇴가 심하다고 지목된 지역이 동시에 가장 불확실할 수 있음.** 실습에서 1순위의 구간 하한이 2순위 점추정보다 낮았고, 조치는 순위를 내리는 것이 아니라 확정 전에 불확실성을 줄이는 것임 (7.2)  
○ **처치 단위가 하나여도 반사실을 만들 수 있지만, SCM이 식별하는 것은 사전 궤적의 재현이지 비교 대상의 선정이 아님.** 참 생성 단위의 가중 합이 0.489에 그쳤어도 사전 적합은 잡음 한계까지 좋았음 (9.3, 9.4)  
○ **공표가 현실의 궤적을 바꿈.** 11장·12장의 되먹임은 자료를 왜곡해 보정할 수 있지만 이 되먹임은 세상이 바뀌므로 대응이 공표 단계에 놓임 (10.1)  
○ **결론을 뒤집은 것은 정교한 모형이 아니라 대조군과 프록시 교체였음.** 상권 사례에서 즉시 철수 비율의 지역 간 차이가 대리변수의 결함으로 반대 방향이 될 뻔했음 (11.4)  
