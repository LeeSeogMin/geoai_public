...
□ 4.4에서 직접 실행할 `14-0-data-prep.py`의 출력에 다음이 나옴  
...
□ **이 숫자들만으로도 문제의 크기가 드러남.** 강남구 한 자치구의 카페가 원자료 기준 2,123개임  
  ○ 이 가운데 생활인구와 결합된 2,113개가 분석 대상이고, 그 안에서 자사(스타벅스)가 102개, 경쟁 브랜드가 2,011개임  
  ○ 빠진 10개는 점포의 행정동코드가 생활인구 자료에 없어 결합되지 않은 것임(결합률 99.5%)  
  ○ 즉 "카페 하나를 더 연다"는 결정은 **이미 붐비는 시장에 한 자리를 더 끼워 넣는 일**임  

□ 5절에서 직접 실행할 `14-1-huff-location-cannibalization.py`의 출력에는 이 붐빔이 계산에 그대로 반영됨  
...
□ 배분받는 1,228명 가운데 자기잠식은 80명뿐이고 나머지는 순증임  
  ○ **왜 자기잠식이 이렇게 작은지는 6절에서 계산으로 확인함.** 결론을 미리 말하면, 자사 점유율(4.8%)이 낮을수록 신규 매장이 가져오는 수요의 대부분이 경쟁사에서 온다는 계산 구조 때문임  
...
○ 이 코드는 눈으로만 보고 넘기지 않음 — **5절 첫머리에서 같은 파일을 직접 실행하고, 위의 4.8%·1,228명·80명이 화면에 그대로 찍히는 것을 확인함**  
...
○ **철수는 부호만 반대임.** 매장을 빼고 다시 계산하면 남은 자사 매장이 흡수하는 몫이 나오고, 그 몫을 원래 배분에서 빼면 손실이 나옴(10절)  
...
□ **고객점표법**은 실제로 온 손님의 거주지를 지도에 찍어 침투율이 떨어지는 거리를 찾는 방법임(Applebaum, 1966). 정확하지만 **이미 영업 중인 매장에만 쓸 수 있음** — 아직 열지 않은 후보지에는 손님이 없어서 쓸 자료가 없음  
  ○ 그래서 **신규 후보지의 상권은 모형으로 그림**(Huff, 1964). 5절의 흡인 모형으로 각 지역에서 해당 매장을 고를 확률을 계산하고, 확률이 같은 지점을 이으면 확률 등고선이 나옴  
...
□ 5절부터 9절까지는 설명과 실습을 나누지 않음 — 각 절 첫머리에 실행할 명령이 나오고 그 아래에 코드와 실제 실행 결과가 이어짐. 순서대로 따라가면 이 장의 모든 수치를 직접 만들게 됨  
...
```
  ...
  ○ 14장이 쓰는 패키지만 따로 보려면 `lecture_practice/chapter14/code/requirements.txt`에 적혀 있으나, 설치는 위 공통 파일 하나로 끝남  

□ 원자료 두 개를 내려받음. 이 장은 시뮬레이션 자료가 아니라 국내 공개 자료를 쓰므로 이 단계를 건너뛸 수 없음  
  ○ 내려받기 안내: `lecture_practice/chapter14/data/raw/README.md`  
  ...
  ○ 저장소에는 `data/` 폴더를 넣지 않으므로 위 README도 내려받기 전에는 보이지 않음 — 같은 절차가 실습 폴더의 `lecture_practice/chapter14/README.md`에도 적혀 있음  

□ 이 장에서 실행할 코드는 두 개이며 순서가 정해져 있음  

| 순서 | 실행할 파일 | 하는 일 | 절 |
| ---: | --- | --- | ---: |
| 1 | `14-0-data-prep.py` | 원자료 두 개를 강남구 카페·행정동 수요 파일로 정리 | 4.4 |
| 2 | `14-1-huff-location-cannibalization.py` | 흡인 모형으로 배분·자기잠식·순증을 계산하고 제약·민감도를 검토 | 5~9 |

○ 2번은 1번이 만든 `cafes.parquet`·`demand_dong.parquet`을 입력으로 받으므로 **1번을 먼저 실행**해야 함  
○ GPU는 필요 없고, 인터넷도 원자료를 받을 때만 필요함 — 두 코드 자체는 로컬 파일만 읽음  
○ 산출 파일은 후보지 540개의 배분·자기잠식·순증을 담은 `location_candidates.csv`이고, 실행 로그는 `lecture_practice/chapter14/results/*.log`에 남음  
○ 본문에 인용한 출력은 모두 `lecture_practice/chapter14/results/`의 실행 로그에서 그대로 가져온 것이며, 어느 파일에서 왔는지를 출력 블록 첫 줄에 적어 두었음  

□ 첫 번째 코드를 실행함. 2절에서 미리 적은 점포 수와 결합률이 여기서 나옴  
...
```text
# 출처: lecture_practice/chapter14/results/14-0-data-prep.log
[1/2] 상가(상권)정보
  원본: 소상공인시장진흥공단_상가(상권)정보_서울_202606.csv (302 MB, utf-8-sig)
  강남구 '카페' 점포 2123개, 행정동 22개
  상호명 상위: {'스타벅스': np.int64(102), '투썸플레이스': np.int64(12), '매머드익스프레스': np.int64(7)}

[2/2] 생활인구(행정동)
  원본: 행정동 단위 서울 생활인구(내국인).csv (179 MB, cp949)
  행정동 424개 | 기간 20260612~20260731 | 11~15시 평균

결합 점검: 점포의 행정동코드 중 생활인구와 매칭 99.5% (2113/2123)
저장: cafes.parquet(2123행), demand_dong.parquet(424행), SOURCES.txt
```

□ 화면에 나온 값에서 읽을 것  
  ○ 강남구 한 자치구의 소분류 '카페' 점포가 2,123개이고 행정동 22개에 걸쳐 있음 — 2절에서 말한 붐빔의 근거가 이 한 줄임  
  ○ 상호명 1위 스타벅스가 102개로, 점포 수가 가장 많은 브랜드조차 전체의 5%에 미치지 못함. 5절의 자사 점유율 4.8%가 여기서 나옴  
  ○ 결합률 99.5%(2,113/2,123)는 4.1의 1번 항목(공간 단위)을 실제로 점검한 값이며, 행정동코드가 생활인구 자료에 없는 점포 10개는 뒤 계산에서 빠짐  
  ○ 생활인구는 서울 424개 동 전체를 11~15시 평균으로 집계해 두고, 그중 강남구 동만 뒤에서 결합해 씀(4.2)  
  ○ 기준 시점은 상가정보 2026년 6월판, 생활인구 2026-06-12~07-31임. **공개 자료가 갱신되면 아래 수치가 달라지므로 본문 수치는 이 기준과 함께 인용함**  

## 5. 흡인 모형으로 배분을 계산한다

□ 두 번째 코드를 실행함. 5절부터 9절까지의 모든 표가 이 한 번의 실행에서 나옴  
...
```text
# 출처: lecture_practice/chapter14/results/14-1-huff-location-cannibalization.log
대상: 강남구 카페 2113개 | 자사 102개 / 경쟁 2011개 (자사 점포 점유율 4.8%)
  수요 단위: 행정동 21개 | 낮 활동 인구 합계 993,215명
  후보지: 250m 격자 540개 | 자사 최근접 거리 중앙값 301m
```

□ 화면에 나온 값에서 읽을 것  
  ○ 2절에서 미리 적은 자사 점유율 4.8%가 여기서 나옴 — 생활인구와 결합된 2,113개 가운데 자사가 102개, 경쟁이 2,011개임  
  ○ 4.4에서 22개였던 행정동이 여기서는 21개임 — 코드가 맞지 않아 빠진 점포 10개가 모두 한 동에 몰려 있어 그 동이 통째로 빠졌기 때문임  
  ○ 후보지 540개는 점포 분포 범위에 250m 격자를 깔고 가장 가까운 점포까지 375m(격자 1.5칸) 이내인 칸만 남긴 것임 — 점포가 없는 구역에는 후보를 놓지 않음  
  ○ 수요 지점은 21개인데 후보지는 540개임. **3.2에서 미리 적은 해상도 제약이 이 두 숫자의 차이로 드러남** — 같은 동 안의 후보지들은 같은 인구를 거리만 달리해 나눠 받음  
  ○ 이 실행의 출력은 여섯 덩어리로 이어짐 — 표 14.4·14.5(9절), 자기잠식 상한 판정(7.2), 표 14.6·14.7과 퇴화 검사(7.3)  

### 5.1 거리와 매력도가 반대로 작용할 때
...
○ 강남구 사례는 둘 다 자료가 없는 조건임. 그래서 매력도는 상수에서 출발해 집적도로 바꿔 보고, β는 1.5·2.0·2.5 세 값으로 비교함(7.3)  

> **주의.** β와 매력도를 관행값으로 고정한 채 배분 수요의 **절대량**을 그대로 보고하면 안 됨. 7.3에서 확인하듯 β를 1.5에서 2.5로 바꾸면 최선 후보의 순증이 721명에서 1,478명으로 두 배가 됨. 방문 자료로 β를 추정하지 못했다면 **순위만 인용하고 절대량은 결정 근거로 쓰지 않음**.

## 6. 자기잠식과 순증을 가른다

□ 이 절의 강남구 수치는 5절에서 실행한 그 명령의 출력임. 계산이 코드의 어디에서 이루어지는지 먼저 확인함  

```python
# 주석에 붙인 출력의 출처: lecture_practice/chapter14/results/14-1-huff-location-cannibalization.log
# [1] 개점 전 — 수요 지점별 분모 D와 그중 자사 몫 S
D = w.sum(axis=1)
S = w[:, own_mask].sum(axis=1)

# [2] 개점 후 — 분모에 신규점 항 a가 하나 늘어남
Dn = D[:, None] + a

# [3] 신규점 배분(gross)과 자사 기존점 감소분(loss)
gross = (pop[:, None] * a / Dn).sum(axis=0)
loss = (pop[:, None] * S[:, None] * (1.0 / D[:, None] - 1.0 / Dn)).sum(axis=0)
# 출력: 총배분 1위: 배분 1,228명, 전이율 6.5%, 순증 1,148명
#       전이율 분포: 중앙값 4.8%, 최대 6.6%
```

□ 화면에 나온 값에서 읽을 것  
  ○ `Dn = D + a` 한 줄이 6.2의 2단계를 그대로 옮긴 것임 — 신규점을 놓아도 전체를 다시 계산하지 않고 분모에 항 하나만 더함  
  ○ `loss`에는 `S`(자사 몫)만 곱해져 있음 — 경쟁사가 잃은 몫은 자기잠식에 넣지 않는다는 6.2의 3단계가 이 곱셈 하나로 표현됨  
  ○ 총배분 1위 후보의 전이율 6.5%는 후보 540개의 중앙값 4.8%보다 높음. 배분이 큰 후보일수록 전이율도 높게 나왔다는 뜻이며, 그래서 배분이 아니라 순증으로 다시 정렬함  

### 6.1 전이율의 정의는 하나가 아니다
...
□ 5.2의 계산 예로 두 값을 비교함. 매장 A와 C가 자사라고 하면 **정의 A로는 267 / 400 = 66.8%, 정의 B로는 267 / 667 = 40.0%**임  
...
□ 실증 연구도 같은 방향을 보고함. Pancras, Sriram, Kumar(2012)는 한 체인의 출점 이력을 분석해 신규 매장 매출의 86.7%가 순증이고, **매장 사이 거리가 1마일 멀어질 때마다 잠식되는 매출이 28.1%씩 줄어 10마일에서는 거의 사라진다**고 보고함  
  ○ 즉 **자기잠식은 거리가 멀수록, 자사 점유율이 낮을수록 작아짐.** 이 관계 때문에 이격 거리를 제약으로 걸어 잠식을 통제할 수 있음(7.2)  

### 6.4 이 계산이 놓치는 것 — 고정 시장 가정

□ 5.2의 계산 예에서 **신규 매장 배분(400명)과 기존 매장 감소분 합계(400명)가 정확히 일치했음**  
...
○ 이 절에서 새로 실행할 명령은 없음 — 7.2의 제약 판정과 7.3의 민감도·퇴화 검사는 모두 5절에서 한 번 실행한 `14-1-huff-location-cannibalization.py`의 출력이며, 표마다 로그의 어느 구간에서 왔는지를 적어 둠  

### 7.1 여러 변수를 하나의 점수로 합치기
...
○ 규제처럼 **"통과냐 아니냐"로만 갈리는 변수는 점수에 넣지 않고 7.2의 제약으로 처리함**  
...
○ 아래 표는 5절에서 실행한 로그의 "결정 — 자기잠식 상한은 언제 구속력을 갖는가" 구간을 옮긴 것임  
...
### 7.3 가정을 바꿔도 순위가 유지되는가

○ 강남구 사례에서 자료 없이 정한 값은 두 개, β와 매력도(A)임  

○ 아래 표는 같은 로그의 표 14.6(거리 저항 β)과 표 14.7(매력도 A)을 하나로 합친 것임  
...
```text
# 출처: lecture_practice/chapter14/results/14-1-huff-location-cannibalization.log
퇴화 검사 — 결과가 '사람 많은 곳'을 되풀이하는 데 그치는가
  총배분 ~ 최근접 행정동 낮인구 상관: -0.051
  순증  ~ 최근접 행정동 낮인구 상관: -0.052
  순증  ~ 자사 최근접 거리 상관    : -0.046
```
...
○ 앞 절에서 나온 숫자들을 결정의 흐름 순서로 다시 정리함. 여기서도 새로 실행할 명령은 없고, 5절에서 실행한 같은 로그를 결정 순서로 다시 늘어놓은 것임  

**단계별 값**

○ **결정·단위**: 신규 출점. 후보지는 점포 분포 범위에 깐 250m 격자 540개, 수요 지점은 행정동 21개, 매장은 점 단위. 자사 최근접 거리 중앙값 301m  
○ **자료**: 강남구 카페 원자료 2,123개 중 활동 인구와 결합된 2,113개(결합률 99.5%)가 분석 대상. 자사 102개, 경쟁 2,011개, 자사 점유율 4.8%. 21개 행정동 낮 활동 인구 합계 993,215명  
○ **수요**: 각 행정동의 대표점은 그 동에 있는 카페 좌표의 중심으로 잡음(상업 밀도 중심점)  
○ **배분·잠식**: β = 2.0, 매력도 상수로 계산  

○ 아래 두 표는 5절에서 실행한 로그의 표 14.4(상위 10곳)와 표 14.5(거리대별)를 그대로 옮긴 것임  
...
□ **총배분 순위와 순증 순위가 열 곳 모두에서 일치함.** 자기잠식이 71~80명, 전이율이 6.5~6.6%로 좁은 범위에 머물러 순위를 뒤집을 만큼 크지 않기 때문임  
  ○ 6.3에서 확인한 대로, 자사 점유율(4.8%)이 낮아서 신규 매장이 가져오는 수요의 대부분이 경쟁 브랜드에서 옴  
...
□ 거리와 전이율의 상관은 **−0.333**임. **방향(멀수록 잠식이 작아짐)은 6.3의 이론과 맞지만, 구간별 평균이 단조롭게 줄지는 않음**(500~800m 구간이 앞 두 구간보다 오히려 높음)  
...
□ **철수**: 자사 매장 하나를 없애고 배분을 다시 계산하면 분모에서 항이 하나 빠져 남은 모든 매장의 확률이 올라감. 올라간 몫 가운데 자사 매장이 회수하는 부분이 **회수분**이고, 철수 손실 = 제거된 매장의 배분 − 회수분임  
  ○ 5.2의 계산 예를 거꾸로 돌리면 확인됨. 매장 A와 C가 자사일 때 C를 닫으면 분모가 다시 줄어 A는 667명으로 돌아감. **회수분은 667 − 400 = 267명, 회수율 66.8%, 철수 손실은 400 − 267 = 133명임**  
  ○ **이 값이 6.1에서 본 전이율(66.8%)·순증(133명)과 똑같음.** 출점과 철수가 같은 계산의 두 방향이기 때문임 — **자사 점유율이 높을수록 출점 시 전이율이 크고, 철수 시 회수율도 큼**  

○ 5.2의 계산 예를 양방향으로 놓으면 이 대칭이 한눈에 들어옴(A·C가 자사, B가 경쟁사, 인구 1,000명)  
...
○ **출점 결정은 배분 수요가 아니라 순증으로 정렬함.** 자사 매장에 가까운 후보는 감소분이 커서, 배분이 같아도 회사 전체로 늘어나는 양이 적음 (6.2)  
○ **매장이 하나 늘면 나머지 전부의 확률이 함께 줄어듦.** 경쟁을 별도 변수로 넣는 것이 아니라 분모에 항을 더하는 구조이기 때문임 (5.1)  
○ **신규 매장이 받는 몫은 반드시 다른 매장에서 옴.** 계산 예에서 신규 400명이 정확히 A 267명 + B 133명이었음 (5.2)  
○ **전이율은 분모를 무엇으로 두느냐로 66.8%와 40.0%가 갈림.** 산출표 열 이름에 분모를 함께 적어야 함 (6.1)  
○ **자사 점유율이 낮으면 자기잠식도 작게 계산됨.** 강남구 사례의 점유율 4.8%가 전이율 6.5%를 만들었고, 실증 연구도 같은 방향을 보고함 (6.3)  
○ **이 모형은 시장 확대를 계산하지 못함.** 확률의 합이 1이라 총수요가 고정되어 있고, 개점 후 실제 재배분의 확인은 15장이 다룸 (6.4)  
○ **가장 거친 단위가 분석 전체의 해상도를 정함.** 후보지를 250m로 촘촘히 깔아도 수요가 행정동 단위면 같은 동 안의 후보는 거의 같은 값을 나눠 받음 (3.2, 9절)  
○ **절대량은 가정에 흔들리지만 순위는 흔들리지 않음.** β를 바꾸면 순증이 721명에서 1,478명으로 두 배가 되는데 최선 후보의 위치는 0m 이동이었음 (7.3)  
○ **퇴화 검사로 모형이 인구 지도의 되풀이가 아님을 확인함.** 배분과 낮 인구의 상관이 −0.051로 0에 가까웠음 (7.3)  
○ **거칠게 집계한 자료가 곧 안전한 자료는 아님.** 시공간 지점 네 개로 95%가 특정되고, 해상도를 낮춰도 유일성이 비례해 줄지 않음 (8.1)  
○ **이 사례에는 학습 모델이 하나도 쓰이지 않았음.** 반경 안 인구를 더하고 거리를 재는 계산은 답이 하나로 정해지며, 학습이 필요한 세 조건 어디에도 해당하지 않았기 때문임 (9절)  
...
2. 그 숫자가 왜 그렇게 나왔는가 — 전이율이나 순증이라면 6.3의 논리로 설명하라
