...
□ `lecture_practice/chapter10/results/10-1-living-soc-accessibility.log`의 실제 실행 결과부터 봄  
  ○ 격자 300개(20×15, 셀 500m), 총 인구 1,870,844명에 도서관 6·돌봄 10·공원 12·체육 8개소를 배치한 자료  
  ○ 이 숫자를 직접 만드는 명령은 4.5에 있음 — 여기서는 결과만 먼저 읽음  
...
○ 아래 표는 그 로그의 `[3층 정책]` 구간에 찍힌 상위 10 가운데 첫 세 행임  
...
□ 4.5절부터 10절까지는 설명과 실습을 나누지 않음 — 각 절 첫머리에 실행할 명령이 나오고 그 아래에 코드와 실제 실행 결과가 이어짐. 순서대로 따라가면 이 장의 모든 수치를 직접 만들게 됨  
...
| 순서 | 실행할 파일 | 하는 일 | 절 |
| ---: | --- | --- | ---: |
| 1 | `10-0-simdata-prep.py` | 10-1~10-3용 시뮬레이션 자료 생성 | 4.5 |
| 2 | `10-1-living-soc-accessibility.py` | 접근성·Gini·결손 점수 | 2, 4.3~4.5 |
| 3 | `10-2-civil-complaint-forecast.py` | 민원 시공간 예측과 예측구간 | 6~7 |
| 4 | `10-3-rdd-regulation-boundary.py` | 규제 경계 RDD와 네 검문 | 9 |
| 5 | `10-0b-delivery-data-prep.py` | 실데이터 스냅샷 생성 | 8 |
| 6 | `10-4-delivery-zone-optimization.py` | 권역 최적화와 원가 컷라인 | 8, 10.2 |

○ 2·3·4번은 1번이 만든 자료를 입력으로 받으므로 **1번을 먼저 실행**해야 하고, 6번은 5번이 만든 스냅샷을 읽으므로 **5번을 먼저 실행**해야 함  
○ 1~4번은 시뮬레이션 자료로 돌아가므로 인터넷이 필요 없음. **5번만 실제 공개 자료(소상공인시장진흥공단 상가정보, 서울시 생활인구)를 씀** — 두 포털이 자동 내려받기를 막아 두어 원자료 CSV 두 개를 직접 받아 저장소 안 `data/raw/` 아래에 두어야 하며, 내려받는 곳은 `lecture_practice/chapter10/README.md`에 적혀 있음  
○ 스냅샷이 이미 만들어져 있으면 5번을 건너뛰고 6번만 실행해도 같은 수치가 나옴. 공개 자료가 갱신되면 스냅샷이 달라지므로, 재현의 기준은 `data/delivery_snapshot_meta.json`에 적힌 취득일과 해시임  
○ 본문에 인용한 출력은 모두 `lecture_practice/chapter10/results/`의 실행 로그에서 그대로 가져온 것이며, 어느 파일에서 왔는지를 출력 블록 첫 줄에 적어 두었음 — 화면에 나온 값과 그 파일을 한 줄씩 대조하면 됨  

### 4.5 접근성 진단을 직접 실행한다

□ 첫 번째와 두 번째 코드를 실행함. 2절과 4.3·4.4의 숫자는 전부 이 명령의 출력임  
...
```python
# 주석에 붙인 출력의 출처: lecture_practice/chapter10/results/10-1-living-soc-accessibility.log
import numpy as np
import pandas as pd

# [1] 격자 인구와 생활SOC 시설 위치를 불러옴
rows, cols, pop, facilities = load_city()
# 출력: 분석 단위: 300개 격자(20×15, 셀 500m), 총 인구 1,870,844명
#       생활SOC: 도서관 6개소, 돌봄 10개소, 공원 12개소, 체육 8개소

# [2] 종류별 최근접 도보 시간 — 격자 간 유클리드 거리를 도보 속도(4km/h)로 나눔
for soc, idx in facilities.items():
    t = nearest_walk_time(rows, cols, idx)
    wmean = np.average(t, weights=pop)                 # 인구가중 평균
    pop_over = pop[t > WALK_15MIN].sum() / pop.sum()   # 15분 초과 인구 비율
# 출력: 도서관 : 인구가중 평균  24.0분 | 15분 초과 사각지대 인구 67.2% | Gini 0.324
#       돌봄  : 인구가중 평균  20.2분 | 15분 초과 사각지대 인구 54.1% | Gini 0.349
#       공원  : 인구가중 평균  16.5분 | 15분 초과 사각지대 인구 50.1% | Gini 0.307
#       체육  : 인구가중 평균  24.9분 | 15분 초과 사각지대 인구 64.3% | Gini 0.362

# [3] 네 종류를 평균한 종합 접근성
df["access_composite"] = df[access_cols].mean(axis=1)
comp = df["access_composite"].values
# 출력: 종합 접근성(4종 평균): 인구가중 평균 21.4분, Gini 0.246
#       종합 15분 초과 격자: 258개 (인구의 69.5%)

# [4] 결손 점수 = 임계값 초과분 × 인구 — 4.4의 식을 그대로 옮긴 두 줄
df["deficit_min"] = np.maximum(comp - WALK_15MIN, 0.0)
df["priority_score"] = df["deficit_min"] * df["pop"]
# 출력: 사각지대 후보 258개 중 상위 10 (아래 표)
```

```text
# 출처: lecture_practice/chapter10/results/10-1-living-soc-accessibility.log [3층 정책]
  사각지대 후보 258개 중 상위 10:
 cell  row  col    pop  access_composite  deficit_min  priority_score  priority_rank
   30    2    0 6321.0             45.12        30.12       190366.86              1
   22    1    7 5476.0             47.51        32.51       178014.40              2
   68    4    8 9701.0             30.43        15.43       149685.78              3
   39    2    9 5003.0             44.67        29.67       148432.57              4
   57    3   12 6517.0             37.55        22.55       146962.84              5
  118    7   13 9571.0             30.10        15.10       144514.15              6
  193   12   13 9220.0             30.54        15.54       143233.56              7
  164   10   14 6679.0             36.27        21.27       142064.00              8
  273   18    3 6784.0             35.64        20.64       140001.38              9
   35    2    5 6199.0             37.47        22.47       139293.83             10
```

□ 화면에 나온 값에서 읽을 것  
  ○ 인구가 가장 많은 격자가 1순위가 아님 — 상위 10에서 인구가 가장 많은 cell 68(9,701명)은 3위이고, 인구가 그 3분의 2인 cell 30(6,321명)이 1위임. 초과분이 30.12분 대 15.43분으로 두 배 벌어져 곱이 뒤집힘  
  ○ 사각지대 후보가 258개인데 이는 전체 300개 격자의 86%임. 임계값 15분을 넘는 격자가 다수이므로, 이 자료에서 "사각지대"는 예외가 아니라 기본값에 가까움 — 4.3의 임계값 하나만으로는 우선순위를 가를 수 없고 인구를 곱해야 하는 이유가 여기 있음  
  ○ 우선순위 점수의 단위는 분×명이므로 190,366.86이라는 값 자체는 읽지 않음. 읽는 것은 `priority_rank` 열임  
  ○ 이 코드는 학습 모델을 쓰지 않음. 시설 좌표와 인구가 주어지면 접근성이 하나로 정해지므로, 3.1 표의 첫 행이 말하는 "결정론적 계산"에 해당함  

○ 전체 300개 격자의 우선순위표는 `lecture_practice/chapter10/results/living_soc_priority.csv`에 저장됨  

## 5. 민원 자료는 문제량이 아니라 신고량을 잰다

### 5.1 세 값의 구분

○ 이 절에는 실행할 명령이 없음 — 6절에서 예측에 쓸 자료가 무엇을 재는 자료인지 먼저 판정하는 절차이며, 이 판정을 건너뛰면 6~7절의 예측 성능을 잘못 읽게 됨  
...
□ 세 번째 코드를 실행함. 6절과 7절의 모든 숫자가 이 한 번의 실행에서 나옴  

```bash
python lecture_practice/chapter10/code/10-2-civil-complaint-forecast.py
```

○ 출력은 네 덩어리로 나뉨 — `[1층 GIS]` 권역별 집계(6.1), `[2층 AI]` 피처 비교와 중요도(6.3), `[3층 정책]` 권역 우선순위(7.2), `[불확실성]` 예측구간과 신뢰등급(7절)  

```text
# 출처: lecture_practice/chapter10/results/10-2-civil-complaint-forecast.log [1층 GIS]
분석 단위: 256개 격자 × 40주(분석 12~39주), 권역 8개 집계, 개인 식별 불가
총 민원(분석기간): 126,117건, 주 평균 4504건
...
□ 화면에 나온 값에서 읽을 것  
  ○ 격자 256개 × 40주 가운데 앞 12주는 자료가 정상상태에 이르는 구간이라 분석에서 빼고 12~39주만 씀  
  ○ 권역 8개의 누적 민원은 권역 0(22,650건)부터 권역 3(10,460건)까지 두 배 넘게 벌어짐. 누적 상위 세 권역(0·1·7)은 7.2의 다음 주 예측 상위 세 권역과 그대로 일치하지만, 4~7위는 순서가 뒤바뀜 — 예측이 과거 누적을 그대로 베끼지는 않음  
  ○ 이 건수는 5절이 말한 접수량이지 문제량이 아님. 아래에서 재는 예측 성능도 "신고량을 얼마나 잘 맞혔는가"임  
...
○ 같은 실행의 `[2층 AI]` 구간이 그 비교를 찍음. 코드는 피처 목록만 바꿔 같은 함수를 두 번 부름  

```python
# 주석에 붙인 출력의 출처: lecture_practice/chapter10/results/10-2-civil-complaint-forecast.log
# [1] 두 피처 구성 — 공간 시차 두 개(neigh_t, neigh_t1)만 차이남
temporal_only = ["road", "self_t", "self_t1", "self_t2"]
spatiotemporal = temporal_only + ["neigh_t", "neigh_t1"]

# [2] 시간 블록 CV — 앞 70% 기간으로 학습하고 뒤 30% 시점에서만 평가함
...
# 출력: 시간 시차만(자기 과거)   R² =  0.753 | MAE = 6.510
#       +공간 시차(이웃 과거)    R² =  0.813 | MAE = 5.498
#         (개선  R² +0.059, MAE +1.012)

# [3] 피처 중요도 — 무엇이 예측을 끌고 가는지 확인함
imp = pd.Series(model_st.feature_importances_, index=spatiotemporal)
# 출력: self_t    0.589
#       neigh_t   0.364
#       self_t2   0.018
#       neigh_t1  0.017
...
○ 6.1에서 이미 실행한 `10-2-civil-complaint-forecast.py`의 `[불확실성]` 구간이 이 계산을 찍음. 명령을 다시 칠 필요는 없음  

```python
# 주석에 붙인 출력의 출처: lecture_practice/chapter10/results/10-2-civil-complaint-forecast.log
# [1] 평가 구간의 잔차 — 학습에 쓰이지 않은 뒤 30% 시점에서만 계산함
oof = model_st.predict(X_cal)
resid = np.abs(cal["target_next"].values - oof)

# [2] 위치별 예상 오차 규모 σ̂ — 절대잔차를 같은 피처로 회귀해 적합함
sigma_model = GradientBoostingRegressor(...).fit(X_cal, resid)
sigma_cal = np.clip(sigma_model.predict(X_cal), 1e-3, None)

# [3] 정규화 점수의 분위 q와 반폭 q × σ̂
scores = np.sort(resid / sigma_cal)
k = int(np.ceil((n_cal + 1) * (1 - alpha)))   # alpha = 0.10
q = scores[min(k, n_cal) - 1]
half_cal = q * sigma_cal
# 출력: 90% 예측구간 경험적 포함률(OOF) = 0.901  (목표 0.90)
#       구간 반폭(건): 중앙값 8.82, 범위 [5.41, 119.80]
```

□ 화면에 나온 값에서 읽을 것  
  ○ 경험적 포함률 0.901은 목표 0.90과 거의 같음 — 구간이 목표한 만큼만 덮고 있다는 확인임  
  ○ 반폭은 중앙값 8.82건인데 범위가 [5.41, 119.80]건으로 **22배 넘게 벌어짐.** 같은 자료 안에서도 맞히기 쉬운 격자와 어려운 격자의 오차 폭이 이만큼 다름  
  ○ 이 예제는 학습·보정·시험을 셋으로 나눈 정식 분할이 아니라 시간 블록 평가 구간의 잔차를 σ̂ 적합과 분위 산정에 함께 쓰는 교육용 근사임. 같은 잔차를 두 번 쓰는 탓에 포함률이 다소 낙관적으로 나올 수 있으며, 로그에도 같은 경고가 함께 찍힘  

### 7.2 예측 상위가 저신뢰가 되는 구조

○ 격자별 반폭을 권역 단위로 쓰려면 등급으로 바꿈. 권역 평균 반폭을 구하고, 전체 권역에서 3분위로 나눠 높음·중간·낮음을 매김  

```text
# 출처: lecture_practice/chapter10/results/10-2-civil-complaint-forecast.log [불확실성]
...
□ 화면에 나온 값에서 읽을 것  
  ○ `mean_pi_half`가 권역 평균 반폭이고 `low_conf_cells`는 그 권역에서 반폭이 상위 3분의 1에 든 격자 수임 — 권역 0·1은 각각 20개로 가장 많음  
  ○ 권역 4는 평균 반폭이 8.96건으로 권역 7(8.89건)과 거의 같은데 등급은 낮음임. 등급이 반폭의 3분위로 갈리므로 경계 부근에서는 작은 차이가 등급을 가름 — 등급은 순서를 거칠게 나눈 표시일 뿐 절대 기준이 아님  
...
□ **예측값이 가장 큰 두 권역(0, 1)이 동시에 신뢰등급 낮음임.** 우연이 아니라 자료 구조에서 나옴  
  ...
  ○ 이 성질을 알면 신뢰등급을 예측 순위와 독립된 정보로 읽지 않고, 순위와 등급의 조합으로 조치를 나누게 됨. 그 규칙은 10절에서 정리함  
...
□ 다섯 번째와 여섯 번째 코드를 실행함. 8절의 모든 표와 10.2의 행정동 목록이 이 한 번의 실행에서 나옴  

```bash
python lecture_practice/chapter10/code/10-0b-delivery-data-prep.py
python lecture_practice/chapter10/code/10-4-delivery-zone-optimization.py
```

○ 출력은 일곱 덩어리로 나뉨 — `[1]` 최적화(8.2), `[2]` 원가 컷라인(8.4), `[3]` 공급 제약(8.3), `[4]` 중첩, `[4-2]` 대조군, `[5]` 퇴화 검사, `[6]` 민감도  
○ 자료는 서울 종로구·서대문구·은평구의 활동 격자 245개(셀 500m), 행정동 47개이며, 평시 생활인구 1,121,348명이 피크에 1,030,742명으로 바뀜  

```text
# 출처: lecture_practice/chapter10/results/10-4-delivery-zone-optimization.log [1]
[1] 최적화 — 목적함수가 거점 배치를 바꾼다 (표 10.7)
  후보 거점 40개(음식점 밀집 상위), 개설 10개, 해법: MILP(HiGHS)
  해                        평균 도달(분)      T분 커버 수요      커버 인구    p-median과 공통  비고
  p-median                     2.96        86.6%      86.6%          10/10
  최대커버(T=5분)                   3.13        88.8%      88.8%           7/10
  용량제약 p-median                2.96        86.6%      86.6%          10/10  미배차 0.0%
  휴리스틱(정점치환)                   2.99        85.5%      85.5%           8/10

  휴리스틱 목적함수 격차: +0.738% (정확해 대비. Teitz–Bart는 전역 최적을 보장하지 않는다)
  해 안정성 — p=(8, 10, 12): 공통 거점 수 6, 후보집합 교체 시 공통 7/10
```
...
□ 화면에 나온 값에서 읽을 것  
  ○ 마지막 줄의 "해 안정성"은 개설 개수를 8·10·12로 바꾸면 공통 거점이 6곳으로 줄고, 후보 집합을 음식점 밀집 대신 전체 POI 밀집으로 바꾸면 7/10만 겹친다는 뜻임 — **후보 집합과 개설 개수를 밝히지 않은 배치안은 검증할 수 없음**  
  ○ 용량제약을 걸어도 미배차가 0.0%이고 배치가 p-median과 같음(10/10). 평시 수요에서는 거점 용량이 아직 묶이지 않는다는 뜻이며, 묶이기 시작하는 지점이 8.3의 피크 시나리오임  
...
### 8.3 용량 제약이 걸리면 원가가 오른다

□ 앞의 두 문제는 거점이 담당할 수 있는 양에 상한이 없다고 가정함. 실제로는 거점마다 인력 상한이 있고, 상한을 넘는 수요는 원가를 밀어 올림  

```text
# 출처: lecture_practice/chapter10/results/10-4-delivery-zone-optimization.log [3]
  거점당 라이더 평시 25명(시간당 1,500분) → 피크 50명(주문은 2.1배)
  시나리오                           평균 도달     평균 이용률     배차 대기     유효 임률     미배차       컷라인 밖 인구
  평시 · 용량 무제약                    2.96분          —         —       300    0.0%          6.6%
  피크 · 용량 무제약                    4.21분          —         —       300    0.0%         10.4%
  피크 · 용량제약+할증                   4.37분       0.93     15.2분       453    0.0%         54.4%

  평시 → 피크(용량·할증 반영): 컷라인 밖 인구 +47.8%p (6.6% → 54.4%)
  그중 수요 이동·혼잡만의 몫은 10.4%이고, 용량 제약·할증이 더한 몫이 +44.0%p다.
```
...
□ 화면에 나온 값에서 읽을 것  
  ...
  ○ 피크에 컷라인 밖 인구가 6.6%에서 54.4%로 뛰는데, 용량을 걸지 않은 피크에서는 10.4%에 그침. 증가분 47.8%p 가운데 44.0%p는 용량 제약과 임률 할증이 만든 것임 — **수요가 옮겨 가고 길이 막히는 것보다 인력 상한이 경계를 더 크게 당김**  
  ○ 미배차는 세 시나리오 모두 0.0%임. 주문을 못 받은 것이 아니라 받는 원가가 올라 남지 않게 된 것이며, 두 가지는 다른 문제임  

### 8.4 어디까지 배달하면 남는가 — 원가 컷라인
...
○ 같은 실행의 `[2]` 구간이 이 계산을 격자 245개에 적용한 결과임  

```python
# 주석에 붙인 출력의 출처: lecture_practice/chapter10/results/10-4-delivery-zone-optimization.log
# [1] 점유시간 = 고정시간 + 왕복 주행시간 ÷ 묶음계수 — 배차 대기는 넣지 않음
...
margin = fee - cost
# 출력: 건당 수수료 4,500원, 임률 300원/분, 고정 6분, 변동비 400원  ← 전부 가정값
#       건당 기여이익 분위(원): p10 -2,068 | p25 -487 | 중앙 843 | p75 1,443 | p90 1,748

# [3] 기여이익을 0으로 놓고 편도 주행시간에 대해 풀면 손익분기 등시선
slack = fee - VAR_COST - wage * FIXED_MIN
tau = slack / wage * bundle / 2.0
# 출력: 손익분기 편도 주행시간: 묶음 없음(계수 1.0) 3.8분 → 최대 묶음(계수 2.0) 7.7분
```

```text
# 출처: lecture_practice/chapter10/results/10-4-delivery-zone-optimization.log [2]
  컷라인 밖: 격자 83개/245 (33.9%), 인구 74,047명 (6.6%), 주문 6.6%
  컷라인 밖 격자의 특성: 평균 도달 7.9분 (안쪽 3.0분), 주문밀도 1.07건/시간 (안쪽 7.76), 묶음계수 1.18 (안쪽 1.72)
```
...
□ 실행 결과에서 컷라인 밖은 격자 83개/245(33.9%), 인구 74,047명(6.6%), 주문 6.6%임  
  ...
  ○ 건당 기여이익의 p10이 −2,068원인데 중앙값은 +843원임. 평균 하나로 보고하면 크게 밑지는 구역이 이익 나는 구역에 묻힘  
  ○ 수수료·임률·고정시간·변동비 네 값은 모두 가정값임. 로그가 같은 줄에 "전부 가정값"이라고 찍는 이유이며, 이 절의 절대 금액은 가정을 바꾸면 함께 움직임  
  ○ 같은 실행의 `[6]` 민감도 구간이 그 폭을 잼 — 임률을 ±30%, 수수료를 ±20%, 묶음계수를 1.0~2.5로 흔들면 컷라인 밖 인구 비율이 **2.0%에서 30.1%까지** 움직임(기준 6.6%). 흔들리지 않는 것은 순서이며, 어느 설정에서도 먼저 잘리는 곳은 멀고 성긴 격자임  
...
### 9.3 추정치를 믿기 전에 통과해야 하는 네 검문

□ 네 번째 코드를 실행함. 네 검문의 출력이 모두 이 한 번의 실행에서 나옴  

```bash
python lecture_practice/chapter10/code/10-3-rdd-regulation-boundary.py
```

○ 출력은 네 덩어리로 나뉨 — `[1]` 대역폭 민감도, `[2]` 전역 다항식과의 대비, `[3]` McCrary 밀도 검정, `[4]` placebo 검정. 아래 ①~④가 그 순서와 그대로 대응함  

**① 대역폭 민감도** — 좁게 잡으면 조건이 비슷한 거래만 비교하지만 표본이 적어 흔들리고, 넓게 잡으면 표본이 늘지만 조건이 다른 거래가 섞여 들어옴

```text
# 출처: lecture_practice/chapter10/results/10-3-rdd-regulation-boundary.log [1]
참 규제효과 τ(설계값) = -60.0 만원/㎡  (규제지역 가격 하락)
표본 N = 2000,  경계 데이터반경 = ±3.0km,  기준가 = 1200 만원/㎡

   대역폭 h(km) |        τ̂ |      SE |               95% CI |    유효N
        0.50 |    -57.86 |   14.01 | [  -85.9,   -30.7] |    335
        0.75 |    -56.71 |   11.48 | [  -80.2,   -34.7] |    501
        1.00 |    -54.92 |    9.91 | [  -74.5,   -37.3] |    655  ← 주 대역폭
        1.50 |    -58.82 |    7.56 | [  -74.9,   -45.5] |    963
        2.00 |    -68.37 |    6.77 | [  -82.7,   -55.6] |   1315
```
...
□ 대역폭을 넓히면 표준오차는 줄어드는데(14.01 → 6.77), **참값과의 차는 h = 2.00에서 오히려 가장 큼**(−8.4)  
  ...
  ○ 유효N 열이 그 대가를 드러냄 — h를 0.50에서 2.00으로 넓히면 표본이 335개에서 1,315개로 네 배 늘고, 늘어난 980개는 모두 경계에서 먼 거래임  
  ○ 95% 신뢰구간은 다섯 대역폭 모두 참값 −60.0을 포함함. 점추정 하나가 아니라 구간과 대역폭 표를 함께 봐야 이 판정을 할 수 있음  
...
○ ②~④의 출력은 같은 실행의 `[2]`·`[3]`·`[4]` 구간에 이어서 찍힘  

```text
# 출처: lecture_practice/chapter10/results/10-3-rdd-regulation-boundary.log [2][3][4]
  전역 2차 다항식 τ̂ =   -63.25 만원/㎡  (참값 대비 편향 -3.25)
  전역 4차 다항식 τ̂ =   -53.61 만원/㎡  (참값 대비 편향 +6.39)
  전역 6차 다항식 τ̂ =   -60.38 만원/㎡  (참값 대비 편향 -0.38)
  지역선형(h=1.0) τ̂ =   -54.92 만원/㎡  (참값 대비 편향 +5.08)

  정상 표본   : θ(log밀도차) = -0.260, SE = 0.173, z = -1.51  → 조작 증거 없음
  정렬 표본   : θ(log밀도차) = +1.232, SE = 0.149, z = +8.29  → 밀도 불연속 탐지

  placebo c=-1.0 : τ̂ =   -1.18 만원/㎡, 95% CI [-21.3, 18.8]  → 0 포함
  placebo c=+1.0 : τ̂ =    1.36 만원/㎡, 95% CI [-19.0, 23.4]  → 0 포함
```

□ 화면에 나온 값에서 읽을 것  
  ○ 전역 6차 다항식의 편향이 −0.38로 가장 작지만, 이것을 근거로 6차를 고를 수는 없음 — **참값을 모르는 실제 자료에서는 어느 차수의 편향이 작은지 알 수 없기 때문임.** 참값을 심은 이 표본에서만 채점이 가능함  
  ○ McCrary 검정의 정상 표본과 정렬 표본은 같은 코드를 서로 다른 자료에 돌린 것임. 정렬 표본은 매수자가 규제를 피해 경계 바깥으로 몰린 상황을 일부러 만든 대조군이며, z = +8.29로 탐지됨  
  ○ 두 placebo 추정치의 신뢰구간 폭이 ±20 안팎임 — "0을 포함한다"는 판정은 효과가 없다는 증명이 아니라 이 폭 안에서는 구분되지 않는다는 뜻임  
...
### 10.1 순위와 신뢰등급을 함께 읽는다

○ 7.2에서 확인했듯 예측 상위 권역이 동시에 저신뢰가 되는 구조가 있으므로, **순위만으로 조치를 정하면 오차 폭이 큰 곳에 자원을 확정 배치하게 됨**  
...
### 10.2 격자와 행정동, 어느 단위로 보고할까

□ 격자에서 계산한 값을 행정동으로 집계해 보고하면 3.2에서 본 손실이 생김. 8.4의 배달 사례가 그 대비를 보여 줌  
...
○ 행정동으로 집계하면 어느 구역인지가 드러남. 8.2에서 실행한 `10-4`의 `[2]` 구간이 그 목록을 함께 찍음  

```text
# 출처: lecture_practice/chapter10/results/10-4-delivery-zone-optimization.log [2]
  컷라인 밖 격자를 가진 행정동 25개 (전체 47개). 상위 5:
    은평구 진관동: 22/26개 셀
    종로구 평창동: 14/14개 셀
    종로구 부암동: 10/10개 셀
    은평구 수색동: 5/7개 셀
...
□ 화면에 나온 값에서 읽을 것  
  ○ 종로구 평창동과 부암동은 셀 전부가 컷라인 밖임 — 행정동 하나가 통째로 빠지는 경우와 일부만 빠지는 경우는 협의의 성격이 다름  
  ○ 서대문구 연희동은 11개 가운데 4개뿐임. 같은 "컷라인 밖 행정동"이라도 동 안에서 어느 셀이 빠졌는지까지 보아야 대응이 정해짐  
  ○ 격자 단위로는 협의 대상을 특정할 수 없지만 행정동 단위로는 담당 부서와 대응함. 다만 3.2에서 본 대로 행정동 평균 하나만 보고하면 이 안쪽 차이가 사라짐  
  ○ 격자별 판정은 `lecture_practice/chapter10/results/delivery_zone_cells.csv`에, 시나리오별 요약은 `delivery_zone_summary.csv`에 저장됨  
...
○ **우선순위는 접근성과 인구의 곱임.** 실습에서 인구 1.5배인 격자가 초과분이 절반이라 순위가 더 낮았고, 단위가 다르므로 순위 용도로만 씀 (2절, 4.4, 4.5)  
○ **민원 건수는 문제량이 아니라 신고량을 잼.** 접수량으로만 자원을 배분하면 신고가 활발한 구역에 쏠리고 다음 회차 접수량이 다시 오르는 되먹임이 생김 (5.1, 5.2)  
○ **시계열을 무작위로 분할하면 미래로 과거를 맞히는 구성이 됨.** 코드는 정상 종료하고 성능 지표는 오히려 좋아짐 (6.2)  
○ **공간 시차는 넣을 근거를 비교로 확인함.** 실습에서 R² 0.753→0.813, MAE 6.510→5.498로 두 지표가 같은 방향으로 움직였음 (6.3)  
○ **예측 상위 권역이 동시에 저신뢰가 되는 것은 자료 구조의 성질임.** 값의 크기와 변동이 함께 커지기 때문이며, 순위와 등급의 조합으로 조치를 나눔 (7.2, 10.1)  
○ **목적함수가 배치를 정함.** 같은 후보 40곳에서 p-median과 최대커버가 열 곳 중 세 곳을 다르게 골랐고, 임계 시간 T를 정하는 행위 자체가 이미 정책 판단임 (8.2)  
○ **원가 컷라인의 절대 수준은 가정에 얹혀 있고 순서만 안정적임.** 임률·수수료·묶음계수를 흔들면 컷라인 밖 인구가 2.0~30.1%까지 움직이지만, 먼저 잘리는 곳은 언제나 멀고 성긴 격자임 (8.4)  
○ **RDD는 경계 근방에서만 성립함.** 네 검문(대역폭·다항식 차수·McCrary·placebo)을 통과해도 복합처치·내생적 지정·국소성 때문에 전국으로 외삽할 수 없음 (9.3, 9.4)  
...
○ **3번이 이 과제의 핵심**. 1·2번은 결과를 들여다보면 답이 나오지만 3번은 9.4의 판단표를 근거로 스스로 답해야 함  
