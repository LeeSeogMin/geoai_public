...
> - 예시를 위해 지어낸 숫자는 없음. 다만 9절의 비용·투자 파라미터는 실행값이 아니라 **가정값**이며, 본문에서 그때마다 표시함
...
□ 시장 40개, 같은 자료에서 실험 설계만 바꿔 봄  
  ○ **단순 무작위 배정 + 사후 비교**: 최소 탐지 가능 효과(MDE)가 로그 스케일 계수로 `0.2518`이고, 백분율로 환산하면 약 28.63%임. **참 효과는 약 5.02%이므로 이 설계로는 실험을 시작할 이유가 없음**  
  ○ **짝지음 무작위 배정 + 이중차분**: MDE가 로그 스케일 계수로 `0.0386`이고, 백분율로는 약 3.94%임. 참 효과보다 작아 잴 수 있음  
  ○ 결과변수가 로그 매출이므로 **로그 스케일 계수를 백분율 효과로 읽으려면 `exp(계수) − 1`로 환산함**. 계수 `0.2518`을 그대로 25.18%로 읽으면 안 됨(6.1 참조)  
...
○ **통계 진단(사전 적합도, placebo 검정)을 다 통과한 뒤에도 이 편차가 남음.** 8절이 이 문제를 다룸  
...
□ 5절부터 9절까지는 설명과 실습을 나누지 않음 — 각 절 첫머리에 실행할 명령이 나오고 그 아래에 실제 실행 결과가 이어짐. 순서대로 따라가면 이 장의 모든 수치를 직접 만들게 됨  
...
```
  ...
  ○ 15장이 쓰는 패키지만 따로 보려면 `lecture_practice/chapter15/code/requirements.txt`에 적혀 있으나, 설치는 위 공통 파일 하나로 끝남  

□ 이 장에서 실행할 코드는 네 개이며 순서가 정해져 있음  

| 순서 | 실행할 파일 | 하는 일 | 절 |
| ---: | --- | --- | ---: |
| 1 | `15-0-simdata-prep.py` | 시장 40개 합성 패널 생성 | 5 |
| 2 | `15-1-geo-experiment-power.py` | 지역 실험 설계 5종 비교, 검정력 계산 | 5.2~6.4, 8.1 |
| 3 | `15-2-market-synthetic-control.py` | 시장 단위 합성통제, 진단 | 7, 8.2~8.3 |
| 4 | `15-3-decision-economics.py` | 임계비·순현재가치·손익분기 | 8.4, 9 |

○ 2·3번은 1번이 만든 패널 파일(`data/geoexp_panel.parquet` 등)을 입력으로 받고, 4번은 3번이 남긴 `results/15-2-estimate.json`을 읽음 — **번호 순서대로 실행**해야 함  
○ 인터넷도 GPU도 필요 없음. 네 개를 합쳐도 실행 시간이 약 10초임  
○ **합성 자료를 쓴 이유는 참 효과를 알아야 추정량을 채점할 수 있기 때문임.** 실제 자료에는 정답이 없어 "계단을 올려도 참값에 안 닿는다"는 확인 자체가 불가능함  
○ 9절의 비용·투자 파라미터는 전부 **가정값**이며, 결론은 절대값이 아니라 민감도임  
○ 본문에 인용한 출력은 모두 `lecture_practice/chapter15/results/`의 실행 로그에서 그대로 가져온 것이며, 어느 파일에서 왔는지를 출력 블록 첫 줄에 적어 두었음  

## 5. 실험 설계를 짠다

□ 먼저 합성 패널을 만듦. 5절부터 8절까지의 모든 수치가 이 자료 위에서 나옴  

```bash
python lecture_practice/chapter15/code/15-0-simdata-prep.py
```

```text
# 출처: lecture_practice/chapter15/results/15-0-simdata-prep.log
시장 = 40개 (4×10 격자, rook 인접)
군집 = 10개 (2×2 블록) — 군집 무작위화 설계용
기간 = 104주 (사전 52 / 사후 52)
참 처치효과 = 로그 +0.049 (+5.02%)
인접 대조 파급 = 로그 -0.015 (-1.49%)
관측 잡음 SD = 0.035 (로그 스케일)

인접 이웃 수: 최소 2 / 중앙값 3 / 최대 4
인접 시장 쌍 = 66개

성장률의 공간 자기상관(인접 쌍 평균 곱) = +0.779

사전 관측 성장률 상위 5개 시장: [5, 15, 4, 16, 6]
```

□ 화면에 나온 값에서 읽을 것  
  ○ 참 효과 로그 `+0.049`(+5.02%)와 파급 로그 `−0.015`는 코드가 심은 값임 — 참값을 알기 때문에 뒤의 추정치를 채점할 수 있음  
  ○ 공간 자기상관 `+0.779`은 가까운 시장끼리 성장률이 닮는다는 뜻이며, 5.2의 짝지음 부작용과 7절의 기증 풀 문제가 모두 이 값에서 나옴  
  ○ 관측 잡음 SD `0.035`는 7.3에서 사전 적합도를 견줄 기준선이 됨  
  ○ 이 코드는 처치를 얹지 않은 기저 패널만 만듦 — 처치효과와 파급은 뒤의 분석 코드가 배정에 따라 얹음. 그래야 같은 기저 위에서 1,000회 배정을 반복하고 대조군 세계를 만들 수 있음  

### 5.1 무작위화 단위를 개인이 아니라 시장으로
...
□ 두 번째 코드를 실행함. 설계 다섯 종을 각각 1,000회 배정해 편향·표준편차·기각률·MDE를 잼. 5.2절부터 6.4절까지, 그리고 8.1절의 숫자가 모두 이 한 번의 실행에서 나옴  

```bash
python lecture_practice/chapter15/code/15-1-geo-experiment-power.py
```

```text
# 출처: lecture_practice/chapter15/results/15-1-geo-experiment-power.log (세계A 기준: 효과+파급)
[짝지음] 사전 52주 궤적 거리로 20개 쌍 구성
  이 중 지리적으로 인접한 쌍 = 11개 / 20개

설계                           평균추정       편향     경험SD     평균SE     기각률    ±MC      MDE    단위     노출률
설계1 단순+사후비교                0.0657   0.0167   0.0881   0.0875   0.100  0.009   0.2518  40.0   0.904
설계2 단순+DID                 0.0617   0.0127   0.0379   0.0378   0.358  0.015   0.1087  40.0   0.904
설계3 짝지음+DID                0.0639   0.0149   0.0129   0.0131   1.000  0.000   0.0386  40.0   0.959
설계4 짝지음+오염제외               0.0465  -0.0025   0.0586   0.0414   0.167  0.028   0.3515   4.6   0.000
설계5 군집무작위+DID              0.0585   0.0095   0.0744   0.0758   0.080  0.009   0.2453  10.0   0.617
```

□ 화면에 나온 값에서 읽을 것 — 이 표 하나가 3.3절의 단위 교환, 4.1절의 오염 편향, 4.4절의 사후 필터, 5.2·5.3절의 설계 비교를 모두 담고 있음  
  ○ `편향` 열은 참값 `0.0490`에서 얼마나 벗어났는지이고, `노출률`은 대조군 가운데 처치 시장과 인접한 비율임  
  ○ 설계4의 `단위 4.6`은 1,000회 평균이며 정수가 아님 — 몇 쌍이 남는지가 배정마다 달라지는 확률변수라는 4.4절의 지적이 이 자리에 숫자로 찍혀 있음  
  ○ `MDE` 열은 로그 스케일 계수임. 백분율 효과로 읽으려면 `exp(계수) − 1`로 환산함(6.1)  
  ○ `±MC` 열은 기각률의 몬테카를로 표준오차이며 구간 반폭이 아님. 8.1절의 판정 블록은 같은 값의 두 배를 95% 구간 반폭으로 찍으므로 두 자리의 폭이 다름(8.1)  
...
□ 앞의 항은 우연을 배제하기 위해 넘어야 하는 문턱임(양측 5%에서 1.96). 뒤의 항은 그 문턱을 목표 확률로 넘기기 위한 여유임(검정력 80%에서 0.84)  
  ...
  ○ **이 식이 말하는 것은 표본 크기가 아니라 표준오차임.** 표본을 늘리는 것은 표준오차를 줄이는 여러 방법 가운데 하나일 뿐이며, 5.2의 짝지음처럼 설계를 바꿔 같은 표본에서 표준오차를 줄이는 경로도 있음  

□ **환산 규약.** 이 장의 결과변수는 로그 매출이므로 MDE도 로그 스케일 계수로 나옴. **백분율 효과로 읽으려면 `exp(계수) − 1`로 환산함**  
  ○ 계수 `0.2518`은 28.63%이지 25.18%가 아님. 계수가 작으면 두 값이 가까워지지만(계수 `0.0386`은 3.94%), 계수가 커질수록 벌어짐  
  ○ 코드도 같은 방식을 씀 — `15-1-geo-experiment-power.py`의 `MDE(%)` 열은 `np.expm1(MDE) * 100`으로 계산함. **이 장에 적힌 MDE 백분율은 전부 이 환산을 거친 값이며, 로그 스케일 계수를 그대로 인용하는 자리는 "로그 스케일 계수"로 밝혀 둠**  

### 6.2 표준오차를 두 경로로 얻고 대조한다
...
□ 실측이 이 판정의 예임. 5.2절에 붙인 출력 표의 `경험SD`와 `평균SE` 두 열을 나란히 읽으면 됨. 설계 1·2·3에서는 두 값이 거의 같음(`0.0881` 대 `0.0875`, `0.0379` 대 `0.0378`, `0.0129` 대 `0.0131`)  
...
### 6.3 설계만 바꿔 MDE가 일곱 배 달라진다

| 설계 | 표준오차 | MDE(로그 스케일 계수) | MDE(백분율, exp−1) | 참 효과 5.02%를 잴 수 있는가 |
| --- | --: | --: | --: | --- |
| 단순 무작위 + 사후 비교 | 0.0875 | 0.245 | 27.76% | 잴 수 없음 |
| 짝지음 + 이중차분 | 0.0131 | 0.0367 | 3.74% | 잴 수 있음 |

○ **같은 시장 40개, 같은 자료에서 설계만 바꿔 MDE가 일곱 배 가까이 달라짐**  
○ 위 표는 표준오차 두 개만으로 손으로 수행한 계산이며, 반복 배정으로 직접 측정한 실측 MDE는 로그 스케일 계수로 `0.2518`과 `0.0386`, 백분율로 환산하면 `28.63%`와 `3.94%`임  
○ **백분율 열은 6.1의 환산 규약대로 `exp(계수) − 1`로 얻은 값임.** 계수를 그대로 퍼센트로 읽으면 큰 계수 쪽에서 3%포인트 가까이 어긋남  

### 6.4 시장 수를 늘려도 MDE가 단조롭게 줄지 않는다

□ 같은 실행(`15-1-geo-experiment-power.py`)이 설계 3을 기준으로 시장 수를 10·20·30·40으로 바꿔 가며 MDE와 검정력을 다시 잼  

```text
# 출처: lecture_practice/chapter15/results/15-1-geo-experiment-power.log (시장 수와 MDE)
    시장 수          파급     MDE(로그)    MDE(%)       검정력
      10       파급 있음      0.0972     10.21     0.242
      10       파급 없음      0.0973     10.22     0.242
      20       파급 있음      0.0538      5.53     0.910
      20       파급 없음      0.0536      5.51     0.644
      30       파급 있음      0.0566      5.83     0.888
      30       파급 없음      0.0567      5.84     0.604
      40       파급 있음      0.0385      3.93     1.000
      40       파급 없음      0.0384      3.92     0.970
```

○ 위 출력을 시장 수별로 한 줄씩 다시 묶으면 이렇게 됨(MDE 열은 로그 계수를 `exp−1`로 환산한 백분율이며, 로그의 `MDE(%)` 열을 그대로 옮긴 값임)  
...
□ 세 번째 코드를 실행함. 시장 하나를 내생적으로 골라 합성통제를 돌리고 진단까지 함께 찍음. 7절과 8.2·8.3절의 숫자가 모두 이 한 번의 실행에서 나옴  

```bash
python lecture_practice/chapter15/code/15-2-market-synthetic-control.py
```

```text
# 출처: lecture_practice/chapter15/results/15-2-market-synthetic-control.log (세계A 기준: 내생선택+효과+파급)
처치 시장 = 35번 (격자 3,5) · 진출 후보 20개 중 선택 · 인접 시장 [25, 34, 36]
사전 관측 성장률 = +0.00247 (전체 8위)

추정 계단                     추정 ATT     참값 대비    사전RMSPE      격차기울기    기증수     가중집중     인접가중
① 사후 단순 비교                0.0884    0.0394          —          —     39        —        —
② DID (전체 기증 풀)           0.1317    0.0827          —          —     39        —        —
③ SCM (전체 기증 풀)           0.1129    0.0639     0.0376   -0.00009     39    0.172    0.276
④ SCM (인접 제외 풀)           0.1092    0.0602     0.0377   +0.00001     36    0.241    0.000
```

□ 화면에 나온 값에서 읽을 것  
  ○ `참값 대비` 열이 참값 로그 `+0.0490`에서 벗어난 크기임 — 네 칸 가운데 가장 작은 것이 첫 칸(`+0.0394`)이고, 가장 정교한 넷째 칸도 `+0.0602`가 남음  
  ○ `인접가중` 열은 파급을 받은 인접 시장에 합성 대조가 준 가중치의 합임. 전체 풀에서는 `0.276`이고 인접을 빼면 정의상 `0.000`이 됨 — 이 값이 8.2절 검산의 입력이 됨  
  ○ 처치 시장의 사전 성장률이 전체 8위라는 표시가 내생 선택의 흔적임. 무작위로 고른 시장이 아니라 성장하던 시장을 골랐다는 뜻이며, 8.3절이 이 선택을 층으로 벗김  
...
> **주의.** 사전 적합도가 좋다는 사실은 옳은 반사실을 만들었다는 뜻이 아님. **세 기준을 모두 통과하고 placebo까지 통과한 추정치가 참값의 2배를 넘게 나온 경우가 있음**(7.5). 통계 진단을 다 통과한 뒤에도 추정치를 결정 단위로 환산해 경제적으로 성립하는 값인지 검산해야 함.
...
□ 같은 실행이 볼록껍질 점검과 기증 풀 교환을 나란히 찍음  

```text
# 출처: lecture_practice/chapter15/results/15-2-market-synthetic-control.log
  [볼록껍질 점검] 사전 추세 기울기
    처치 시장 +0.002471 · 기증 풀 범위 [-0.012222, +0.004552]
    처치 시장보다 기울기가 큰 기증 단위 = 7개 / 39개

기증 풀      : 39개 → 36개 (인접 3개 제외)
사전 RMSPE   : 0.0376 → 0.0377 (+0.2%)
가중 집중도  : 0.172 → 0.241 (1에 가까우면 소수 단위에 몰렸다)
인접 시장 가중 합: 0.276 → 0.000
추정 ATT     : +0.1129 → +0.1092 (참값 +0.0490)

[도넛이 평균적으로 이득인가] 후보 20개 전수 반복 결과
  전체 풀   평균 절대편향 0.0250 · 사전 RMSPE 평균 0.0454
  인접 제외 평균 절대편향 0.0389 · 사전 RMSPE 평균 0.0654
  인접 가중 합 평균 0.520  → 오염 기대량 +0.0078
  판정: 인접 제외가 평균적으로 **손해**다.
```

□ 화면에 나온 값에서 읽을 것  
  ○ 이 한 사례만 보면 인접 제외가 추정치를 참값 쪽으로 조금 당김(`+0.1129 → +0.1092`). 그런데 후보 20개를 전수로 돌리면 평균 절대편향이 오히려 커짐 — **사례 하나로 처방의 우열을 정할 수 없다는 것이 이 대조의 내용임**  
  ○ 도넛을 쓸지 말지는 관행이 아니라 두 값이 정함 — 인접 기증 단위의 가중치 합과 파급의 크기. 둘을 먼저 재고 판정함  
...
```text
# 출처: lecture_practice/chapter15/results/15-2-market-synthetic-control.log (⑤ placebo 순열추론)
처치 시장 사후/사전 RMSPE 비율 = 3.051
기증 36개의 가짜 처치 비율: 중앙값 1.208 · 최대 1.796
유사 p값 = 0.027  (유의, 명목 0.05 기준)
```
...
○ 세 세계를 만드는 것은 5.2절에서 실행한 `15-1-geo-experiment-power.py` 한 번이며, 효과 0 세계의 판정 블록은 이렇게 찍힘  

```text
# 출처: lecture_practice/chapter15/results/15-1-geo-experiment-power.log (대조군 2 — 효과·파급이 없는 세계)
  설계1 단순+사후비교              0.044 ± 0.013  가짜 효과 +0.0031  → 명목 수준 포함
  설계2 단순+DID               0.045 ± 0.013  가짜 효과 -0.0009  → 명목 수준 포함
  설계3 짝지음+DID              0.052 ± 0.014  가짜 효과 +0.0005  → 명목 수준 포함
  설계4 짝지음+오염제외             0.233 ± 0.063  가짜 효과 -0.0025  → **부풀림 — 없는 효과를 유의하다고 말한다**
  설계5 군집무작위+DID            0.024 ± 0.010  가짜 효과 +0.0002  → 보수적 — 기각을 덜 한다(검정력 손실)
  판정: 부풀림 설계 = ['설계4 짝지음+오염제외']
```

  ○ 실측: p=0.052, R=1,000이면 표준오차는 0.007이고 95% 구간 반폭은 그 두 배인 0.014임. [0.038, 0.066]에 명목 5%가 포함되므로 절차가 정상 작동함  
  ○ **초과와 미달을 다르게 읽음.** 초과는 효과가 없는데 있다고 말하는 절차이므로 결론을 뒤집는 실패임(사후 필터 설계: `0.233`, 반폭 `±0.063`). 미달은 있는 효과를 놓쳐 검정력을 깎는 실패임(군집 설계: `0.024`, 반폭 `±0.010`)  
  ○ 실데이터에서는 세계를 만들 수 없으므로 사전기간의 가짜 처치 시점 검정이나 8.2의 항등식 검산으로 대체함  

□ **같은 `±` 기호가 로그 두 자리에서 서로 다른 양을 가리킴.** 코드를 확인한 결과 계산은 맞고 라벨만 부족했음  
  ○ 세계별 요약표의 `±MC` 열은 몬테카를로 **표준오차 자체**임 — `sqrt(p(1−p)/R)`로 계산함. 설계 4의 `0.233`에서 유효 반복이 180회뿐이므로 `0.032`가 나옴  
  ○ 위 판정 블록의 `±`는 같은 표준오차의 **두 배**, 즉 95% 신뢰구간 반폭임. 그래서 `0.063`으로 정확히 두 배임(설계 3의 `0.007`과 `0.014`도 같은 관계)  
  ○ **두 값은 어긋난 것이 아니라 1배와 2배임.** 구간 판정에 쓰는 값은 반폭 `0.063`이며, [0.170, 0.296]이 명목 5%를 한참 넘으므로 부풀림 판정은 그대로임  
  ○ 결론을 구간으로 말할 때는 표준오차를 쓸지 그 두 배를 쓸지 먼저 정하고, **숫자 옆에 무엇인지 적음**. 같은 기호로 다른 양을 찍으면 읽는 쪽이 폭을 두 배 또는 절반으로 잘못 읽음  
  ○ 저장된 로그는 이 라벨을 고치기 전에 실행한 기록임 — 로그에는 열 이름이 `±MC`로, 판정 블록에는 설명 없는 `±`로 남아 있음. 지금 코드는 요약표 열을 `±1SE`로, 판정 블록을 `± …(95% CI 반폭)`으로 찍음  

### 8.2 오염량이 계산과 맞는지 검산한다
...
○ 7절에서 실행한 `15-2-market-synthetic-control.py`가 이 검산을 직접 찍음  

```text
# 출처: lecture_practice/chapter15/results/15-2-market-synthetic-control.log (대조군 1)
  ③ 전체 풀 : 세계 A +0.1129 − 세계 B +0.1087 = +0.0041
  예측값     : 인접 가중 합 0.276 × 파급 0.0150 = +0.0041
  판정: 통과 — 오염량이 가중치×파급으로 정확히 설명된다
  ④ 인접 제외: 세계 A +0.1092 = 세계 B +0.1092  (차이 +0.0000 — 정의상 0이어야 한다)
```

□ 화면에 나온 값에서 읽을 것  
  ○ 오염의 크기를 정하는 것은 파급이 있느냐가 아니라 **파급받은 시장에 얼마나 가중치를 주었느냐**임. 가중치가 0이면 파급이 있어도 추정에 들어오지 않음  
  ○ 이 진단은 실데이터에서도 계산할 수 있음 — 가중치는 관측되기 때문임  

### 8.3 선택 규칙을 층으로 벗겨 원인을 찾는다

□ 같은 실행이 선택 규칙을 한 겹씩 벗긴 세계C를 함께 돌림  

```text
# 출처: lecture_practice/chapter15/results/15-2-market-synthetic-control.log (세계C — 선택 규칙 분해)
  C1 = 전체 40개 시장에서 무작위 선택 (선택 규칙을 완전히 제거)
  C2 = 진출 후보(시장 규모 상위 절반) 20개에서 무작위 선택 (규모 필터만 남김)
  A  = 후보 중 사전 성장률 1위 (실제 의사결정 규칙, 단일 사례)

추정 계단                      C1 평균     C2 평균      A 편향    C2 SD     C2 절댓값
① 사후 단순 비교                0.0013    0.2186    0.0394   0.1856     0.2220
② DID (전체 기증 풀)           0.0013   -0.0925    0.0827   0.0991     0.1188
③ SCM (전체 기증 풀)           0.0104    0.0094    0.0639   0.0321     0.0250
④ SCM (인접 제외 풀)          -0.0016    0.0105    0.0602   0.0519     0.0389
```
...
○ **방법 자체는 평균적으로 작동하는데 기업이 실제로 고르는 시장에서 가장 부정확한 것이며**, 원인은 7.4의 볼록껍질에 있음 — 성장률 상위 시장은 기증 풀 범위의 가장자리에 있어 재현할 조합이 적음  
○ 표의 `C2 SD` 열도 함께 읽음. 무작위로 골라도 편향의 산포가 크며(사후 단순 비교 `0.1856`), **처치 단위가 하나이면 무작위 선택조차 개별 추정의 편향을 없애 주지 않음** — 무작위 배정의 이점은 표본이 커야 나타남  

### 8.4 경제적 정합성으로 마지막 검산을 한다

□ 네 번째 코드를 실행함. 7절이 남긴 `15-2-estimate.json`을 읽어 추정치를 결정 단위로 환산함. 8.4절과 9절의 숫자가 모두 이 한 번의 실행에서 나옴  

```bash
python lecture_practice/chapter15/code/15-3-decision-economics.py
```
...
```text
# 출처: lecture_practice/chapter15/results/15-3-decision-economics.log (3) 두 경로의 대조
15-2의 추정: 시장 매출 +11.54% (placebo p=0.027, 유의)
15-2의 참값: 시장 매출 +5.02%

N (시장 내 동종 매장 수)                 참값 ATT 함의 전이율         추정 ATT 함의 전이율
10                                       49.8%                -15.4%
15                                       24.7%                -73.0%
20                                       -0.4%               -130.7%
25                                      -25.6%               -188.4%
30                                      -50.7%               -246.1%
   ...
   N      참값 전이율     참값 NPV     판정        추정 전이율     추정 NPV     판정  결론
  10       49.8%     -4,698     기각        -15.4%      5,515     통과  **뒤집힌다**
  15       24.7%        454     통과        -73.0%      5,515     통과  같다
  20       -0.4%      5,515     통과       -130.7%      5,515     통과  같다
  25      -25.6%      5,515     통과       -188.4%      5,515     통과  같다
  30      -50.7%      5,515     통과       -246.1%      5,515     통과  같다
```
...
□ 위 판정표에서 읽을 것 — 편향이 소수점 문제로 끝나지 않음  
  ○ `N = 10`에서 판정이 뒤집힘. 참값으로 계산하면 순현재가치 `−4,698만원`으로 기각인 출점을, 부풀린 추정치는 `+5,515만원`으로 통과시킴  
  ○ **N도 가정값임.** 시장 단위 추정치를 매장 단위 결정에 쓰려면 두 단위를 잇는 환산 계수가 필요한데, 그 계수가 N임. 실무에서 먼저 할 일은 더 정교한 추정이 아니라 **시장 안에 자사·경쟁 매장이 몇 개인지 세는 것**임  
...
○ 8.4절에서 실행한 `15-3-decision-economics.py`가 이 계산을 찍음. 아래 비용 네 개는 전부 **가정값**임  

```text
# 출처: lecture_practice/chapter15/results/15-3-decision-economics.log (1) 비대칭 손실과 임계비
예측: 점추정 100, 90% 구간 [70, 130]
  → 정규 가정으로 환산한 표준편차 σ = 18.24
  (구간 폭 = 2·z·σ. 수요 분포가 비대칭이면 이 환산은 틀린다.)

[신선식품] Cu 1,200원 / Co 2,000원
  임계비 = 1,200 / (1,200 + 2,000) = 0.375
  최적 발주량 = 수요분포의 37.5% 분위수 = 94.2 → 94단위
  검산(몬테카를로 기대손실 최소) = 94단위  일치

[내구재] Cu 15,000원 / Co 1,500원
  임계비 = 15,000 / (15,000 + 1,500) = 0.909
  최적 발주량 = 수요분포의 90.9% 분위수 = 124.4 → 124단위
  검산(몬테카를로 기대손실 최소) = 124단위  일치
```

○ 공식으로 구한 발주량과 몬테카를로로 기대손실을 최소화해 찾은 발주량이 두 경우 모두 일치함 — 임계비 공식이 계산 지름길이지 다른 답이 아니라는 확인임  
○ 위 출력을 표로 옮기면 이렇게 됨(90% 구간 [70, 130]을 정규 가정으로 환산하면 σ = `18.24`)  
...
```text
# 출처: lecture_practice/chapter15/results/15-3-decision-economics.log (2) hurdle rate와 NPV
연 12% → 월 할인율 0.00949, 60개월 연금계수 45.59
월 기여이익(전이율 0%) = 3,000만원 × 15% = 450만원

     전이율       월 순기여이익     NPV(만원)      회수기간  판정
      0%           450       5,515      41개월  통과
     15%           382       2,437      50개월  통과
     25%           338         386      58개월  통과
     35%           292      -1,666       미회수  기각

**손익분기 전이율 = 26.9%**

[민감도] 손익분기 전이율은 어느 가정에 민감한가
변화                                손익분기 전이율
기준                                   26.9%
기여이익률 15% → 12%                       8.6%
기여이익률 15% → 20%                      45.2%
초기 투자 1.5억 → 2.0억                     2.5%
계약 60개월 → 36개월                       -9.7%  ← 음수 = 자기잠식이 0이어도 기각
hurdle 12% → 8%                      32.9%
```

○ 위 출력을 표로 옮기면 이렇게 됨(기여이익률 15%, 초기 투자 1.5억원, 60개월, hurdle 연 12% — **전부 가정값**)  
...
○ 출력에는 한 행이 더 있음 — hurdle을 연 12%에서 8%로 낮추면 손익분기 전이율이 `32.9%`로 올라감. 자본의 기회비용을 어떻게 잡느냐가 상한을 그만큼 움직임  
...
○ 같은 실행의 마지막 블록이 수요를 위아래로 흔들어 실현 매출과 순현재가치를 다시 계산함  

```text
# 출처: lecture_practice/chapter15/results/15-3-decision-economics.log (4) 용량 제약
월 처리 가능 매출 상한 = 3,500만원 (기준 매출 3,000만원의 1.17배)

수요 시나리오                   수요(만원)       실현 매출       월 기여이익     NPV(만원)
하방 −20%                    2,400       2,400          360       1,412
기준                         3,000       3,000          450       5,515
상방 +20%                    3,600       3,500          525       8,934
상방 +40%                    4,200       3,500          525       8,934
```
...
□ 좌석 수, 주방 처리 능력, 재고 회전이 상한을 만들면 수요가 그 상한을 넘어도 실현 매출은 늘지 않음. **이 구조는 상방만 자르고 하방은 자르지 않음**  
  ...
  ○ 순현재가치로 보면 상방 `+3,419만원` 대 하방 `−4,103만원`. 기준 `5,515만원`에서 위로는 `8,934`까지만 올라가고 아래로는 `1,412`까지 내려간다는 뜻임. **입지 점수 상위가 곧 최선이 아님** — 용량을 늘리지 않은 채 점수만 높은 자리는 점수만큼의 현금흐름을 내지 못함  
...
○ **정밀도를 높이려 만든 짝지음이 오염을 늘림.** 닮은 시장이 곧 이웃이라 노출률이 0.904에서 0.959로 오르고, 표준편차는 가장 작은데 편향은 더 큼 (5.2)  
○ **설계만 바꿔 MDE가 일곱 배 달라짐.** 28.63%로는 못 재는 5.02% 효과를 3.94% 설계로는 잼. 착수를 접는 판정도 정보임 (6.3, 6.4)  
○ **기각률이 오른 것을 검정력이 좋아진 신호로 읽으면 안 됨.** 파급은 표준오차를 키우지 않고 추정치만 위로 옮김 (6.4)  
○ **사전 적합도가 좋아도 옳은 반사실이라는 보장이 없음.** 세 기준과 placebo를 모두 통과한 추정치가 참값의 2.3배였음 (7.3, 7.5)  
○ **방법이 평균적으로 작동해도 기업이 실제로 고르는 시장에서 가장 부정확할 수 있음.** 편향의 94%가 파급이 아니라 선택과 볼록껍질에서 왔음 (8.2, 8.3)  
○ **통계가 통과시킨 것을 산수가 잡음.** 유의 판정된 추정치를 결정 단위로 환산하니 함의 전이율이 −130.7%로 성립하지 않았음. 순서는 추정 → 유의성 → 경제적 정합성 → 결정임 (8.4)  
○ **실험이 가능한 처치라면 실험이 나음.** 짝지음 이중차분의 편향은 +0.0149이고 출처가 파급으로 확정되지만, 합성통제의 편향은 +0.0639이고 진단을 전부 통과하고도 틀렸음 (5.2, 8.3)  
○ **결정을 정하는 것은 모형이 아니라 비용 구조와 제도임.** 같은 예측구간에서 발주량이 94와 124로 갈리고, 순증이 가장 큰 후보가 기존 가맹점 영업지역 안이면 법적으로 실행할 수 없음 (9.1, 10.2)  
...
2. 그 숫자가 왜 그렇게 나왔는가 — 추정 계단의 값이라면 8.3의 선택 층 분해로 설명하라
3. **이 추정치는 통계 진단을 통과했다. 그대로 결정에 넣겠는가, 한 칸을 더 두겠는가? 그 한 칸은 무엇인가?**

○ **3번이 이 과제의 핵심.** 1·2번은 로그를 보면 답이 나오지만 3번은 8.4를 근거로 스스로 답해야 함  
