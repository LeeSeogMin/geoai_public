...
□ 4절부터 9절까지는 설명과 실습을 나누지 않음 — 각 절 첫머리에 실행할 명령이 나오고 그 아래에 코드와 실제 실행 결과가 이어짐. 순서대로 따라가면 이 장의 모든 수치를 직접 만들게 됨  
...
| 순서 | 실행할 파일 | 하는 일 | 절 |
| ---: | --- | --- | ---: |
| 1 | `7-0-simdata-prep.py` | 레이어 카탈로그와 교통량 시계열 생성 | 4·6절 준비 |
| 2 | `7-1-autonomous-gis-query.py` | 자연어 공간 질의와 검증 관문 | 4 |
| 3 | `7-2-spatiotemporal-uncertainty.py` | LSTM + MC Dropout 예측구간 | 6 |
| 4 | `7-0b-demand-simdata.py` | 점포 6곳 × 품목 2종 × 1,460일 수요 패널 생성 | 5·7~9절 준비 |
| 5 | `7-3-demand-newsvendor.py` | 정규화 conformal 구간과 임계비 발주 채점 | 5·7~9 |

○ 1번이 2·3번의 입력을 만들고 4번이 5번의 입력을 만듦 — **준비 코드(1·4번)를 먼저 실행**해야 나머지가 돌아감  
○ 다섯 코드 모두 인터넷 없이 돌아가고 GPU도 필요 없음  
○ 3·5번은 실제로 LSTM을 학습시킴. 가속기(CUDA·MPS)를 쓰면 LSTM 커널이 달라져 같은 시드에서도 소수점 아래가 흔들리므로, 코드가 장치를 CPU로 고정해 둠(`7-2`는 `DEVICE = torch.device("cpu")`, `7-3`은 `PIN_CPU = True`)  
...
□ 준비 코드와 첫 번째 코드를 실행함. 2절의 표와 이 절의 결과는 전부 이 두 명령의 출력임  
...
```text
# 출처: lecture_practice/chapter7/results/7-0-simdata-prep.log
  인구격자 400셀 → pop_grid.parquet
  도서관 6개소 → libraries.parquet, 학교 12개소 → schools.parquet (좌표계 EPSG:5179)
  교통량 시계열 1200시간 → traffic_volume.npy
```

○ 준비 코드가 만드는 것은 인구격자 400셀·도서관 6개소·학교 12개소의 가상 도시 카탈로그(EPSG:5179)와, 6절에서 쓸 교통량 1,200시간 시계열임  
...
□ 수요 패널을 만들고 이 장의 대표 실습을 실행함. **5·7·8·9절의 모든 표가 이 한 번의 실행에서 나옴**  
...
○ 준비 코드는 점포 6곳 × 품목 2종 × 1,460일 = 17,520행의 수요 패널을 두 벌 만듦 — 잡음 크기가 점포·수요 수준에 따라 달라지는 **본 데이터**와, 평균 수요는 같고 잡음 크기 구조만 없앤 **등분산 대조군**임(7.3절에서 씀)  
...
□ 세 번째 코드를 실행함. 4절에서 준비 코드가 만든 교통량 시계열을 입력으로 받아 두 성분을 각각 잼  
...
# 출력: 데이터: 1200시간 합성 교통량, 윈도우 24h → 다음 1h 예측
#       학습 823 / 검증 353 (앞 70% 학습, 시계열 누수 차단)
...
□ 같은 판단을 다른 방식으로도 확인할 수 있음. `7-3`은 "임계비를 얼마나 틀려야 예측 모델을 통째로 계절 나이브로 바꾼 것과 같은 손실이 나는가"도 계산해 `[5]` 구간 끝에 찍음  
...
○ **예측 모델은 학습이 필요 없는 규칙과 먼저 겨룸.** 실습에서 LSTM 23.45 대 계절 나이브 32.22 (5절)  
○ **예측 오차의 두 성분은 처방이 다름.** 모델 불확실성은 관측을 늘려 줄이고 데이터 잡음은 완충 자원으로 흡수하며, 두 성분은 제곱해 더한 뒤 제곱근을 취해 결합함 (6.2, 6.3)  
○ **전체 포함률만 보면 구조적 문제가 안 드러남.** 비정규화 방식은 전체로는 88.9%로 멀쩡해 보이지만 수요 3분위별 편차가 14.9%p였음 (6.4, 7.2)  
○ **단위마다 잡음 규모가 다르면 폭도 달라야 하고, 그 이득은 이분산이 있을 때만 나옴.** 등분산 대조군에서 폭 변동계수가 0.364에서 0.032로 떨어짐 (7.1, 7.3)  
○ **구간을 수량 하나로 바꾸는 값은 예측 모델 밖에 있음.** 임계비 CR = Cu/(Cu+Co)이며, 도시락 0.375와 우산 0.909가 0.5의 반대편에 놓임 (8.1)  
○ **구간 상한을 발주량으로 쓰면 안 됨.** 신뢰수준과 최적 서비스 수준은 다른 값이고, 도시락에서 이 혼동의 대가가 2.6배 손실이었음. 임계비는 방향이 중요하고 소수점 정밀도는 덜 중요함 (8.2, 9절)  
