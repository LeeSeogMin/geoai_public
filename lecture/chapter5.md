...
○ 모델은 지도 값을 정답 삼아 배우지만, 결정에 쓰고 싶은 값은 실제 값임. 이 간격은 4절(라벨 만들기)과 8절(오류의 방향)에서 다시 등장함  
...
○ 이 숫자는 지금 정하고 끝나는 값이 아님. 9절의 원가 계산에서 판독 1건의 단가와 손익분기 면적에 그대로 곱해짐. 패치 크기를 바꾸면 성능만 바뀌는 것이 아니라 원가 구조가 함께 바뀜  
...
□ **이 장부터 딥러닝(PyTorch)을 씀.** 여기에 더해 아래 명령을 한 번 실행함(`lecture_practice/README.md` 4단계)  
...
□ 위성영상은 4장 실습에서 내려받은 클립을 그대로 씀. 없으면 먼저 아래를 실행함  
...
| 순서 | 실행할 파일 | 하는 일 | 절 |
| ---: | --- | --- | ---: |
| 1 | `5-1-cnn-sentinel2-classification.py` | 패치 추출·라벨 붙이기, CNN 학습, 무작위·공간 분할 비교 | 3·4·7·8 |
| 2 | `5-2-baseline-vs-cnn.py` | 저비용 기준선 네 가지와 CNN을 같은 조건에서 채점 | 5~6 |
| 3 | `5-3-adoption-cost-breakeven.py` | 손익분기 면적과 민감도 계산 | 9 |

○ 3번은 2번이 남긴 `ch5_perf_handoff.csv`에서 실측 정확도를 읽으므로 **2번을 먼저 실행**해야 함. 그 파일이 없으면 3번은 계산을 시작하지 않고 스스로 멈춤  
○ 1번과 2번은 같은 시드로 같은 패치 724장을 각자 다시 만들어 쓰므로 둘 사이에는 실행 순서 제약이 없음  
○ 3절과 4절이 이미 인용한 값(밴드별 Macro-F1, 패치 2,318장→724장)도 1번 코드의 출력이며, 그 실행 명령은 7절 첫머리에 나옴  
○ 인터넷은 위의 4장 자료를 내려받을 때만 필요하고, 세 코드는 내려받은 클립을 로컬에서 읽어 돌아감. 그래픽카드는 없어도 됨  
○ 본문에 인용한 출력은 모두 `lecture_practice/chapter5/results/`의 실행 로그에서 그대로 가져온 것이며, 어느 파일에서 왔는지를 출력 블록 첫 줄에 적어 두었음 — 화면에 나온 값과 그 파일을 한 줄씩 대조하면 됨  

## 5. 기준선부터 채점한다

□ 두 번째 코드를 실행함. 5절과 6절의 표는 모두 이 한 번의 실행에서 나옴  
...
□ 코드가 가장 먼저 찍는 것은 클래스별 지수 평균임 — 규칙이 기대는 물리가 실제로 갈라지는지부터 확인함  

```text
# 출처: lecture_practice/chapter5/results/5-2-baseline-vs-cnn.log [1]
  지수 평균 (클래스별) — 규칙이 기대는 물리가 실제로 갈라지는지 확인
    클래스    |    NDVI |    NDWI |    NDBI
...
□ 고정 임계 규칙(R1)은 이 지수 평균에 문헌 임계를 그대로 대는 것이며 학습 파라미터가 0개임. 다섯 방법을 같은 패치 724장, 같은 공간 타일 5겹으로 나란히 채점함  

```python
# 주석에 붙인 출력의 출처: lecture_practice/chapter5/results/5-2-baseline-vs-cnn.log
import numpy as np
from sklearn.model_selection import GroupKFold

# [1] R1 고정 임계 규칙 — 물부터 떼고, 식생을 떼고, 남은 것 중 건조·불투수면을 시가지로 봄
...
# [2] 다섯 방법을 같은 타일 분할로 채점 — 폴드 목록을 한 번 만들어 모두에게 똑같이 씀
gkf = GroupKFold(n_splits=5)
folds = list(gkf.split(np.zeros(len(y)), groups=groups))
# 출력: R1 고정 임계 규칙            Acc=0.588±0.125  Macro-F1=0.514±0.037  학습=0.00초
#       R2 학습된 임계(트리)          Acc=0.880±0.060  Macro-F1=0.857±0.050  학습=0.01초
#       L  로지스틱 회귀             Acc=0.947±0.032  Macro-F1=0.934±0.034  학습=0.08초
#       RF 랜덤 포레스트             Acc=0.935±0.044  Macro-F1=0.926±0.053  학습=2.31초
#       CNN SimpleCNN    seed=42  Acc=0.975±0.011  Macro-F1=0.967±0.012  학습=6.12초
#       CNN 시드 3개의 Macro-F1 범위: 0.963 ~ 0.967 (폭 0.004)
```

○ 같은 출력을 파라미터 수·구현 난이도와 함께 정리하면 아래와 같음  
...
□ 화면에 나온 값에서 읽을 것  
  ...
  4. **CNN이 이기되 격차는 크지 않음.** Macro-F1 +0.032, Accuracy +0.028. 이 격차가 우연이 아닌지는 7절에서 확인함
...
○ CNN의 이 성질이 실측에서 어디에 나타나는지 확인함. 아래 두 줄은 5절에서 실행한 같은 명령의 `[4]` 구간(`lecture_practice/chapter5/results/5-2-baseline-vs-cnn.log`)에서 옮긴 클래스별 F1임  
...
□ CNN이 버는 몫은 **경작지(0.916→0.959)와 시가지(0.926→0.967)에 몰림.** 5절에서 분광지수가 붙어 있던 바로 그 두 클래스임  
...
□ 첫 번째 코드를 실행함. 3절의 밴드 비교, 4절의 패치 2,318장→724장, 이 절의 분할 비교, 8절의 클래스별 성능이 모두 이 한 번의 실행에서 나옴  
...
```python
# 주석에 붙인 출력의 출처: lecture_practice/chapter5/results/5-1-cnn-sentinel2-classification.log
from sklearn.model_selection import GroupKFold, KFold

# [1] 무작위 5겹 — 같은 타일의 이웃 패치가 학습과 검증에 갈라져 들어감
...
# 출력: Fold 1~5: Acc=0.945 0.972 0.979 0.959 0.965
#       평균: Acc=0.964±0.012, Macro-F1=0.964±0.013

# [2] 공간 분할 — 타일 ID를 그룹으로 묶어 검증 타일을 학습에서 통째로 제외
gkf = GroupKFold(n_splits=5)
for fold, (tr, va) in enumerate(gkf.split(range(len(dataset)), groups=tile_patch)):
    ...
# 출력: Fold 1~5: Acc=0.951 0.986 0.979 0.972 0.966
#       평균: Acc=0.971±0.012, Macro-F1=0.965±0.011
...
□ 같은 잣대를 5절의 CNN 대 기준선 격차에도 대야 함. CNN Macro-F1 0.967, 최고 기준선(로지스틱 회귀) 0.934, 격차 +0.032  
...
## 8. 오류의 방향: 재현율 0.889가 만드는 편의

○ 7절에서 실행한 같은 명령의 `[7]` 구간 출력임. 고정 분할로 떼어 둔 시험표본 143장의 클래스별 성능이며, 그 전체 정확도는 0.979로 찍힘(`lecture_practice/chapter5/results/5-1-cnn-sentinel2-classification.log`)  
...
□ 이 모델의 결과를 세어 지역 통계를 만들면 — 실제 시가지 패치 100장이 있는 구역에서 모델은 약 89장만 시가지로 부름. **그 구역의 시가지 비율은 실제보다 약 11% 낮게 산출됨**  
  ...
  ○ **총점 뒤에 방향이 있는 오류가 숨어 있음.** 이 고정 분할의 총점은 0.979이고 5절 5겹 교차검증의 평균 총점은 0.975인데, 어느 쪽을 적어도 위 두 건의 누락은 총점에 드러나지 않음. 결과를 통계로 보고할 때는 클래스별 정밀도·재현율을 함께 실어야 이 편의를 읽을 수 있음  
...
□ 세 번째 코드를 실행함. 이 절의 모든 금액과 면적이 이 한 번의 실행에서 나옴  
...
```text
# 출처: lecture_practice/chapter5/results/5-3-adoption-cost-breakeven.log [1]
  고정 임계 규칙 정확도 : 0.588
  최고 저비용 기준선     : 0.947  (L  로지스틱 회귀)
  CNN 정확도            : 0.975
  CNN − 기준선 격차      : +0.028
  학습에 쓴 라벨 패치     : 724장
```

○ 코드가 가장 먼저 하는 일은 성능을 새로 지어내지 않고 5절의 실측을 `ch5_perf_handoff.csv`에서 그대로 읽어 오는 것임 — 그 파일이 없으면 계산을 시작하지 않고 멈춤  

### 9.1 정확도 0.028이 결정할 수 있는 것은 없다
...
○ 값의 출처를 셋으로 나눠 적음. `[검증값]`은 출처가 있는 수치, `[산출값]`은 검증값에서 계산한 값, `[가정]`은 근거를 못 찾아 정한 값. 아래 표는 코드가 찍는 `[2]` 구간 출력을 옮긴 것임  
...
```python
# 주석에 붙인 출력의 출처: lecture_practice/chapter5/results/5-3-adoption-cost-breakeven.log
PATCH_M = 32 * 10                       # 32픽셀 × 10m 해상도
PATCH_KM2 = (PATCH_M / 1000) ** 2       # 판독 단위 0.1024 km²

# [1] 주기 1회 비용 — 인건비·영상비·오분류 처리비를 한 식에 담음
...
# 출력: [산출값] 판독 1건 인건비 = 1,023원 (= 2.0분 × 30,692원/시간)

# [2] 패치 1건의 손익 — 아끼는 인건비에서 더 내는 오분류 비용을 뺌
saved = LABOR_HUMAN
add_rule = (ACC_HUMAN - ACC_BASE) * COST_ERR
add_cnn = (ACC_HUMAN - ACC_CNN) * COST_ERR
# 출력: 아끼는 인건비           :      1,023원
#       규칙이 더 내는 오분류 비용 :      1,628원 (정확도 0.980 → 0.947)
#       CNN이 더 내는 오분류 비용 :        241원 (정확도 0.980 → 0.975)
#       → 규칙 순손익 -605원/건, CNN 순손익 +782원/건
```

□ 화면에 나온 값에서 읽을 것  
  ○ 판독 1건을 자동화하면 절감 인건비 = (2.0÷60)×30,692 = **1,023원**  
  ○ 대신 사람(정확도 0.980)보다 더 틀리는 만큼 처리비를 냄. 규칙(정확도 0.947)의 추가 오분류비는 **1,628원**, CNN(정확도 0.975)은 **241원**  
...
□ CNN은 연 12회 기준 **52.05km²부터** 비용을 갚음. 기초자치단체 하나가 대체로 수백km² 규모이므로 월 1회 판독하는 조직 하나면 이 컷라인을 넘음  
  ○ **판정을 가른 것은 자동화 여부가 아니라 정확도 0.947과 0.975의 차이임.** 그 0.028이 자동화를 손해에서 이득으로 뒤집음. 5절의 "격차는 크지 않음"이 여기서 "손익의 부호를 가르는 격차"로 바뀜  

○ 5절의 성능 비교가 이 절의 도입 판정으로 이어지는 경로  
...
### 9.4 어느 값에서 판정이 뒤집히는가

○ `[가정]`이 넷이므로 결론이 그 값에 얼마나 매달려 있는지 확인함. 아래는 같은 실행의 `[6]` 구간에서 기준 한 줄과 뒤집히는 네 줄을 뽑은 것임  
...
○ **CNN 전에 값싼 방법부터 같은 조건에서 채점함.** 고정 임계 0.514 → 임계만 학습 0.857 — 실패의 대부분은 정보 부족이 아니라 임계 부적합. 아래 계열이 위 계열을 못 이기면 위 계열을 씀 (5절, 그림 5.1)  
○ **CNN은 픽셀의 배치(무늬)를 읽는 모델.** 같은 필터를 모든 자리에 반복해 쓰므로(가중치 공유) 경계에서 큰 값, 균질한 자리에서 0을 냄. 실측에서 CNN의 이득은 분광지수가 붙어 있던 경작지(0.916→0.959)·시가지(0.926→0.967)에 몰림 — 색으로 안 갈리고 무늬로 갈리는 클래스가 있을 때 CNN이 값을 함 (6절)  
○ **채점은 공간 타일 단위로 나눠서 함.** 무작위 분할은 이웃 패치를 학습·검증 양쪽에 넣어 "본 동네의 옆 칸" 실력을 잼. 이 실습에서는 과대추정이 검출되지 않았지만, 미리 알 수 없으므로 공간 분할 성능을 항상 함께 보고함 (7절)  
○ **잡음 바닥보다 작은 차이는 결론이 아님.** 시드를 바꾸면 부호가 뒤집히는 차이(무작위 대 공간)는 "검출되지 않음"으로 적고, 다섯 폴드 전부 같은 방향인 차이(CNN 대 기준선 +0.032)는 신뢰함 (7절)  
○ **총점 뒤에 방향이 있는 오류가 숨음.** 시가지는 정밀도 1.000, 재현율 0.889로 놓치는 쪽으로만 틀리므로, 지역 통계의 시가지 비율이 약 11% 낮게 나옴. 클래스별 정밀도·재현율을 함께 보고해야 이 편의를 읽음 (8절)  
○ **격차는 비용으로 바꿔야 결정이 됨.** 건당 순손익이 규칙 −605원, CNN +782원으로 부호가 갈리고, CNN은 연 12회 판독 기준 52.05km²부터 개발비를 갚음. 정확도 0.028이 손익의 부호를 가름 (9절, 그림 5.2)  
○ **결론은 가정 값에 매달려 있음.** 오분류 처리비가 1만원이면 규칙도 이득, 50만원이면 CNN조차 사람을 못 이김. 절차를 가져가고 값은 자기 조직에서 다시 재며, 52.05km²를 예산 근거로 옮기지 않음 (9.4절)  
...
□ **3번이 이 과제의 핵심.** 1·2번은 결과를 들여다보면 답이 나오지만 3번은 9.4절의 판단이 필요함  
...
> - 가중치 초기화와 연산 순서가 완전히 고정되지 않기 때문임. 7절의 잡음 바닥 절차(시드 여러 개로 재실행)가 바로 이 문제에 대응하는 방법임
